...
//!
//! [`StdioHarness`] launches the `mcps-stdio-server` binary as a child process,
...
/// Launches `mcps-stdio-server` and exchanges newline-delimited messages.
#[derive(Debug, Clone)]
...
/// against:
///   - a JSON-RPC error object ⇒ its `error.message` (the frozen `mcps.*` code),
