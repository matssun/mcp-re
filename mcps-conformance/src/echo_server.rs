//! Native MCP-S echo server (MCPS-011) — the first conformance target.
//!
//! [`EchoServer`] natively speaks the MCP-S profile: it verifies an inbound
//! signed MCP-S request with `mcps_core::verify_request`, runs the trivial
//! `echo` tool, and returns a **signed** MCP-S response bound to the verified
//! request (`request_hash`). Verification failures become JSON-RPC error
//! objects carrying the `mcps.*` wire code (spec §12/§13).
//!
...
//!
//! This crate (`mcps-conformance`) MAY use `std`; `mcps-core` stays pure.
...
/// A native MCP-S echo server. Holds the server signing key, the inbound trust
...
impl EchoServer {
    ...
    /// MCP-S response on success, or a JSON-RPC error object on any failure).
    /// Never panics.
...
/// Build a signed MCP-S `tools/call` echo request (conformance/test helper).
///
