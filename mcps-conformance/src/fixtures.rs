...
//!
//! The fixed test identities and keys from MCPS_SPEC §10, in one place so every
...
/// A plain, MCP-S-unaware inner server: echoes `params.arguments.text` in an
...
/// Build the documented sidecar: the MCP-S proxy (same key/resolver/audience as
