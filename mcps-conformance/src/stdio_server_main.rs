//! `mcps-stdio-server` — a documented MCP-S server over stdio (MCPS-012/017).
//!
//! Reads newline-delimited JSON-RPC requests from stdin and writes one signed
//! MCP-S response (or JSON-RPC error object) line per request to stdout.
//!
//! Usage:
//!   mcps-stdio-server --now-unix <SECONDS> [--mode native|proxy]
//!
...
use mcps_conformance::build_server;
use mcps_conformance::serve_stdio;
...
fn parse_args(argv: &[String]) -> Result<Args, String> {
    ...
    let now_unix = now_unix.ok_or_else(|| {
        "usage: mcps-stdio-server --now-unix <SECONDS> [--mode native|proxy]".to_string()
    })?;
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("mcps-stdio-server: {err}");
            ExitCode::FAILURE
        }
    }
}
