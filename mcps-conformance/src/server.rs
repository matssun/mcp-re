...
//! shape. [`McpsServer`] unifies them so the stdio/HTTP harnesses can drive
...
pub enum ServerKind {
    /// The native MCP-S echo server.
    Native,
    /// An ordinary MCP server fronted by the MCP-S sidecar proxy.
    SidecarWrapped,
}
...
pub fn build_server(kind: ServerKind) -> Box<dyn McpsServer> {
    match kind {
