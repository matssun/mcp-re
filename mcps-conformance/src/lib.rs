//! MCP-S conformance harness library (MCPS-010..013).
//!
...
//! TRUTH at `components/mcps/mcps-core/tests/vectors/`) directly through
//! `mcps_core::verify_request` / `verify_response` and produces a deterministic,
...
//!
//! Crate boundary (ADR-MCPS-011/012): `mcps-conformance` may use `std::fs`
//! (vector loading) — `mcps-core` must not, and stays pure. No networking/async
...
pub use server::McpsServer;
pub use server::ServerKind;
pub use mcps_core::unix_to_rfc3339_utc;
