...
//!
//! The MCP-S audit-evidence vocabulary (`mcps-core/src/audit.rs`) derives its
//! rejection reasons from the FROZEN `McpsError::wire_code()` taxonomy
//! (`mcps-core/src/error.rs` is the sole authority) and adds ONLY the two
...
//!      `McpsError::wire_code()` (a minted parallel token);
...
//! and the method-name drift guard), with the `mcps-test-paths` cargo fallback —
//! so adding an `McpsError` variant (a new frozen wire_code) or editing the audit
...
const EXPECTED_SUCCESS_EVENTS: &[&str] = &["mcps.request.accepted", "mcps.response.signed"];

/// The two rejection `event_type`s. Each carries a frozen `wire_code()` token in
/// `reason`; neither is itself a `wire_code()` token.
const EXPECTED_REJECTION_EVENTS: &[&str] = &["mcps.request.rejected", "mcps.response.rejected"];
...
/// Every `mcps.*` wire token the frozen taxonomy renders, parsed from the
/// `wire_code()` match arms in `error.rs`. We scan the `=> "mcps...."` string
/// literals so a newly added `McpsError` variant (with its new wire_code) is
...
fn frozen_wire_codes(error_rs: &str) -> BTreeSet<String> {
    ...
    mcps_string_literals(body)
}

/// Every `mcps.*` token mentioned anywhere in `audit.rs` (event_type constants,
/// allowlists, doc examples). The audit vocabulary is small; we partition these
/// into the success/rejection event types and confirm no other `mcps.*` token is
/// minted as a reason that is not a frozen wire_code.
fn audit_mcps_tokens(audit_rs: &str) -> BTreeSet<String> {
    ...
    mcps_string_literals(production)
}
...
/// The `event_type` module's `pub const NAME: &str = "mcps...."` map, parsed from
...
/// `mcps.*` string value without depending on literal duplication.
...
/// Resolve the `mcps.*` values referenced inside a `&[ ... ]` slice body, where
/// each element is either a bare `mcps.*` string literal OR an `event_type::NAME`
/// reference resolved through [`event_type_consts`].
fn resolve_slice_tokens(slice_body: &str, consts: &[(String, String)]) -> BTreeSet<String> {
    let mut out = mcps_string_literals(slice_body);
...
/// Extract every double-quoted string literal beginning with `mcps.` from `text`.
fn mcps_string_literals(text: &str) -> BTreeSet<String> {
...
/// Condition (1): every `mcps.*` token the audit vocabulary uses is EITHER a
...
fn every_audit_token_is_a_wire_code_or_a_fixed_event_type() {
    ...
    assert!(
        foreign.is_empty(),
        "audit.rs mentions mcps.* token(s) that are neither a frozen McpsError::wire_code() nor \
...
fn success_event_set_is_exactly_the_two_item_allowlist() {
    ...
    assert_eq!(
        ...
        "audit.rs SUCCESS_EVENT_TYPES must be EXACTLY the two-item allowlist \
         (mcps.request.accepted, mcps.response.signed) — no third success event without an ADR \
         (ADR-MCPS-035 §3)"
    );
}

/// Condition (3): no audit `event_type` is itself a frozen `wire_code()` token.
/// A rejection sub-name like `mcps.request.rejected.bad_signature` would either
...
fn no_authorization_hash_mismatch_audit_reason() {
    let codes = frozen_wire_codes(&read("MCPS_CORE_SRC_ERROR"));
    assert!(
        !codes.contains("mcps.authorization_hash_mismatch"),
    ...
    let tokens = audit_mcps_tokens(&read("MCPS_CORE_SRC_AUDIT"));
    assert!(
        !tokens.contains("mcps.authorization_hash_mismatch"),
...
fn guard_inputs_are_non_empty() {
    ...
    assert!(
        codes.contains("mcps.invalid_signature"),
