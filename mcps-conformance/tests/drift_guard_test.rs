...
//!
//! The committed manifest at `components/mcps/mcps-conformance/conformance_manifest.json`
//! is the SINGLE SOURCE OF TRUTH for the MCP-S conformance corpus: every vector
//! (Core + Phase 5 authorization) and every `//components/mcps/...` Bazel
...
//!   4. a `rust_test` target is added/removed under `components/mcps` without a
...
//!
//! std + serde_json only (no new crates; mcps-conformance already depends on
//! serde_json and may use std::fs — mcps-core stays pure).
...
fn manifest() -> Value {
    serde_json::from_str(&read("MCPS_MANIFEST")).expect("conformance_manifest.json parses")
}
...
fn on_disk_core_vector_files() -> BTreeSet<String> {
    let manifest_path = locate("MCPS_CORE_MANIFEST");
...
/// delivered as data, producing the canonical `//components/mcps/<pkg>:<name>`
...
fn on_disk_test_targets() -> BTreeSet<String> {
    ...
    for (env_key, package) in [
        ("MCPS_BUILD_CONFORMANCE", "//mcps-conformance"),
        ("MCPS_BUILD_CORE", "//mcps-core"),
        ("MCPS_BUILD_HOST", "//mcps-host"),
        ("MCPS_BUILD_POLICY", "//mcps-policy"),
        ("MCPS_BUILD_PROXY", "//mcps-proxy"),
        // All 9 packages are scanned (MCPS-082, audit M-11/M-13): the manifest's
        // single-source-of-truth claim covers EVERY //components/mcps/... rust_test
        ...
        ("MCPS_BUILD_DEMO", "//mcps-demo"),
        ("MCPS_BUILD_DEMO_SERVER", "//mcps-demo-server"),
        ("MCPS_BUILD_TRANSPORT", "//mcps-transport"),
        ("MCPS_BUILD_FILESERVER", "//mcps-demo-fileserver"),
    ] {
...
/// Condition (4): the recorded `//components/mcps/...` rust_test target set must
