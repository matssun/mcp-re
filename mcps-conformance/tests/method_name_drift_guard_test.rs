...
//!
//! MCP-S Core is method-transparent: no NON-TEST `mcps-core/src` code path may
...
//! `TEST_SRCDIR`/`RUNFILES_DIR`), with the `mcps-test-paths` cargo fallback. A
...
/// The `mcps-core/src` directory, derived from the delivered `lib.rs` sentinel's
...
fn core_src_dir() -> PathBuf {
    let lib_rs = locate("MCPS_CORE_SRC_LIB");
    lib_rs
        .parent()
        .expect("mcps-core/src/lib.rs has a parent dir")
        .to_path_buf()
}

/// Every `*.rs` file physically in `mcps-core/src` (non-recursive: Core's
...
fn no_banned_method_literal_in_non_test_core_src() {
    ...
    assert!(
        violations.is_empty(),
        "banned MCP method-name literal(s) found in NON-TEST mcps-core/src — MCP-S Core must be \
...
fn guard_inputs_are_non_empty_and_exclusion_is_load_bearing() {
    ...
    assert!(
        files.len() >= 10,
        "scanned too few mcps-core/src files ({}) — runfiles wiring is broken",
        files.len()
    );
    assert!(
        files.iter().any(|(n, _)| n == "lib.rs"),
        "expected mcps-core/src/lib.rs among the scanned files"
    );
