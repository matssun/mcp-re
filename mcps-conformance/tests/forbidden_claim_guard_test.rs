...
//! same line — i.e. asserted as a capability MCP-S has. This is exactly the
...
//! `RUNFILES_DIR`), with the `mcps-test-paths` cargo fallback — so the guard
...
fn legacy_security_boundary_is_a_stub_with_no_live_claim() {
    let text = read("MCPS_DOC_SECURITY_BOUNDARY_STUB");
...
fn guard_inputs_are_non_empty() {
    ...
    assert!(
        !read("MCPS_DOC_SECURITY_BOUNDARY_STUB").trim().is_empty(),
...
fn detector_catches_asserted_claim_but_allows_repudiation() {
    ...
    let asserted = "MCP-S provides RBAC for every tool call.";
    assert_eq!(
