...
//! Phase 3): a verifying mTLS client to the remote MCP-S server/proxy.
//!
//! The pure `mcps-client-proxy` library abstracts the remote leg behind
//! [`RemoteTransport`] so its security pipeline is testable without real I/O.
//! This adapter binds that trait to [`mcps_transport::MtlsClient`] — a single
