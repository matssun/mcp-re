...
use mcp_re_client_core::resolve_signer_audience;
...
use mcp_re_client_core::SignerAudiencePolicy;
use mcp_re_client_core::TransportIdentity;
...
fn verify_with(bytes: &[u8], expected_audience: &str) -> Result<(), McpReError> {
    let key = SigningKey::from_seed_bytes(&SEED);
