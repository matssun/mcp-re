...
//!
//! This is the client-side mirror of `mcp-re-core`'s server-side
//! [`mcp_re_core::verify_request_draft02`]: given an ordinary MCP request (a
...
//! `params._meta["se.syncom/mcp-re.request"]`, computes the canonical signing
//! preimage with `mcp-re-core` (so signer and verifier share ONE preimage rule),
...
use mcp_re_core::DRAFT_02_CANONICALIZATION_ALLOWLIST;
...
impl RequestSigningInputs {
    ...
    /// (`mcp-re-jcs-int53-json-v1`). This is the normal constructor — a caller only
    ...
    /// inside the signed preimage. Use [`mcp_re_core::build_mcp_mrt_continuation`] to
...
/// maps to [`McpReError::CanonicalizationIdMissing`] and any non-allowlisted token
/// to [`McpReError::CanonicalizationIdNotAllowed`] — the client never emits
/// evidence under a scheme the profile does not admit.
fn check_canonicalization_id(id: &str) -> Result<(), McpReError> {
    if id.is_empty() {
        return Err(McpReError::CanonicalizationIdMissing);
    }
...
/// Construct and sign a draft-02 MCP-RE request.
///
...
/// 3. compute the canonical preimage via `mcp-re-core` and sign it with `signing_key`;
...
/// server-side [`mcp_re_core::verify_request_draft02`] accepts.
...
pub(crate) fn build_signed_request_with(
    ...
    params: Map<String, Value>,
    inputs: &RequestSigningInputs,
    sign: impl FnOnce(&[u8]) -> Result<String, McpReError>,
) -> Result<SignedRequest, McpReError> {
    ...
    if let Some(continuation) = &inputs.continuation {
        let value =
            serde_json::to_value(continuation).map_err(|_| McpReError::CanonicalizationFailed)?;
        envelope
            .as_object_mut()
    ...
    let wire_bytes = serde_json::to_vec(&request).map_err(|_| McpReError::CanonicalizationFailed)?;
    Ok(SignedRequest {
        wire_bytes,
        request_hash,
        object: request,
...
/// params and signs them (mirrors `mcp_re_host::HostSigner::sign_tool_call` but on
/// the draft-02 wire).
...
mod tests {
    ...
    fn unsupported_canonicalization_id_fails_closed() {
        let mut inputs = default_inputs();
        inputs.canonicalization_id = "mcp-re-jcs-floats-v2".to_string();
        let key = SigningKey::from_seed_bytes(&SEED);
        assert_eq!(
            build_signed_tool_call(&json!(1), "echo", json!({}), &inputs, &key).unwrap_err(),
