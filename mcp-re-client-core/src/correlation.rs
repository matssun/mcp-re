...
/// client error vocabulary never forks from `mcp-re-core`'s taxonomy.
...
impl CorrelationError {
    /// Map this local correlation failure to the frozen `mcp-re-core` wire taxonomy
    /// (MCPS-48): the client never invents a parallel wire reason. A duplicate id or
    /// nonce reuse is a replay ([`McpReError::ReplayDetected`]); an uncorrelatable
    /// (late-after-cleanup) response cannot be bound to any request, a response-
    /// binding failure ([`McpReError::ResponseHashMismatch`]); an expired entry is
    ...
    pub fn to_mcp_re_error(self) -> McpReError {
        match self {
            ...
            CorrelationError::Uncorrelatable => McpReError::ResponseHashMismatch,
            CorrelationError::Expired => McpReError::ExpiredRequest,
        }
    }
}
...
impl CorrelationStore {
    ...
    /// preimage ([`mcp_re_core::response_hash`]); this store binds, it does not verify.
...
mod tests {
    ...
    fn pending(correlation_id: &str, nonce: &str, deadline: i64) -> PendingRequest {
        PendingRequest {
            ...
            canonicalization_id: "mcp-re-jcs-int53-json-v1".to_string(),
