...
impl AudienceTuple {
    ...
    /// discriminator is rejected with [`McpReError::InvalidAudience`] (hostname alone
    ...
    pub fn to_audience_string(&self) -> String {
        format!(
            "mcp-re-audience:v1:scheme={};host={};port={};tenant={};route={};realm={}",
...
///
/// Fails closed when the route is unknown ([`McpReError::InvalidAudience`]) or when
/// the bound audience host does not match the verified transport host
/// ([`McpReError::TransportBindingFailed`]) — the audience is only valid on a
/// transport the client actually authenticated.
pub fn resolve_signer_audience(
    policy: &SignerAudiencePolicy,
    transport: &TransportIdentity,
    route_id: &str,
) -> Result<SignerAudienceBinding, McpReError> {
    let binding = policy
        .bindings
        .get(route_id)
        .ok_or(McpReError::InvalidAudience)?;
    if binding.audience.host() != transport.verified_host {
        return Err(McpReError::TransportBindingFailed);
    }
    Ok(binding.clone())
}
...
/// the resolved audience; a mismatch is [`McpReError::InvalidAudience`].
...
mod tests {
    ...
    fn request_audience_self_check() {
        ...
        assert_eq!(
            enforce_request_audience("mcp-re-audience:v1:scheme=https;host=other", &aud).unwrap_err(),
            McpReError::InvalidAudience
        );
    }
}
