...
//! semantics; CONTEXT.md §MCP-RE discovery).
//!
//! # Stateless-primary
//! There is NO trusted pre-flight "do you speak MCP-RE?" ask. The canonical model is
//! `local policy → signed request → verified signed response`: the FIRST verified
//! signed exchange IS both the proof of MCP-RE support and the discovery result
//! (an HSTS-like posture, not ask-then-trust). [`ProvenSupport`] can therefore only
//! be minted from a real [`mcp_re_core::VerifiedResponse`] — an advert can never
//! produce it.
//!
//! # Advisory advert (legacy/session transports only)
//! A server that still runs `initialize` MAY advertise MCP-RE under
//! `capabilities.experimental["se.syncom/mcp-re"]`. [`parse_legacy_advert`] reads it,
...
use mcp_re_core::EXTENSION_ID;
use mcp_re_core::VERSION_DRAFT_01;
use mcp_re_core::VERSION_DRAFT_02;
use serde_json::Value;
use std::collections::HashMap;

/// Proof of MCP-RE support derived from the first verified signed exchange. The ONLY
/// constructor takes an `mcp-re-core` [`VerifiedResponse`] (whose own construction is
...
/// Parse the advisory MCP-RE advert from an `initialize` `capabilities` object, if
/// present under `experimental["se.syncom/mcp-re"]`. Returns `None` when absent or
...
mod tests {
    ...
    fn self_contradictory_version_canon_is_protocol_error() {
        ...
        let bad2 = ExchangeCapability {
            version: VERSION_DRAFT_02.to_string(),
            canonicalization_id: Some("mcp-re-jcs-floats-v2".to_string()),
        };
        assert_eq!(
    ...
    fn parse_advert_present_and_absent() {
        let caps = json!({
            "experimental": {
                "se.syncom/mcp-re": {
                    "versions": ["draft-02"],
                    "canonicalization_ids": ["mcp-re-jcs-int53-json-v1"]
                }
            }
        });
        ...
        assert!(parse_legacy_advert(&json!({ "experimental": { "se.syncom/mcp-re": 7 } })).is_none());
    }
    ...
    fn proof_only_comes_from_a_verified_exchange() {
        ...
        // VerifiedResponse cannot be built outside mcp-re-core's verifier — so an
