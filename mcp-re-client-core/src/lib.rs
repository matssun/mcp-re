//! MCP-RE client-side core — the shared seam consumed by BOTH the local client
//! proxy and the SDK (ADR-MCPS-044 §`mcp-re-client-core`).
//!
//! This crate is the client-side mirror of `mcp-re-core`'s server-side verifier.
...
//!
//! # Why a separate crate (NOT in `mcp-re-core`)
//! `mcp-re-core` is the pure-crypto, method-transparent verification layer; pushing
//! client construction/policy into it would break that boundary (ADR-MCPS-044,
//! CONTEXT.md §`mcp-re-client-core`). Equally, the MODE-specific layers — the local
...
//! therefore depends only on `mcp-re-core` (pure primitives) and `serde_json`; it
...
//!   rejected under production `require_mcp_re`, rotation/revocation by config,
...
//!   [`correlation::CorrelationError::to_mcp_re_error`]) — no parallel wire taxonomy;
...
pub use correlation::InputRequiredRecord;
