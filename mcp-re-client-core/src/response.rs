...
//! request we sent, it confirms the response is genuine MCP-RE evidence bound to
//! THIS request. `mcp-re-core`'s [`verify_response_draft02`] (one shared verifier
...
//! `require_mcp_re` there is no fallback), `server_signer` resolution through the
...
//! reaches the network itself. The returned [`VerifiedResponse`] is `mcp-re-core`'s
...
use mcp_re_core::verify_response_draft02;
...
/// unknown signer with [`McpReError::ActorBindingFailed`]).
#[derive(Debug, Clone)]
...
pub fn verify_and_classify_response(
    ...
    resolver: &dyn TrustResolver,
    expectation: &ResponseExpectation,
) -> Result<ClassifiedResponse, McpReError> {
...
mod tests {
    ...
    fn signed_response(
        ...
        let preimage = mcp_re_core::response_signing_preimage(&object).unwrap();
    ...
    fn unsigned_response_is_rejected() {
        // A plain MCP response with no MCP-RE envelope: no fallback under require_mcp_re.
        let rh = expected_request_hash();
    ...
    fn input_required_response_classifies_non_terminal_with_hash() {
        ...
        // The response hash equals mcp-re-core's response_hash over the same bytes.
        let object: Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(c.response_hash, mcp_re_core::response_hash(&object).unwrap());
    }
    ...
    fn canonicalization_id_mismatch_fails_closed() {
        ...
        let rh = expected_request_hash();
        let bytes = signed_response(&rh, &SERVER_SEED, SERVER_SIGNER, SERVER_KEY_ID);
        let exp = ResponseExpectation::new(&rh, "mcp-re-jcs-int53-json-v9-mismatch");
        assert_eq!(
            verify_signed_response(&bytes, &resolver_with_server(), &exp).unwrap_err(),
