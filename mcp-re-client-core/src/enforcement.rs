...
//!
//! This is the bright line that decides — after an MCP-RE exchange has been
...
//! Production has exactly two modes — [`EnforcementMode::RequireMcpRe`] (strict,
//! fail-closed) and [`EnforcementMode::AllowLegacyExplicit`] (migration; legacy
//! only where config permits). `opportunistic_mcp_re` is deliberately EXCLUDED from
...
//! - **Absence** of MCP-RE evidence ([`EvidenceOutcome::Absent`]) — a connection
...
//! envelope ([`McpReError::MissingEnvelope`]) becomes [`EvidenceOutcome::Absent`];
...
/// modes). Deliberately a closed two-variant set — `opportunistic_mcp_re` is not a
/// normative mode and is not representable here.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EnforcementMode {
    /// Strict: every call MUST present a verified MCP-RE exchange. Absence and bad
    /// evidence both fail closed; the per-route legacy allowlist is ignored.
    RequireMcpRe,
    /// Migration: a verified MCP-RE exchange is accepted; ABSENCE of evidence may
    /// fall back to legacy ONLY for routes explicitly legacy-allowlisted. Bad
    /// evidence still fails closed.
    AllowLegacyExplicit,
}

/// Why MCP-RE evidence was ABSENT for an exchange — the fallback-eligible set
...
pub enum AbsenceReason {
    /// The connection/transport failed BEFORE any MCP-RE evidence was produced.
    TransportFailurePreEvidence,
    /// A plain-MCP / unsigned response with no MCP-RE envelope at all.
    PlainUnsigned,
...
/// The classified outcome of one attempted MCP-RE exchange. The engine decides
/// purely from this plus the mode and the route's legacy allowlist flag.
#[derive(Debug, Clone)]
pub enum EvidenceOutcome {
    /// A signed MCP-RE response verified successfully (MCPS-41).
    Verified(VerifiedResponse),
    /// No MCP-RE evidence was present (the fallback-eligible absence set).
    Absent(AbsenceReason),
    /// MCP-RE evidence was present but failed verification (bad/downgrade-shaped).
    /// Carries the frozen [`McpReError`] so the fail-closed reason is exact.
    Invalid(McpReError),
}
...
pub enum EnforcementDecision {
    /// The verified MCP-RE exchange satisfies policy — proceed with its result.
    AcceptMcpRe,
    ...
    /// Fail closed with the exact frozen wire reason (→ `McpReError::wire_code()`).
    FailClosed(McpReError),
}
...
/// absence is [`McpReError::MissingEnvelope`]; a connection failure or an unsigned
/// downgrade hint is a refused downgrade ([`McpReError::DowngradeForbidden`]) —
/// strict mode will not proceed in plaintext.
fn absence_fail_closed_code(reason: AbsenceReason) -> McpReError {
    match reason {
        AbsenceReason::PlainUnsigned => McpReError::MissingEnvelope,
...
/// envelope ([`McpReError::MissingEnvelope`]) is treated as absence (fallback-
...
/// downgrade.
pub fn classify_response_result(result: Result<VerifiedResponse, McpReError>) -> EvidenceOutcome {
    match result {
        Ok(verified) => EvidenceOutcome::Verified(verified),
        Err(McpReError::MissingEnvelope) => EvidenceOutcome::Absent(AbsenceReason::PlainUnsigned),
...
///
/// | outcome \ mode      | RequireMcpRe            | AllowLegacyExplicit                    |
/// |---------------------|------------------------|----------------------------------------|
/// | Verified            | AcceptMcpRe             | AcceptMcpRe                             |
...
pub fn decide(
    ...
    match outcome {
        ...
        EvidenceOutcome::Verified(_) => EnforcementDecision::AcceptMcpRe,
...
mod tests {
    ...
    const BAD_EVIDENCE: &[McpReError] = &[
        McpReError::InvalidSignature,
        McpReError::ResponseSigInvalid,
        McpReError::ActorBindingFailed, // unexpected server_signer
    ...
    fn classify_only_missing_envelope_is_absence() {
        assert!(matches!(
            classify_response_result(Err(McpReError::MissingEnvelope)),
    ...
    #[test]
    fn absence_fails_closed_under_require_mcp_re_regardless_of_allowlist() {
        for reason in ABSENCE {
            let outcome = EvidenceOutcome::Absent(*reason);
            for allowed in [true, false] {
                assert_eq!(
                    decide(EnforcementMode::RequireMcpRe, allowed, &outcome),
                    EnforcementDecision::FailClosed(absence_fail_closed_code(*reason)),
                    "absence {reason:?} under require_mcp_re must fail closed"
                );
            }
        }
    }
    ...
    fn every_fail_closed_reason_is_a_frozen_wire_code() {
        ...
        for reason in ABSENCE {
            ...
            assert!(code.wire_code().starts_with("mcp-re."));
        }
    }
}
