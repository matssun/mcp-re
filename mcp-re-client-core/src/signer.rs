...
//!   signer is rejected under production `require_mcp_re`. Hardware/KMS-only is a
...
//!
//! All custody/policy failures map to [`McpReError::ActorBindingFailed`] — there is
...
pub enum CustodyClass {
    ...
    /// `mcp-re-core::SigningKey`, e.g. ADR-028's production-capable file `KeySource`).
    ...
    /// dev/test; FORBIDDEN under production `require_mcp_re`.
    DevFileUnprotected,
}

impl CustodyClass {
    /// Whether this class is acceptable under production `require_mcp_re` WITHOUT the
...
pub enum Environment {
    /// Production: unprotected dev file keys are rejected under `require_mcp_re`.
    Production,
...
/// key-id set (rotation/revocation), the environment, whether `require_mcp_re` is in
...
/// dev-file rule (`require_mcp_re` + production rejects an unprotected file key).
/// Every failure is [`McpReError::ActorBindingFailed`] — no usable signing binding.
pub fn authorize_signer(policy: &SignerPolicy, signer: &dyn ClientSigner) -> Result<(), McpReError> {
    // Identity: the signer must be exactly the one policy bound to this route.
    if signer.signer_id() != policy.expected_signer {
        return Err(McpReError::ActorBindingFailed);
    }
    ...
    // Production base rule: under require_mcp_re an unprotected dev file key is
    // forbidden outside an explicitly-labelled dev/test environment.
    if policy.require_mcp_re
...
pub fn build_signed_request_with_signer(
    ...
    policy: &SignerPolicy,
) -> Result<SignedRequest, McpReError> {
...
/// In-process software signer over a seed-backed `mcp-re-core::SigningKey` (held
...
/// policy gate rejects it under production `require_mcp_re`. Use ONLY in
...
mod tests {
    ...
    impl ClientSigner for UnavailableSigner {
        ...
        fn sign_preimage(&self, _preimage: &[u8]) -> Result<String, McpReError> {
            Err(McpReError::ActorBindingFailed)
        }
    }

    #[test]
    fn matching_software_signer_authorizes_in_production_require_mcp_re() {
    ...
    #[test]
    fn dev_file_key_rejected_under_production_require_mcp_re() {
        let policy = SignerPolicy::new(SIGNER, Environment::Production, true);
    ...
    #[test]
    fn dev_file_key_allowed_in_production_when_not_require_mcp_re() {
        // allow_legacy_explicit (require_mcp_re = false): the strict dev-file rule
        // only bites under require_mcp_re.
    ...
    fn unavailable_signer_fails_closed_at_signing_time() {
        ...
        let inputs = RequestSigningInputs::with_default_canonicalization(
            ...
            "user:alice",
            "did:example:server",
            mcp_re_core::AuthorizationBinding::OpaqueBytes {
