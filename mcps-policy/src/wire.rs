...
//!
//! Mirrors `mcps_core::json_rpc_error_object` so a sidecar enforcing policy emits
...
//! `mcps.authorization_*` token. Authorization denials, like pre-dispatch
//! verification failures, are UNSIGNED and fail closed.

use mcps_core::MCPS_JSON_RPC_ERROR_CODE;
use serde_json::json;
use serde_json::Value;

use crate::error::PolicyError;

/// Serialize a [`PolicyError`] as the MCP-S JSON-RPC error object bound to the
