...
//! remote MCP-S endpoint, verifies the signed response, and returns plain MCP —
...
/// A remote MCP-S endpoint: verifies the signed request and returns a signed
/// response bound to the request_hash.
struct McpsRemote;
impl RemoteTransport for McpsRemote {
...
/// A legacy remote: returns a PLAIN, unsigned MCP response (no MCP-S envelope).
struct LegacyRemote;
...
fn plain_request() -> Value {
    // Ordinary MCP — NO MCP-S fields anywhere.
    json!({
        "jsonrpc": "2.0",
        "id": "req-1",
        "method": "tools/call",
...
fn signed_round_trip_returns_plain_mcp() {
    let mut proxy = proxy(route(EnforcementMode::RequireMcps, false), McpsRemote);
    let req = plain_request();
    let out = proxy.handle("tools", &req, &params()).expect("round trip");

    // Transparency: the input carried no MCP-S envelope...
    ...
    assert_eq!(out.path, mcps_client_core::ClientPath::McpsVerified);
}
...
fn legacy_route_under_explicit_policy_passes_through_audited() {
    ...
    assert_eq!(out.path, mcps_client_core::ClientPath::LegacyExplicit);
    assert_eq!(
        out.audit.outcome,
...
#[test]
fn unsigned_response_under_require_mcps_fails_closed() {
    // Same plain/unsigned remote, but strict mode -> no fallback.
    let mut proxy = proxy(route(EnforcementMode::RequireMcps, true), LegacyRemote);
    let err = proxy
        .handle("tools", &plain_request(), &params())
        .unwrap_err();
    match err {
        ProxyError::FailedClosed(e) => assert_eq!(e.wire_code(), "mcps.missing_envelope"),
