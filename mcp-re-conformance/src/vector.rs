...
/// otherwise it is an `mcp-re.*` wire token.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Expected {
    /// Verification (or, for raw fixtures, canonicalization) succeeds.
    VerifyOk,
    /// Verification fails with the given frozen `mcp-re.*` wire token.
    Error(String),
}
...
impl Expected {
    ...
    /// The wire token form (`"verify_ok"` or the `mcp-re.*` code) for reporting.
