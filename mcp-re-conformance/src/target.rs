...
//! `mcp_re_core::verify_request` / `verify_response`. Later phases (MCPS-012 stdio,
...
/// The documented test audience (signer/server seed scheme, MCP_RE_SPEC §10).
...
/// In-process target: runs vectors directly against `mcp-re-core` (MCPS-010).
...
impl ObjectTarget {
    ...
    fn map_result(result: Result<impl Sized, McpReError>) -> TargetOutcome {
        match result {
...
pub fn now_unix_for_case(case: &VectorCase) -> Result<i64, String> {
    ...
    let envelope = &message["params"]["_meta"]["se.syncom/mcp-re.request"];
    ...
    if case.expected == "mcp-re.expired_request" {
