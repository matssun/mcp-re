//! MCP-RE conformance CLI (MCPS-010).
//!
//! Usage:
//!   mcp-re-conformance object [--vectors-dir DIR] [--profile core] [--json]
//!
//! Runs the in-process object suite (committed vectors directly against
//! `mcp-re-core`) and prints the report (human by default, JSON with `--json`).
//! Exits non-zero if any case fails. Arg parsing is std-only — no clap.

use std::path::PathBuf;
use std::process::ExitCode;

use mcp_re_conformance::load_from_dir;
...
const DEFAULT_VECTORS_DIR: &str = "../mcp-re-core/tests/vectors";
...
fn parse_args(argv: &[String]) -> Result<Args, String> {
    ...
    let subcommand = iter.next().ok_or_else(|| {
        "usage: mcp-re-conformance object [--vectors-dir DIR] [--profile core] [--json]".to_string()
    })?;
...
fn main() -> ExitCode {
    match run() {
        ...
        Err(err) => {
            eprintln!("mcp-re-conformance: {err}");
            ExitCode::FAILURE
        }
    }
}
