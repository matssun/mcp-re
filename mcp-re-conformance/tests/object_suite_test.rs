...
//! vectors.
//!
//! The committed vectors at `components/mcp-re/mcp-re-core/tests/vectors/` are the
//! SINGLE SOURCE OF TRUTH. They are NOT duplicated into this crate: they are
//! embedded at compile time via `include_str!` of the sibling mcp-re-core package
//! paths (listed as `compile_data` from the `//components/mcp-re/mcp-re-core:vectors`
...
use mcp_re_conformance::parse_manifest;
use mcp_re_conformance::run_suite;
...
const MANIFEST: &str = include_str!("../../mcp-re-core/tests/vectors/manifest.json");

/// (manifest file name, embedded JSON text). Mirrors mcp-re-core's `committed()`.
fn embedded() -> Vec<(&'static str, &'static str)> {
    macro_rules! v {
        ($file:literal) => {
            (
                $file,
                include_str!(concat!("../../mcp-re-core/tests/vectors/", $file)),
            )
        };
    }
    vec![
...
fn runner_reports_failure_when_expected_is_wrong() {
    ...
    bogus.expected = "mcp-re.invalid_audience".to_string();
    ...
    assert_eq!(bogus_result.expected, "mcp-re.invalid_audience");
    assert_eq!(
