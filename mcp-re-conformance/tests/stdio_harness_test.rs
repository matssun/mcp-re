...
//!
//! Launches the `mcp_re_stdio_server` child process and drives the full MCP-RE
...
//! transport parity).
//!
//! The server binary is located from `MCP_RE_STDIO_SERVER` (set by the BUILD
//! target to `$(rlocationpath :mcp_re_stdio_server)`) resolved against the bazel
...
fn stdio_matches_object_outcomes_for_shared_request_vectors() {
    ...
    for raw in [
        ...
        let bytes = request_bytes(&case);
        let expected_hash = case_request_hash(&case);
        let stdio_token = if case.expected == "mcp-re.replay_detected" {
            let responses = harness()
