...
//! equivalence proof.
//!
//! MCP-RE Core is **method-transparent**: it treats every MCP message as an
...
//!
//! The MCP-RE signing preimage is the COMPLETE request object minus
...
//! object's frozen `mcp-re.*` code. The per-method request hashes differ (method
//! is signed), so we compare the Core DECISION, not response bytes.

use mcp_re_conformance::fixtures;
...
fn now_fresh() -> i64 {
    mcp_re_core::parse_rfc3339_utc(fixtures::ISSUED_AT).expect("parse issued_at") + 60
}
...
fn now_expired() -> i64 {
    mcp_re_core::parse_rfc3339_utc(fixtures::EXPIRES_AT).expect("parse expires_at") + 3600
}

/// Build a fully-signed MCP-RE request carrying `method`, re-signing the complete
...
///   - a JSON-RPC error object ⇒ its frozen `mcp-re.*` `error.message`;
///   - a signed response that verifies AND binds to `expected_request_hash`
///     ⇒ `"accepted"`;
///   - a signed response that does not verify/bind ⇒ the `mcp-re.*` wire code.
    ...
    for (i, method) in METHODS.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "Core verdict differs by method: '{method}' gave {token:?}, expected {baseline:?}. \
             MCP-RE Core must be method-transparent (ADR-MCPS-030)."
        );
    }
}
...
/// pass) nor worsen (no method changes which `mcp-re.*` code is emitted).
#[test]
fn rejected_verdict_is_identical_across_all_methods() {
    ...
    assert_eq!(
        baseline, "mcp-re.expired_request",
    ...
    for (i, method) in METHODS.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "Core rejection verdict differs by method: '{method}' gave {token:?}, \
             expected {baseline:?}. MCP-RE Core must be method-transparent (ADR-MCPS-030)."
        );
    }
}
...
fn accepted_and_rejected_verdicts_are_distinct() {
    ...
    assert_eq!(rejected, "mcp-re.expired_request");
    assert_ne!(
        accepted, rejected,
