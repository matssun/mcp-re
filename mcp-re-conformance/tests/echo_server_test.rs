//! MCPS-011 — native MCP-RE echo server round-trip tests (in-process).
//!
...
//! failures surface the correct `mcp-re.*` wire codes).

use mcp_re_conformance::build_signed_request;
use mcp_re_conformance::EchoServer;
...
fn tampered_request_is_rejected_invalid_signature() {
    ...
    assert_eq!(error_message(&resp), "mcp-re.invalid_signature");
}
...
fn replayed_request_is_rejected_on_second_submission() {
    ...
    assert_eq!(error_message(&second), "mcp-re.replay_detected");
}
