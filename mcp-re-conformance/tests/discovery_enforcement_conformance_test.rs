...
//! enforcement modes, exercised against the real `mcp-re-client-core` decision
...
//! map it (`//mcp-re-conformance:discovery_enforcement_conformance_test`).

use mcp_re_client_core::advert_mismatch;
...
use mcp_re_client_core::evaluate_capability;
use mcp_re_client_core::parse_legacy_advert;
use mcp_re_client_core::verify_signed_response;
use mcp_re_client_core::AbsenceReason;
use mcp_re_client_core::CapabilityPolicy;
use mcp_re_client_core::CapabilityVerdict;
...
use mcp_re_client_core::ExchangeCapability;
...
// ---- 1. advertised + valid accepted under require_mcp_re ----------------------

#[test]
fn v01_advertised_and_valid_accepted_under_require_mcp_re() {
    let rh = request_hash();
    let out = outcome(&good_response(&rh), &rh, &resolver());
    assert!(matches!(out, EvidenceOutcome::Verified(_)));
    assert_eq!(
        decide(EnforcementMode::RequireMcpRe, false, &out),
        EnforcementDecision::AcceptMcpRe
    );
}

// ---- 2. absent under require_mcp_re rejected ----------------------------------

#[test]
fn v02_absent_under_require_mcp_re_rejected() {
    let out = EvidenceOutcome::Absent(AbsenceReason::PlainUnsigned);
    assert_eq!(
        decide(EnforcementMode::RequireMcpRe, true, &out),
...
// ---- 4. unsigned response rejected (under require_mcp_re) ---------------------
...
fn v04_unsigned_response_rejected() {
    ...
    // Absence (no envelope) -> fails closed under require_mcp_re.
    assert!(matches!(
        out,
...
fn v05_invalid_server_signer_rejected() {
    ...
    assert!(matches!(
        out,
        EvidenceOutcome::Invalid(McpReError::ActorBindingFailed)
    ));
    assert!(matches!(
        decide(EnforcementMode::AllowLegacyExplicit, true, &out),
        EnforcementDecision::FailClosed(_)
...
fn v08_advert_vs_message_mismatch_does_not_downgrade() {
    ...
    let advert = parse_legacy_advert(
        &json!({ "experimental": { "se.syncom/mcp-re": { "versions": ["draft-01"] } } }),
    )
    .unwrap();
...
fn v10_tampered_discovery_does_not_weaken_policy() {
    // A tampered advert claiming no MCP-RE support cannot lower the verdict.
    let _tampered =
        parse_legacy_advert(&json!({ "experimental": { "se.syncom/mcp-re": { "versions": [] } } }));
    assert_eq!(
        evaluate_capability(&draft02_exchange(), &CapabilityPolicy::draft02_only()),
        CapabilityVerdict::SatisfiesPolicy
    );
...
fn v12_observability_change_does_not_change_decision() {
    ...
    assert_eq!(
        decide(EnforcementMode::RequireMcpRe, false, &base_out),
        decide(EnforcementMode::RequireMcpRe, false, &traced_out)
    );
}
...
fn v13_legacy_explicitly_allowed_succeeds_and_marked() {
    ...
    let audit = mcp_re_client_core::audit_for_decision(&decision);
...
fn v14_request_hash_mismatch_rejected() {
    ...
    assert!(matches!(
        out,
        EvidenceOutcome::Invalid(McpReError::ResponseHashMismatch)
    ));
    assert!(matches!(
        decide(EnforcementMode::AllowLegacyExplicit, true, &out),
        EnforcementDecision::FailClosed(_)
...
/// verification (its constructor is crate-private to mcp-re-core), so the accept
