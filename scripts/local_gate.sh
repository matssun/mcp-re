...
stage_static() {
  ...
  python3 scripts/jcs_vocabulary_gate.py --selftest \
    ...
    && python3 scripts/semantic_altitude_gate.py \
    ...
    `# ADR-MCPRE-069's CLOSURE criterion is NOT the merge-time question, and \`--closure\`` \
    `# is no longer run here. It carries two clauses: residue zero, and — since` \
    `# \`ratified_as\` became an enforced fact — the recorded proposition population zero.` \
    `# The first is already enforced above, and PRINTED, by the ratchet's own verdict line.` \
    `# The second is a campaign-length debt, and a merge path red for the length of a` \
    `# campaign is a gate nobody can act on. \`control-census --closure\` states the release` \
    `# criterion and is run at closure, by the campaign that owes it.` \
    ...
    && python3 tools/verification/test_compose.py \
...
# --- stage 5 (opt-in): the seven fleet proofs on a local kind cluster -------------
