//! End-to-end NEGATIVE / security-path suite (MCPS-050, MCP-RE-EPIC-P6 Child 6).
//!
...
//! MCP-RE error token (`error.message`), never merely "an error". The pieces are
...
//! error object whose `error.message` (and `error.data.mcp_re_error`) is the frozen
//! `mcp-re.*` reason code — observed directly here, printed line-per-case by the
...
/// `error.message` (which equals `error.data.mcp_re_error`). Returns `None` for a
...
fn denial_reason(response: &[u8]) -> Option<String> {
    ...
    // The structured denial line is self-consistent: message == data.mcp_re_error.
    assert_eq!(
...
fn missing_request_envelope_is_rejected_before_dispatch() {
    ...
    // Strip the MCP-RE request envelope from _meta: the proxy can no longer locate
