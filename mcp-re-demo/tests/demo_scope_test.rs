...
//! drives the SAME `mcp-re-proxy` serving path as `demo_authorization_test`, with
//! Phase 5 (ADR-MCPS-013) policy enforcement on and the real
//! `mcp-re-demo-fileserver` as the inner subprocess — but here the grant carries a
...
//! MCP-RE boundary, BEFORE dispatch:
//!
//!   1. a reader-signed `write_file` → `mcp-re.authorization_scope_denied`, and the
...
fn inner_binary() -> String {
    mcp_re_test_paths::resolve_runfile("INNER_FILESERVER_BIN")
        .to_string_lossy()
        .into_owned()
}
...
fn writable_demo_root() -> PathBuf {
    ...
    let root = base.join(format!("mcp-re-demo-scope-{}-{n}", std::process::id()));
