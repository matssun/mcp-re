...
//! `tools/call`, the reusable `mcp-re-transport` `MtlsClient` PRESENTS the client
//! cert + VERIFIES the server cert against a configured server CA + round-trips
//! the signed bytes over mTLS to a REAL `mcp_re_proxy::serve_once` server, and the
...
//! using `mcp-re-core` primitives (the same envelope the proxy produces). The full
//! multi-process wiring against `mcp_re_proxy_cli` is #3943; this validates the
...
fn make_ca() -> Ca {
    ...
    params.distinguished_name.push(DnType::CommonName, "mcp-re-test-ca");
...
/// Sign an MCP-RE response for `request_bytes` with the server key, bound to the
