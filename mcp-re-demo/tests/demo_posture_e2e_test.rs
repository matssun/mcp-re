//! Multi-process DEPLOYMENT-POSTURE proofs over the REAL `mcp_re_proxy_cli` binary
...
//!     `mcp_re_proxy_cli` (built without that feature) FAILS CLOSED on env key
...
//!     feature-built `//mcp-re-proxy:dev_env_key_source_test`. This is the anchor for
...
//!     across-PROCESS case.)
//!
//! Reuses the shared [`DemoFixtures`] (#3942) material, the `mcp-re-transport`
//! verifying client, and the `mcp-re-host` HostSession, exactly like the positive
...
fn spawn_env_keysource_proxy(
    ...
    const SEED_VAR: &str = "MCP_RE_C1_SIGNING_SEED";
    const CERT_VAR: &str = "MCP_RE_C1_TLS_CERT";
    const KEY_VAR: &str = "MCP_RE_C1_TLS_KEY";
    const CA_VAR: &str = "MCP_RE_C1_CLIENT_CA";
...
/// PRODUCTION `mcp_re_proxy_cli` binary (built without that feature) FAILS CLOSED on
...
/// feature-built unit test `//mcp-re-proxy:dev_env_key_source_test`
...
/// A spawned `mcp_re_proxy_cli` OS process bound to an EXPLICIT durable replay
...
fn build_grant(fixtures: &DemoFixtures, now: i64) -> DemoGrant {
    let spec = DemoGrantSpec {
        ...
        not_before: mcp_re_core::unix_to_rfc3339_utc(now - SKEW_SECS),
        expires_at: mcp_re_core::unix_to_rfc3339_utc(now + REQUEST_LIFETIME_SECS),
