//! Integration test for the demo proxy wiring (MCPS-047, MCP-RE-EPIC-P6 Child 3).
//!
//! These tests drive the EXISTING `mcp-re-proxy` serving path IN-PROCESS while it
//! launches the REAL `mcp-re-demo-fileserver` binary as its inner stdio
...
//!
//!   1. the proxy spawns `mcp-re-demo-fileserver` and a verified request reaches
...
/// Absolute path to the `mcp-re-demo-fileserver` binary in runfiles.
