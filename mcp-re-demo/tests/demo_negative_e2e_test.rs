...
//! `mcp_re_proxy_cli` OS process, reached over a REAL mTLS socket via the exported
//! `mcp-re-transport` client. Nothing reinvents crypto, policy, or transport: a
//! validly signed request (authored by the `mcp-re-host` `HostSession` / bare
...
//! refuses to bind it. Each case asserts the SPECIFIC frozen `mcp-re.*` reason
...
//! `mcp-re-proxy: inner-event inner_spawned …` on its stderr the instant it
...
/// `mcp-re-proxy: listening on <addr> (PEP; …)` (mcp-re-proxy/src/main.rs). Requires
...
/// Spawn the real `mcp_re_proxy_cli` with the full P1 flag set (mTLS,
...
fn spawn_proxy(fixtures: &DemoFixtures) -> ProxyProcess {
    ...
    // the proxy's own startup marker, printed by `mcp_re_proxy_cli` immediately
    // AFTER `TcpListener::bind` succeeds (mcp-re-proxy/src/main.rs): once it appears
...
// ---------------------------------------------------------------------------
// Signing + transport helpers (delegating to mcp-re-host + mcp-re-transport).
// ---------------------------------------------------------------------------
...
/// `error.data.mcp_re_error`) from a proxy response. `None` for a success.
fn denial_reason(response: &[u8]) -> Option<String> {
    ...
    assert_eq!(
        error["data"]["mcp_re_error"].as_str().expect("data.mcp_re_error"),
        message,
        "structured denial: message and data.mcp_re_error must agree"
    );
    Some(message)
}
...
fn a1_tampered_request_body_rejected_over_wire() {
    ...
    assert_eq!(
        ...
        Some(McpReError::InvalidSignature.wire_code())
    );
...
fn a3_replayed_request_rejected_on_fresh_connection() {
    ...
    assert_eq!(
        ...
        Some(McpReError::ReplayDetected.wire_code())
    );
...
fn a4_expired_request_rejected_over_wire() {
    ...
    assert_eq!(
        denial_reason(&response).as_deref(),
        Some(McpReError::ExpiredRequest.wire_code())
    );
...
fn a5_wrong_audience_rejected_over_wire() {
    ...
    assert_eq!(
        denial_reason(&response).as_deref(),
        Some(McpReError::InvalidAudience.wire_code())
    );
    assert!(!proxy.inner_was_reached(), "wrong-audience request must NOT reach inner");
}

// ===========================================================================
// A6 — missing MCP-RE request envelope → missing_envelope; inner not reached.
// ===========================================================================
...
fn a6_missing_envelope_rejected_over_wire() {
    ...
    // Strip the MCP-RE request envelope from _meta: the proxy can no longer
    ...
    assert_eq!(
        denial_reason(&response).as_deref(),
        Some(McpReError::MissingEnvelope.wire_code())
    );
...
fn a9_wrong_response_hash_rejected_by_client_over_wire() {
    ...
    assert_eq!(err, McpReError::ResponseHashMismatch);
...
fn a10_bad_response_signature_rejected_by_client_over_wire() {
    ...
    assert_eq!(err, McpReError::ResponseSigInvalid);
    assert_eq!(cl.pending_count(), 1, "a refused response leaves the pending entry");
}
