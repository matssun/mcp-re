//! The multi-process mTLS flow against the LONG-LIVED `mcp-re-demo-server`
//! (MCPS-066, MCP-RE-EPIC-P6.6B).
//!
...
//! correlation stay in `mcp-re-host`'s [`HostSession`](mcp_re_host::HostSession)
...
//! stay in `mcp-re-transport`'s [`MtlsClient`](mcp_re_transport::MtlsClient); the
//! per-scope reference grants are minted by `mcp-re-policy`; response verification
...
use mcp_re_core::extract_response_envelope;
...
pub struct PersistentCallOutcome {
    ...
    /// directly, not only via [`HostSession`](mcp_re_host::HostSession).
...
pub struct PersistentE2eOutcome {
    ...
    /// The frozen `mcp-re.*` reason the denied admin call was rejected with
...
pub enum PersistentE2eError {
    ...
    Grant(String),
    /// Signing a request via the [`HostSession`](mcp_re_host::HostSession) failed.
...
fn grant_artifact(
    ...
    let spec = ReferenceGrantSpec {
        ...
        operations,
        not_before: mcp_re_core::unix_to_rfc3339_utc(now_unix - skew_secs),
        expires_at: mcp_re_core::unix_to_rfc3339_utc(now_unix + lifetime_secs),
...
/// `proxy_addr` (a separate `mcp_re_proxy_cli --inner-mode persistent` OS process
/// fronting `mcp-re-demo-server`), using `fixtures` for all security material and
...
/// asserting it is rejected with `mcp-re.authorization_scope_denied`.
///
...
pub fn run_persistent_e2e(
    ...
    let denied_reason = denied_parsed
        ...
        .to_string();
    if denied_reason != "mcp-re.authorization_scope_denied" {
        return Err(PersistentE2eError::Denial(format!(
            "expected mcp-re.authorization_scope_denied, got: {denied_reason}"
        )));
    }
...
/// session ([`HostSession`](mcp_re_host::HostSession)).
...
///
/// This does NOT go through [`HostSession`](mcp_re_host::HostSession) or a
/// [`TrustResolver`](mcp_re_core::TrustResolver): it recomputes the canonical
...
pub struct PersistentE2eAssertions {
    ...
    /// `//mcp-re-transport:fault_injection_test`, not in this evidence object.
