...
//! human-facing `bazel run //mcp-re-demo:demo_e2e` wrapper drive against a REAL,
//! separately-spawned `mcp_re_proxy_cli` process over a REAL mTLS socket. It
//! reinvents NOTHING:
//!
//!   * signing, nonce, freshness, and `request_hash` correlation stay in
//!     `mcp-re-host`'s [`HostSession`](mcp_re_host::HostSession) (driven through the
//!     demo's [`DemoHostClient`](crate::DemoHostClient));
//!   * the mTLS connection, server-cert + server-identity verification, client-
//!     cert presentation, and HTTP framing stay in `mcp-re-transport`'s
//!     [`MtlsClient`](mcp_re_transport::MtlsClient);
//!   * the reference authorization grant is minted by `mcp-re-policy` via the
...
pub enum E2eError {
    ...
    Grant(String),
    /// Signing the request via the [`HostSession`](mcp_re_host::HostSession) failed.
...
///   1. mint the reference grant (`mcp-re-policy`) and derive its
///      `authorization_hash`;
///   2. SIGN ONE `list_files` request with the grant attached under
///      `params._meta` (`mcp-re-host` `HostSession`), storing the request hash by
///      id;
///   3. POST the signed bytes over mTLS (`mcp-re-transport` `MtlsClient`) — the
