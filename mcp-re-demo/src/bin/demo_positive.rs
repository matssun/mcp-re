//! Runnable positive happy-path demo (MCPS-049, MCP-RE-EPIC-P6 Child Issue 5).
//!
...
//!   -> mcp-re-proxy verifies the Core envelope
//!   -> mcp-re-proxy checks freshness / replay
//!   -> mcp-re-proxy evaluates Phase 5 authorization (ALLOW)
//!   -> mcp-re-proxy strips the external MCP-RE request envelope
//!   -> mcp-re-proxy injects the verified context (sole writer)
//!   -> mcp-re-demo-fileserver executes list_files
...
//! cargo run -p mcp-re-demo --bin demo_positive
//! ```
//!
