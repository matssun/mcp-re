...
//!   `mcp-re-host`'s [`HostSession`](mcp_re_host::HostSession) (which builds on
//!   `mcp-re-core`). The runner drives the session; it never touches the bare
...
//!   `mcp-re-transport`'s [`MtlsClient`](mcp_re_transport::MtlsClient). The runner
...
//!
//! The full multi-process wiring against `mcp_re_proxy_cli` is #3943; this module
//! is validated in-process against the real `mcp_re_proxy::serve_once` server, so
...
pub enum RunnerError {
    ...
    /// in-flight id, [`McpReError::ReplayDetected`]).
    ...
    /// [`McpReError::ResponseHashMismatch`], [`McpReError::ResponseSigInvalid`], or
    /// [`McpReError::MissingEnvelope`] for an unknown id).
