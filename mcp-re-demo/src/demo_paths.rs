...
//! var, so a public user running `cargo run -p mcp-re-demo --bin demo_positive`
...
//! 2. **Cargo-built binary** — `target/{debug,release}/mcp-re-demo-fileserver`
//!    relative to the workspace, for the public `cargo build --workspace --bins`
//!    quickstart.
//! 3. **Workspace-relative fixture** — `mcp-re-demo-fileserver/demo_root/` for the
...
//!
//! This intentionally does NOT reuse [`mcp_re_test_paths::resolve_runfile`]: that
...
/// `mcp-re-demo-fileserver` built by the same `cargo build` is found next to it.
...
/// Resolve the inner `mcp-re-demo-fileserver` binary, in fallback order:
...
/// `mcp-re-demo-fileserver/demo_root/` → runfiles.
