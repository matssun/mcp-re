//! Demo delegated-authorization wiring (MCPS-048, MCP-RE-EPIC-P6 Child Issue 4).
//!
...
//! the [`AuthorizationProfile`](mcp_re_policy::AuthorizationProfile) abstraction,
//! the Reference Signed Authorization Profile, the
//! [`PolicyEvaluator`](mcp_re_policy::PolicyEvaluator) (hash-binding check +
//! profile dispatch over a `VerifiedRequest`), and the proxy's opt-in
//! deny-before-dispatch enforcement path all live in `mcp-re-policy` /
//! `mcp-re-proxy`. This module only assembles them for the demo:
//!
...
//! * it builds a [`PolicyEvaluator`](mcp_re_policy::PolicyEvaluator) with the
//!   reference profile registered and hands it to the EXISTING
//!   [`build_demo_proxy`](crate::build_demo_proxy) wiring via
//!   [`Proxy::with_policy_enforcement`](mcp_re_proxy::Proxy::with_policy_enforcement).
//!
...
// depends on `mcp-re-demo-fileserver`; it launches its binary as a subprocess).
...
///
/// This mirrors the fileserver's INERT `net.mcp-re.intendedScope` tags. `Reader`
...
/// Build a [`PolicyEvaluator`](mcp_re_policy::PolicyEvaluator) with the Reference
...
/// supply its own [`InMemoryRevocationSource`](mcp_re_policy::InMemoryRevocationSource).
pub fn demo_revocation_source() -> InMemoryRevocationSource {
    InMemoryRevocationSource::new()
}

/// Assemble a demo [`Proxy`](mcp_re_proxy::Proxy) that launches the real
/// `mcp-re-demo-fileserver` AND enforces Phase 5 authorization before dispatch.
///
/// This is exactly [`build_demo_proxy`](crate::build_demo_proxy) with the
/// EXISTING [`Proxy::with_policy_enforcement`](mcp_re_proxy::Proxy::with_policy_enforcement)
...
/// closed with the matching `mcp-re.authorization_*` error and the inner fileserver
/// is never reached.
///
