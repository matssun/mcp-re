...
//! MCP-S SDK instead of the Rust reference proxy: the Python driver signs every
//! request with a NON-EXPORTING GCP Cloud KMS key (via the SDK's
//! `Signer.non_exporting` seam → `asymmetricSign`), and the Rust `mcps-proxy` PEP
...
//! (does not fail) when `MCPS_DRIVER_PYTHON` is unset; fails loud when it is set but
//! the KMS config is absent. Run from the cloud script (command 6):
//! ```sh
//! MCPS_DRIVER_PYTHON="…/.venv/bin/python -m mcps_sdk.driver" \
//!   MCPS_GCP_ACCESS_TOKEN=… MCPS_GCP_KEY_VERSION=<server> \
//!   MCPS_GCP_KEY_VERSION_CLIENT=<client> \
...
use mcps_walkthrough::SigningMode;
...
/// Build the Python driver from `MCPS_DRIVER_PYTHON` (whitespace-split command), or
/// `None` when it is unset (the lane is skipped, not failed).
fn python_driver() -> Option<ClientDriver> {
    let raw = std::env::var_os("MCPS_DRIVER_PYTHON")?;
...
fn python_client_signs_via_cloud_kms_across_the_four_hop() {
    let Some(driver) = python_driver() else {
        eprintln!("[t4-python-kms] SKIP: MCPS_DRIVER_PYTHON not set");
        return;
    };
