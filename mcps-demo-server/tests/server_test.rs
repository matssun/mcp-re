...
//! is MCP-S-UNAWARE: no signing, no envelopes, no verified context here. The
...
fn initialize_returns_well_formed_result() {
    ...
    assert_eq!(result["serverInfo"]["name"], "mcps-demo-server");
}
...
fn tools_list_includes_three_scoped_tools_after_initialize() {
    ...
    let scope_of = |name: &str| -> String {
        tools
            .iter()
            .find(|t| t["name"] == name)
            .and_then(|t| t["annotations"]["net.mcps.intendedScope"].as_str())
...
fn echo_public_tool_returns_the_message() {
    ...
    let response = handle(
        ...
        json!({
            ...
            "id": 3,
            "method": "tools/call",
            "params": { "name": "echo", "arguments": { "message": "hello mcps" } }
        }),
    );
    ...
    assert_eq!(result["content"][0]["text"], "hello mcps");
    assert_eq!(result["structuredContent"]["message"], "hello mcps");
}
