//! The long-lived demo server (MCPS-062): an MCP-S-UNAWARE stdio MCP server
...
//! `mcps-demo-fileserver`: a single process serves an `initialize` handshake
//! followed by ANY number of tool calls, staying alive until stdin EOF (or
//! `shutdown`). It knows nothing about MCP-S signing, envelopes, or verified
...
//! The server is MCP-S-UNAWARE: it does NOT enforce scopes. The "scope" is
//! pure metadata — surfaced in each tool's `annotations` (the
//! `net.mcps.intendedScope` key) and its description — so the policy layer can
...
/// A long-lived, MCP-S-UNAWARE MCP server over a small in-memory item set.
///
...
impl DemoServer {
    ...
    /// intended Phase-5 scope under `annotations.net.mcps.intendedScope` (the
