...
//!
//! This is the MODE-SPECIFIC adapter the pure `mcp-re-client-core` deliberately
...
//!
//! It reuses the live-tested backend from `mcp-re-proxy`
...
//! is the thin trait bridge and the `KeyError → McpReError` mapping (a delegated
...
use mcp_re_client_core::CustodyClass;
...
impl KmsClientSigner {
    ...
    /// otherwise an operator-supplied `MCP_RE_GCP_ACCESS_TOKEN` is used. Construction
    ...
    /// under the unmodified `mcp-re-core` verifier) offline.
    #[cfg(test)]
...
mod tests {
    ...
    #[test]
    fn kms_signed_preimage_verifies_under_mcp_re_core() {
        // The whole point: a signature produced through the KMS client-signer
        // bridge verifies under the unmodified mcp-re-core Ed25519 verifier, with no
        // network. (Mirrors the server-side kms_signature_verifies_under_mcp_re_core
        ...
        let pubkey = VerificationKey::from_bytes(
            &mcp_re_core::SigningKey::from_seed_bytes(&SEED).public_key().to_bytes(),
        )
        .expect("verify key");
        assert!(
            verify_ed25519(preimage, &sig_b64url, &pubkey).is_ok(),
            "a KMS-bridge signature must verify under mcp-re-core"
        );
    }

    #[test]
    fn passes_the_non_exporting_hardening_profile() {
        use mcp_re_client_core::authorize_signer;
    ...
    //   MCP_RE_GCP_KEY_VERSION=<client key version> MCP_RE_GCP_ACCESS_TOKEN=... \
    //     cargo test -p mcp-re-client-proxy-cli --features gcp_kms -- --ignored
    ...
    #[test]
    #[ignore = "requires a live GCP Cloud KMS client key (run with --ignored and MCP_RE_GCP_* set)"]
    fn gcp_kms_client_signs_a_draft02_request_that_verifies() {
        ...
        const CANON_ID: &str = "mcp-re-jcs-int53-json-v1";
