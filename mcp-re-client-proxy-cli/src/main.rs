//! `mcp-re-client-proxy-cli` — the binary front-end for the local client-side MCP-RE
...
//! `mcp-re-client-core`) signs a draft-02 envelope, forwards it to the remote MCP-RE
...
//! MCP-RE field — only surfaced security errors leak.
//!
...
fn build_kms_signer(
    ...
    let config = mcp_re_proxy::GcpKmsConfig {
...
fn build_proxy(args: &CliArgs) -> Result<(ClientProxy, SocketAddr), String> {
    ...
    // The static route: strict (require_mcp_re), no legacy fallback. The authz
    ...
    let route = Route {
        ...
        enforcement_mode: EnforcementMode::RequireMcpRe,
        ...
        authz_provider: Box::new(OpaqueBytesProvider::new(b"mcp-re-walkthrough-grant".to_vec())),
    };

    // Verifying mTLS transport to the remote MCP-RE server/proxy.
...
/// only ever sees plain MCP: a fail-closed MCP-RE verdict surfaces as its frozen
/// `mcp-re.*` wire reason; a transport/local failure surfaces as a diagnostic.
    ...
    for line in BufReader::new(stdin.lock()).lines() {
        ...
        let request: Value = match serde_json::from_str(&line) {
            ...
            Err(e) => {
                ...
                eprintln!("mcp-re-client-proxy-cli: parse error: {e}");
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("mcp-re-client-proxy-cli: {err}");
            ExitCode::FAILURE
        }
    }
}
