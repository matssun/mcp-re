...
- **SDK request/response transport adapter (`McpReHttpTransport`), Python and TypeScript.**
  ...
  **It is not a general standard-MCP transport.** One-way `notifications/*` carry no
  response and therefore no evidence, so the adapter drops every client→server
  notification — including `notifications/initialized` and `notifications/cancelled` —
  silently unless `on_dropped_notification` is installed. Whether MCP-RE signs one-way
  notifications, rejects them, or declares a request/response-only subset is undecided
  (#418). Callers still supply the HTTP leg via an injected `poster`; `connect_mtls_http` /
  `connectMtlsHttp` remain unbuilt (#413). The ADR-MCPS-047 open leg is implemented
  (`on_input_required` surfaces the answer leg's handles) but the adapter does not drive
  the answer leg (#419).
