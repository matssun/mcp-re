...
### Added
- **SDK transport adapter (`McpReHttpTransport`), Python and TypeScript.** A standard
  `mcp.ClientSession` / `Client` now speaks MCP-RE by construction: the adapter signs each
  outgoing request and verifies each incoming delegated response underneath it, so
  ...
  nor hang it. Callers still supply the HTTP leg via an injected `poster`;
  `connect_mtls_http` / `connectMtlsHttp` remain unbuilt.
- **A recorded delegated-session fixture** (`sdk/fixtures/delegated_response_replay.json`,
  ...
  replay a recording of a genuine delegated session offline instead. Recorded through the
  adapter itself and request-byte-asserted on replay, so the fixture also extends the
