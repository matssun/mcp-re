...
struct Fixture {
    ...
    check: Check,
    /// Expected outcome: `"verify_ok"` or an `mcp-re.*` wire code.
    expected: &'static str,
}
...
fn corpus() -> Vec<Fixture> {
    ...
    d04["params"]["_meta"][REQUEST_META_KEY]["canonicalization_id"] = json!("mcp-re-jcs-unknown-v9");
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        check: only.clone(),
        expected: "mcp-re.canonicalization_id_unknown",
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        check: only.clone(),
        expected: "mcp-re.authorization_binding_ambiguous_bytes",
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        check: only.clone(),
        expected: "mcp-re.downgrade_forbidden",
    });
    ...
    out.push(Fixture {
        ...
        check: Check::Response {
            ...
            expected_canonicalization_id: "mcp-re-jcs-future-floats-v2".to_string(),
        },
        expected: "mcp-re.canonicalization_id_mismatch",
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        check: only.clone(),
        expected: "mcp-re.continuation_type_unsupported",
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        check: only.clone(),
        expected: "mcp-re.continuation_malformed",
    });
...
/// scheme as mcp-re-conformance/acceptance_test.)
fn vectors_root() -> std::path::PathBuf {
    if let Ok(rel) = std::env::var("MCP_RE_CORE_VECTORS_MANIFEST") {
        ...
        panic!("MCP_RE_CORE_VECTORS_MANIFEST set but the runfile was not found (rel={rel})");
    }
...
fn write_draft02_fixtures() {
    ...
    let manifest_doc = json!({
        "schema": "mcp-re-draft-02-conformance/v1",
