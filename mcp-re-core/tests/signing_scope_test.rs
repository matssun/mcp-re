...
//! - mutating `protocolVersion` MUST fail (`mcp-re.invalid_signature`) — it is in
//!   scope and therefore integrity-protected.

use mcp_re_core::error::McpReError;
use mcp_re_core::ids::REQUEST_META_KEY;
...
/// (in scope) as peer keys alongside the MCP-RE request envelope.
