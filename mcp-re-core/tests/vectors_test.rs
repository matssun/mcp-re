//! MCP-RE conformance vectors: generator + golden + Phase-1 primitive assertions
//! (MCPS-002).
//!
//! This single integration-test crate:
//!   1. Builds every conformance vector from MCP_RE_SPEC §10 with REAL crypto, using
...
const REQUEST_META_KEY: &str = "se.syncom/mcp-re.request";
const RESPONSE_META_KEY: &str = "se.syncom/mcp-re.response";
...
struct Fixture {
    ...
    expected: &'static str, // "verify_ok" | "mcp-re.*"
...
/// Build every conformance fixture (MCP_RE_SPEC §10), deterministically.
fn all_fixtures() -> Vec<Fixture> {
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.invalid_signature",
        resolver: signer_resolver(),
        requires_pipeline: false,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.response_sig_invalid",
        resolver: server_resolver(),
        requires_pipeline: false,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.response_hash_mismatch",
        resolver: server_resolver(),
        requires_pipeline: true,
    });

    // replay — same nonce twice; the SECOND submission is mcp-re.replay_detected.
    out.push(Fixture {
        name: "replay_request",
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.replay_detected",
        resolver: signer_resolver(),
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.expired_request",
        resolver: signer_resolver(),
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.invalid_audience",
        resolver: signer_resolver(),
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.missing_envelope",
        resolver: None,
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.batch_forbidden",
        resolver: None,
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.notification_forbidden",
        resolver: None,
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        raw_text: None,
        raw_bytes_b64url: None,
        expected: "mcp-re.unknown_envelope_field",
        resolver: signer_resolver(),
        requires_pipeline: true,
    });
    ...
    out.push(Fixture {
        ...
        message: Value::Null,
        raw_text: None,
        raw_bytes_b64url: Some(mcp_re_core::b64url_encode(&[0x22u8, 0xFF, 0x22])),
        expected: "mcp-re.canonicalization_failed",
        resolver: None,
        requires_pipeline: false,
    });
    out.push(raw_fixture(
...
fn v3_response_signature_verifies() {
    ...
    assert!(
        verify_ed25519_with(&preimage, &sig, &key, McpReError::ResponseSigInvalid).is_ok(),
        "V3 must verify"
    );
}
...
fn v4_garbage_response_signature_fails() {
    ...
    let sig = response_sig_value(&msg);
    assert_eq!(
        verify_ed25519_with(&preimage, &sig, &key, McpReError::ResponseSigInvalid).unwrap_err(),
...
fn raw_bytes_for(file: &str) -> Vec<u8> {
    ...
    if let Some(text) = fx.get("raw_text").and_then(|v| v.as_str()) {
        ...
        mcp_re_core::b64url_decode(b64).expect("raw bytes decode")
    } else {
