...
//!   point, or a cryptographic mismatch) maps to [`McpReError::InvalidSignature`]
//!   via [`verify_ed25519`]. The response path needs the SAME failure to surface
//!   as [`McpReError::ResponseSigInvalid`]; rather than duplicate the logic we
...
/// declares the only algorithm MCP-RE v1 accepts, `Ed25519`.
///
...
/// alg is a signature failure in v1 (MCP_RE_SPEC §3), and the concrete sentinel
/// differs by path (request → [`McpReError::InvalidSignature`], response →
/// [`McpReError::ResponseSigInvalid`]), so the caller supplies `on_error` rather
/// than this helper inventing a new variant.
pub fn ensure_ed25519_alg(alg: &str, on_error: McpReError) -> Result<(), McpReError> {
...
/// is a trust-binding failure ([`McpReError::ActorBindingFailed`]) per spec §6.
#[derive(Debug, Clone)]
...
impl VerificationKey {
    ...
    /// curve point) → [`McpReError::ActorBindingFailed`].
    pub fn from_bytes(bytes: &[u8; 32]) -> Result<Self, McpReError> {
        let inner =
            DalekVerifyingKey::from_bytes(bytes).map_err(|_| McpReError::ActorBindingFailed)?;
        Ok(VerificationKey { inner })
    }

    /// Build a verification key from a Base64URL-no-pad string. Bad encoding,
    /// wrong length, or invalid point → [`McpReError::ActorBindingFailed`].
    pub fn from_b64url(s: &str) -> Result<Self, McpReError> {
        let decoded = b64url_decode(s).map_err(|_| McpReError::ActorBindingFailed)?;
...
/// [`verify_ed25519`] wrapper ([`McpReError::InvalidSignature`]); response-side
/// callers pass [`McpReError::ResponseSigInvalid`].
///
/// # Layering (MCPS-02)
...
/// a signed MCP-RE envelope MUST gate the declared algorithm with
...
/// Request-path Ed25519 verification: any failure → [`McpReError::InvalidSignature`].
///
...
mod tests {
    ...
    fn ensure_ed25519_alg_accepts_the_supported_alg() {
        assert_eq!(
            ensure_ed25519_alg(SIG_ALG_ED25519, McpReError::InvalidSignature),
            Ok(())
        );
    }
    ...
    fn ensure_ed25519_alg_rejects_unknown_alg_with_supplied_error() {
        ...
        assert_eq!(
            ensure_ed25519_alg("RS256", McpReError::InvalidSignature),
            Err(McpReError::InvalidSignature)
        );
        assert_eq!(
            ensure_ed25519_alg("ES256", McpReError::ResponseSigInvalid),
            Err(McpReError::ResponseSigInvalid)
        );
        // Case-sensitive: "ed25519" is not the canonical "Ed25519".
        assert_eq!(
            ensure_ed25519_alg("ed25519", McpReError::InvalidSignature),
            Err(McpReError::InvalidSignature)
        );
    }
    ...
    fn response_variant_maps_to_response_sig_invalid() {
        ...
        assert!(verify_ed25519_with(preimage, &sig, &vk, McpReError::ResponseSigInvalid).is_ok());
        // Tampered -> ResponseSigInvalid (not InvalidSignature).
        let mut tampered = preimage.to_vec();
        tampered[1] ^= 0x10;
        assert_eq!(
            verify_ed25519_with(&tampered, &sig, &vk, McpReError::ResponseSigInvalid).unwrap_err(),
