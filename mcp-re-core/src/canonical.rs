...
//! domain (MCP_RE_SPEC §4 / ADR-MCPS-005).
//!
//! This is the most security-critical unit in MCP-RE: the entire signature
...
//!
//! # JCS-safe domain (every violation => [`McpReError::CanonicalizationFailed`])
...
/// [`McpReError::CanonicalizationFailed`] rather than overflowing the stack.
...
pub fn canonicalize(input: &[u8]) -> Result<Vec<u8>, McpReError> {
...
pub fn parse(input: &[u8]) -> Result<JcsValue, McpReError> {
    // (2) Reject invalid UTF-8 up front.
    let text = std::str::from_utf8(input).map_err(|_| McpReError::CanonicalizationFailed)?;
    ...
    if !parser.at_end() {
        ...
        return Err(McpReError::CanonicalizationFailed);
    }
    Ok(value)
}
...
/// bound itself and fails closed via [`McpReError::CanonicalizationFailed`]
...
pub fn canonicalize_value(value: &JcsValue) -> Result<Vec<u8>, McpReError> {
...
pub fn canonicalize_json_value(value: &serde_json::Value) -> Result<Vec<u8>, McpReError> {
...
fn from_serde_value(value: &serde_json::Value) -> Result<JcsValue, McpReError> {
...
fn from_serde_value_at(value: &serde_json::Value, depth: usize) -> Result<JcsValue, McpReError> {
    match value {
...
fn check_safe_integer(i: i64) -> Result<(), McpReError> {
...
/// closed via [`McpReError::CanonicalizationFailed`] instead of overflowing the
...
/// it.
fn write_value(value: &JcsValue, depth: usize, out: &mut String) -> Result<(), McpReError> {
    match value {
...
impl<'a> Parser<'a> {
    ...
    fn expect(&mut self, expected: char) -> Result<(), McpReError> {
        match self.bump() {
    ...
    fn parse_value(&mut self) -> Result<JcsValue, McpReError> {
        match self.peek() {
    ...
    fn parse_literal(&mut self, literal: &str) -> Result<(), McpReError> {
    ...
    fn parse_bool(&mut self) -> Result<JcsValue, McpReError> {
        match self.peek() {
            Some('t') => {
    ...
    fn parse_null(&mut self) -> Result<JcsValue, McpReError> {
        self.parse_literal("null")?;
        Ok(JcsValue::Null)
    }

    fn parse_object(&mut self) -> Result<JcsValue, McpReError> {
        self.expect('{')?;
        self.skip_ws();
        ...
        loop {
            ...
            match self.bump() {
                Some(',') => continue,
                Some('}') => break,
                _ => return Err(McpReError::CanonicalizationFailed),
            }
        }
        Ok(JcsValue::Object(members))
    }

    fn parse_array(&mut self) -> Result<JcsValue, McpReError> {
        self.expect('[')?;
        self.skip_ws();
    ...
    fn parse_string(&mut self) -> Result<String, McpReError> {
        self.expect('"')?;
        let mut out = String::new();
        loop {
            match self.bump() {
                None => return Err(McpReError::CanonicalizationFailed),
                Some('"') => break,
                Some('\\') => {
                    let esc = self.bump().ok_or(McpReError::CanonicalizationFailed)?;
                    match esc {
                        '"' => out.push('"'),
                        '\\' => out.push('\\'),
                        '/' => out.push('/'),
    ...
    fn parse_unicode_escape(&mut self) -> Result<char, McpReError> {
        ...
        if (0xD800..=0xDBFF).contains(&first) {
            ...
            char::from_u32(combined).ok_or(McpReError::CanonicalizationFailed)
        } else if (0xDC00..=0xDFFF).contains(&first) {
            // Lone low surrogate => unpaired => reject.
            Err(McpReError::CanonicalizationFailed)
        } else {
            char::from_u32(first).ok_or(McpReError::CanonicalizationFailed)
        }
    }

    fn read_hex4(&mut self) -> Result<u32, McpReError> {
        let mut value = 0u32;
        for _ in 0..4 {
            let c = self.bump().ok_or(McpReError::CanonicalizationFailed)?;
            let digit = c.to_digit(16).ok_or(McpReError::CanonicalizationFailed)?;
    ...
    fn parse_number(&mut self) -> Result<JcsValue, McpReError> {
        let start = self.pos;
        if self.peek() == Some('-') {
            self.bump();
        }
...
mod tests {
    ...
    /// ADR-MCPS-037 / decision B.1 — the int53 scheme (`mcp-re-jcs-int53-json-v1`)
    ...
    fn jcs_05_exponent_number_rejected() {
        assert_eq!(canonicalize(b"1e3").unwrap_err(), McpReError::CanonicalizationFailed);
        assert_eq!(canonicalize(b"1E3").unwrap_err(), McpReError::CanonicalizationFailed);
    }
    ...
    fn trailing_comma_rejected() {
        assert_eq!(canonicalize(br#"{"a":1,}"#).unwrap_err(), McpReError::CanonicalizationFailed);
        assert_eq!(canonicalize(br#"[1,2,]"#).unwrap_err(), McpReError::CanonicalizationFailed);
    }
    ...
    fn json_value_helper_rejects_non_integer() {
        ...
        assert_eq!(canonicalize_json_value(&v).unwrap_err(), McpReError::CanonicalizationFailed);
    }
