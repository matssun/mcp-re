//! MCP-RE envelope data structures (MCP_RE_SPEC §2 / ADR-002, ADR-008).
//!
...
//! type-level enforcement of the fail-closed `mcp-re.unknown_envelope_field` rule
...
///
/// All frozen request fields in their exact wire order/names (MCP_RE_SPEC §2).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
///
/// `trust_label` is REMOVED from Core (MCP_RE_SPEC §2); response envelopes MUST
...
//   * `canonicalization_id: "mcp-re-jcs-int53-json-v1"` — the audit-facing record
...
/// the sibling `se.syncom/mcp-re.authorization` `_meta` block continues to carry
...
///
/// MCP-RE **binds and never interprets**: Core validates structure (`binding_type`
/// ∈ base set, mandatory fields, `digest_alg` shape); the `mcp-re-policy` profile
...
pub enum AuthorizationBinding {
    ...
    /// state. MCP-RE binds it; a configured reference profile/resolver validates.
...
/// `mcp-re.unknown_envelope_field` rule; a stray draft-01-shaped (e.g.
...
pub struct Draft02RequestEnvelope {
    ...
    pub version: String,
    /// Protected canonicalization-scheme id; must be `"mcp-re-jcs-int53-json-v1"`
...
/// The verified-context sidecar block (MCP_RE_SPEC §2 / ADR-008).
///
...
mod tests {
    ...
    // A frozen-vocabulary request envelope, constructed from MCP_RE_SPEC §2.
    ...
    fn draft02_request_envelope_carries_both_protected_identifiers() {
        ...
        assert_eq!(parsed.canonicalization_id, "mcp-re-jcs-int53-json-v1");
        assert_eq!(parsed.signer, "did:example:host");
        assert!(matches!(
