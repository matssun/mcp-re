//! Full verification pipeline (MCP_RE_SPEC §9 / ADR-004/005/006/007/009).
//!
...
//! step ordering is NORMATIVE (MCP_RE_SPEC §9) — each function fails closed at the
//! FIRST failing step and returns the listed [`McpReError`].
//!
...
//!
//! ## verify_request order (MCP_RE_SPEC §9, steps 1-12)
//! 1. Parse JSON; reject a top-level array (batch) -> [`McpReError::BatchForbidden`].
//! 2. Reject a notification (no `id`) -> [`McpReError::NotificationForbidden`].
//! 3. JCS-safe domain on the ORIGINAL raw bytes (duplicate keys, unsafe integers,
//!    invalid UTF-8, ...) -> [`McpReError::CanonicalizationFailed`].
//! 4. (steps 4-6) Locate / deny-unknown-fields / version-check the request
//!    envelope (-> [`McpReError::MissingEnvelope`] /
//!    [`McpReError::UnknownEnvelopeField`] / [`McpReError::UnsupportedVersion`],
...
//!      -> else [`McpReError::AuthorizationHashMissing`] (present-but-empty OR
//!      malformed; a STRUCTURALLY absent value already yielded the same token at
//!      step 5 — see the documented choice below).
//!    - `on_behalf_of` non-empty -> else [`McpReError::OnBehalfOfInvalidFormat`]
//!      (this is the present-but-empty case; a STRUCTURALLY absent `on_behalf_of`
//!      already yielded [`McpReError::OnBehalfOfMissing`] at step 5).
//!    - `signature.alg == "Ed25519"` -> else [`McpReError::InvalidSignature`];
//!      `signature.value` present -> else [`McpReError::InvalidSignature`].
//! 8. `audience == config.expected_audience` -> else [`McpReError::InvalidAudience`].
//! 9. Freshness window -> else [`McpReError::ExpiredRequest`].
//! 10. Resolve `(signer, key_id)` -> [`McpReError::ActorBindingFailed`] /
//!     [`McpReError::TrustResolverUnavailable`].
//! 11. Build the request signing preimage (`signature.value` removed) and verify
//!     Ed25519 -> [`McpReError::InvalidSignature`].
//! 12. Parse `expires_at`, then replay-cache check-and-insert ->
//!     [`McpReError::ReplayCacheUnavailable`] / [`McpReError::ReplayDetected`].
//!
//! ## Documented step-7 error choice (per MCP_RE_SPEC §9 step 7's request)
//! - `authorization_hash`: a structurally absent value maps to
//!   [`McpReError::AuthorizationHashMissing`] at step 5 (deserialization); a
...
//!   [`McpReError::OnBehalfOfMissing`] (P005) at step 5 (deserialization); a
//!   present-but-empty value maps to [`McpReError::OnBehalfOfInvalidFormat`] at
...
//!
//! ## verify_response order (MCP_RE_SPEC §9 verify_response, with the structural
...
//!    [`McpReError::MissingEnvelope`] / [`McpReError::UnknownEnvelopeField`].
//! 6. `signature.alg == "Ed25519"` -> else [`McpReError::ResponseSigInvalid`].
//! 7. Resolve `(server_signer, key_id)` -> [`McpReError::ActorBindingFailed`] /
//!    [`McpReError::TrustResolverUnavailable`].
...
//!    [`McpReError::ResponseHashMismatch`]. Vector 4B proves this fires even when
...
/// Verifier-side configuration for [`verify_request`] (MCP_RE_SPEC §9).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VerificationConfig {
    /// The verifier's own identity; an envelope's `audience` must equal this
    /// (step 8) or the request is rejected with [`McpReError::InvalidAudience`].
...
/// `mcp-re-policy` profile interprets it.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// The successful outcome of [`verify_request`] (MCP_RE_SPEC §9).
///
...
/// The successful outcome of [`verify_response`] (MCP_RE_SPEC §9).
///
...
impl VerifiedResponse {
    ...
    /// be forged from outside `mcp-re-core` (issue #83 / ADR-MCPS-003 type-as-evidence).
...
pub fn verify_request(
    ...
    if envelope.audience != config.expected_audience {
        return Err(McpReError::InvalidAudience);
    }
...
/// Verify a signed MCP-RE response end-to-end (MCP_RE_SPEC §9 verify_response
/// steps 1-7).
///
...
/// step 7 ([`McpReError::ResponseHashMismatch`]) fires even when the signature
...
pub fn verify_response(
    ...
    ensure_ed25519_alg(&envelope.signature.alg, McpReError::ResponseSigInvalid)?;
    ...
    let signature_value = envelope
        ...
        .value
        .as_deref()
        .ok_or(McpReError::ResponseSigInvalid)?;
    verify_ed25519_with(
        &preimage,
        signature_value,
        &key,
    ...
    if envelope.request_hash != expected_request_hash {
        return Err(McpReError::ResponseHashMismatch);
    }
...
/// Verify a signed MCP-RE **draft-02** request end-to-end. Mirrors
...
/// Verify a signed MCP-RE **draft-02** response end-to-end. Mirrors
...
/// [`McpReError::ResponseHashMismatch`]; a response declaring a different scheme
/// than the bound request → [`McpReError::CanonicalizationIdMismatch`] (decision
...
pub fn verify_response_draft02(
    ...
    if envelope.canonicalization_id != expected_canonicalization_id {
        return Err(McpReError::CanonicalizationIdMismatch);
    }
    if envelope.request_hash != expected_request_hash {
        return Err(McpReError::ResponseHashMismatch);
    }
...
/// a STARTUP error, distinct from the wire [`McpReError`] taxonomy: the service
...
/// Verify a signed MCP-RE request through the **dual verifier** with strict
...
///
/// - unknown/unrecognized version → [`McpReError::UnsupportedVersion`]
///   ("cannot select a known profile");
/// - a recognized version the `policy` forbids (e.g. `draft-01` under
///   [`ExpectedVersionPolicy::Draft02Only`]) → [`McpReError::DowngradeForbidden`]
...
pub fn verify_request_dispatch(
    ...
    if !policy.admits(&version) {
        // Recognized the (lower) profile, policy forbids it.
        return Err(McpReError::DowngradeForbidden);
    }
...
/// envelope → [`McpReError::MissingEnvelope`]; absent/non-string version →
/// [`McpReError::UnsupportedVersion`] (no profile can be selected).
fn read_request_envelope_version(value: &Value) -> Result<String, McpReError> {
    let envelope = value
        .get("params")
    ...
    envelope
        ...
        .map(str::to_string)
        .ok_or(McpReError::UnsupportedVersion)
}

/// Step 7 — `authorization_hash` must be present, non-empty, and `sha256:`-
/// prefixed. Absent/empty/malformed all -> [`McpReError::AuthorizationHashMissing`]
/// (documented choice; fails closed without a separate format error).
fn check_authorization_hash(authorization_hash: &str) -> Result<(), McpReError> {
    if authorization_hash.is_empty() || !authorization_hash.starts_with(SHA256_PREFIX) {
        return Err(McpReError::AuthorizationHashMissing);
    }
    Ok(())
}
...
/// [`McpReError::OnBehalfOfInvalidFormat`]. True absence is rejected earlier as
/// [`McpReError::CanonicalizationFailed`] at extraction (see module docs).
fn check_on_behalf_of(on_behalf_of: &str) -> Result<(), McpReError> {
    if on_behalf_of.is_empty() {
        return Err(McpReError::OnBehalfOfInvalidFormat);
    }
    Ok(())
}

/// Step 7 — `signature.alg == "Ed25519"` and `signature.value` present. Any
/// other alg, or an absent value, -> [`McpReError::InvalidSignature`] (unknown alg
/// is treated as a signature failure in v1, MCP_RE_SPEC §3).
fn check_signature_block(alg: &str, value: Option<&str>) -> Result<(), McpReError> {
    ensure_ed25519_alg(alg, McpReError::InvalidSignature)?;
    if value.is_none() {
        return Err(McpReError::InvalidSignature);
    }
    Ok(())
}
...
mod tests {
    ...
    fn on_behalf_of_empty_is_invalid_format() {
        assert_eq!(
            check_on_behalf_of(""),
            Err(McpReError::OnBehalfOfInvalidFormat)
        );
    ...
    /// identifiers (version="draft-02", canonicalization_id="mcp-re-jcs-int53-json-v1").
    ...
    fn draft02_response_scheme_mismatch_is_rejected() {
        ...
        assert_eq!(
            verify_response_draft02(
                ...
                &server_resolver(),
                &verified_req.request_hash,
                "mcp-re-jcs-future-floats-v2",
            ),
            Err(McpReError::CanonicalizationIdMismatch)
        );
    }
    ...
    fn dispatch_draft01_under_draft02_only_is_downgrade_forbidden() {
        ...
        assert_eq!(
            ...
            Err(McpReError::DowngradeForbidden)
        );
    }
    ...
    fn unavailable_resolver_is_trust_resolver_unavailable() {
        ...
        assert_eq!(
            ...
            Err(McpReError::TrustResolverUnavailable)
        );
    }
    ...
    fn unavailable_replay_cache_is_replay_cache_unavailable() {
        ...
        assert_eq!(
            ...
            Err(McpReError::ReplayCacheUnavailable)
        );
    }
    ...
    fn response_hash_mismatch_with_valid_signature() {
        ...
        assert_eq!(result, Err(McpReError::ResponseHashMismatch));
        assert_ne!(result, Err(McpReError::ResponseSigInvalid));
    }
    ...
    fn response_bad_signature_is_response_sig_invalid() {
        ...
        assert_eq!(
            ...
            Err(McpReError::ResponseSigInvalid)
        );
    }
