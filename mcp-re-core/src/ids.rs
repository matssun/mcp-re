//! Frozen string constants for the MCP-RE security profile (MCP_RE_SPEC §1).
//!
...
/// `se.syncom` root by ADR-MCPS-027; MCP_RE_SPEC §1). It appears inside the signed
...
pub const CANONICALIZATION_ID_INT53_V1: &str = "mcp-re-jcs-int53-json-v1";
...
/// be a member; absent → `mcp-re.canonicalization_id_missing`, unrecognized →
/// `mcp-re.canonicalization_id_unknown`, recognized-but-not-here →
/// `mcp-re.canonicalization_id_not_allowed`. This constant is the seam through
...
/// digest + reference, bound (never interpreted) by MCP-RE.
...
/// as [`crate::McpReError::ContinuationTypeUnsupported`]; future continuation
...
/// (scalar/array/null) before signing — see `mcp-re-proxy` `build_signed_response`
...
/// response carrying no `result`) before signing — see `mcp-re-proxy`
...
/// W3C Trace Context `_meta` keys that are EXCLUDED from the MCP-RE signing
...
mod tests {
    ...
    fn meta_keys_are_namespaced_under_the_extension_id() {
        assert_eq!(REQUEST_META_KEY, "se.syncom/mcp-re.request");
        assert_eq!(RESPONSE_META_KEY, "se.syncom/mcp-re.response");
        assert_eq!(VERIFIED_META_KEY, "se.syncom/mcp-re.verified");
    ...
    fn draft02_identifier_constants() {
        ...
        assert_eq!(CANONICALIZATION_ID_INT53_V1, "mcp-re-jcs-int53-json-v1");
    ...
    fn observability_keys_are_the_w3c_trace_context_set() {
        ...
        // None of the excluded observability keys may collide with the MCP-RE
