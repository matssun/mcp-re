...
//! returns a non-object or an error (see `mcp-re-proxy` `build_signed_response`):
//!
...
///
/// Returns [`McpReError::CanonicalizationFailed`] only for the structurally
/// impossible-post-verification case where `result` is not a JSON object.
pub fn unwrap_verified_result(result_value: &Value) -> Result<UnwrappedResult, McpReError> {
...
mod tests {
    ...
    fn signed_meta() -> serde_json::Value {
        json!({ "se.syncom/mcp-re.response": { "signature": { "alg": "Ed25519" } } })
    }
