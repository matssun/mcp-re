//! Signing-preimage and `request_hash` construction (MCP_RE_SPEC §3 / ADR-004).
//!
//! The MCP-RE signing rule signs the COMPLETE JSON-RPC object, not just the
...
pub fn request_signing_preimage(object: &Value) -> Result<Vec<u8>, McpReError> {
...
/// `signature.value`, canonicalize.
pub fn response_signing_preimage(object: &Value) -> Result<Vec<u8>, McpReError> {
...
///
/// The MCP-RE signed region is the COMPLETE JSON-RPC object MINUS exactly two
...
/// signature block → [`McpReError::MissingEnvelope`].
pub fn signing_preimage(object: &Value, location: EnvelopeLocation) -> Result<Vec<u8>, McpReError> {
...
///    (req `params._meta["se.syncom/mcp-re.request"].signature.value`;
///     resp `result._meta["se.syncom/mcp-re.response"].signature.value`); and
...
/// `request_hash` (MCP_RE_SPEC §3.6): SHA-256 of the REQUEST signing preimage,
/// formatted `sha256:<b64url-no-pad>`.
pub fn request_hash(request_object: &Value) -> Result<String, McpReError> {
...
pub fn response_hash(response_object: &Value) -> Result<String, McpReError> {
...
/// [`McpReError::MissingEnvelope`] if the container, `_meta`, the envelope, or its
/// `signature` block is absent.
fn strip_signature_value(object: &mut Value, location: EnvelopeLocation) -> Result<(), McpReError> {
...
mod tests {
    ...
    fn preimage_equals_canonicalize_of_object_with_value_removed() {
        ...
        expected_obj["params"]["_meta"]["se.syncom/mcp-re.request"]["signature"]
            .as_object_mut()
            .unwrap()
            .remove("value");
    ...
    fn missing_signature_block_errors() {
        ...
        obj["params"]["_meta"]["se.syncom/mcp-re.request"]
            .as_object_mut()
            .unwrap()
            .remove("signature");
        assert_eq!(
    ...
    fn request_hash_independent_of_signature_value() {
        ...
        other_value["params"]["_meta"]["se.syncom/mcp-re.request"]["signature"]["value"] =
    ...
    fn draft02_request_object() -> Value {
        let mut obj = request_object();
        let env = obj["params"]["_meta"]["se.syncom/mcp-re.request"]
            .as_object_mut()
            .unwrap();
    ...
    fn draft02_predicate_excludes_container_trace_but_keeps_protected_and_nested() {
        ...
        assert!(text.contains("mcp-re-jcs-int53-json-v1"));
        ...
        let pretty = r#"{
            ...
            "params": {
                ...
                "_meta": {
                    "se.syncom/mcp-re.request": {
                        "version": "draft-02",
                        "canonicalization_id": "mcp-re-jcs-int53-json-v1",
                        "signer": "did:example:host",
                        "on_behalf_of": "user:alice",
                        "audience": "did:example:server",
                        "authorization_hash": "sha256:AAAA",
    ...
    fn draft02_mutating_protected_fields_changes_the_preimage() {
        ...
        for (field, value) in [
            ...
            ("canonicalization_id", json!("mcp-re-jcs-other-v1")),
        ] {
            let mut m = draft02_request_object();
            m["params"]["_meta"]["se.syncom/mcp-re.request"][field] = value;
            assert_ne!(
                baseline,
        ...
        for (field, value) in [("alg", json!("RS256")), ("key_id", json!("key-2"))] {
            let mut m = draft02_request_object();
            m["params"]["_meta"]["se.syncom/mcp-re.request"]["signature"][field] = value;
            assert_ne!(
                baseline,
                request_signing_preimage(&m).expect("preimage"),
    ...
    fn exclusion_predicate_enumerates_exactly_the_documented_paths() {
        ...
        assert_eq!(
            ...
            vec![
                vec![
                    "params".to_string(),
                    "_meta".to_string(),
                    "se.syncom/mcp-re.request".to_string(),
                    "signature".to_string(),
                    "value".to_string()
                ],
                vec![
        ...
        assert_eq!(resp[0][2], "se.syncom/mcp-re.response");
