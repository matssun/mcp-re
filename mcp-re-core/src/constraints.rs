...
//! (MCP_RE_SPEC §7 / §9 steps 1, 2, 4, 5, 6).
//!
...
//! crypto (MCP_RE_SPEC §9 notes "steps 1-2 and 4-7 are cheap structural checks
...
//!   is forbidden (`mcp-re.batch_forbidden`).
//! - [`reject_notification`] — §9 step 2: a JSON-RPC notification (no `id`) is
//!   forbidden for the security path (`mcp-re.notification_forbidden`).
//! - [`extract_request_envelope`] / [`extract_response_envelope`] — §9 steps 4-6:
//!   locate the envelope under the appropriate `_meta` key (absent ->
//!   `mcp-re.missing_envelope`), deserialize against the `deny_unknown_fields`
//!   struct (an unknown field -> `mcp-re.unknown_envelope_field`; a wrong-type or
//!   missing required field -> `mcp-re.canonicalization_failed`), then version-check
//!   the request envelope (`!= "draft-01"` -> `mcp-re.unsupported_version`).
//!
...
/// *unknown* id (names no scheme at all → `mcp-re.canonicalization_id_unknown`)
/// from a *recognized-but-disallowed* id (a real scheme not admitted by the
/// active profile → `mcp-re.canonicalization_id_not_allowed`). In v0.6 exactly one
...
///
/// MCP-RE Core forbids batches outright (MCP_RE_SPEC §7): a top-level JSON array
/// maps to [`McpReError::BatchForbidden`]. Any non-array top-level value passes.
pub fn reject_batch(msg: &Value) -> Result<(), McpReError> {
    if msg.is_array() {
        return Err(McpReError::BatchForbidden);
    }
    Ok(())
}
...
/// message passing through MCP-RE verification is security-relevant by definition
/// (MCP_RE_SPEC §7: "operations with security consequences MUST be id-bearing
/// requests"), so an absent `id` -> [`McpReError::NotificationForbidden`].
///
...
pub fn reject_notification(msg: &Value) -> Result<(), McpReError> {
    match msg.get("id") {
        Some(Value::Null) | None => Err(McpReError::NotificationForbidden),
        Some(_) => Ok(()),
    }
}
...
///    [`McpReError::UnknownEnvelopeField`]. A structurally absent `on_behalf_of`
///    maps to [`McpReError::OnBehalfOfMissing`] (P005) and a structurally absent
///    `authorization_hash` to [`McpReError::AuthorizationHashMissing`] (P007).
///    Any other deserialization failure (wrong type, any OTHER missing required
///    field) maps to [`McpReError::CanonicalizationFailed`] — a structural
///    rejection that fails closed without claiming a more specific verdict.
/// 3. Step 6: enforce `version == "draft-01"`; any other value ->
///    [`McpReError::UnsupportedVersion`]. Folding step 6 in here means extraction
///    always yields a version-checked envelope.
pub fn extract_request_envelope(msg: &Value) -> Result<RequestEnvelope, McpReError> {
    ...
    if envelope.version != VERSION_DRAFT_01 {
        return Err(McpReError::UnsupportedVersion);
    }
    Ok(envelope)
}
...
/// mis-routes to [`McpReError::CanonicalizationFailed`]. We instead presence-check
...
/// verdict.
fn classify_request_envelope_error(raw: &Value, original: McpReError) -> McpReError {
    ...
    if original != McpReError::CanonicalizationFailed {
        return original;
    }
...
/// (absent -> [`McpReError::MissingEnvelope`]) and deserializes into
/// [`ResponseEnvelope`]. The `deny_unknown_fields` struct surfaces an unknown
/// field (e.g. the removed `trust_label`) as [`McpReError::UnknownEnvelopeField`];
/// other deserialization failures map to [`McpReError::CanonicalizationFailed`].
/// The response envelope carries no `version` field, so there is no §6-style
/// version check here.
pub fn extract_response_envelope(msg: &Value) -> Result<ResponseEnvelope, McpReError> {
...
///
/// Any missing segment along the path yields [`McpReError::MissingEnvelope`].
fn locate_envelope<'a>(
    msg: &'a Value,
    outer: &str,
    meta_key: &str,
) -> Result<&'a Value, McpReError> {
    msg.get(outer)
...
///
/// Discrimination rule:
///   * `unknown field \`<name>\``  -> [`McpReError::UnknownEnvelopeField`]
///     (the `deny_unknown_fields` violation);
///   * every other failure (type mismatch, any missing required field) ->
///     [`McpReError::CanonicalizationFailed`] — a structural rejection that fails
///     closed without claiming a more specific verdict.
///
/// The dedicated absence tokens P005 ([`McpReError::OnBehalfOfMissing`]) and P007
/// ([`McpReError::AuthorizationHashMissing`]) are **NOT** classified here from
/// serde's message wording (M-01 / M-02). A structurally absent `on_behalf_of` or
/// `authorization_hash` lands on the generic [`McpReError::CanonicalizationFailed`]
...
fn deserialize_envelope<T>(raw: &Value) -> Result<T, McpReError>
where
...
///
/// Fail-closed mapping:
/// - missing/absent envelope → [`McpReError::MissingEnvelope`];
/// - `version != "draft-02"` (or absent) → [`McpReError::UnsupportedVersion`];
/// - `canonicalization_id` absent → [`McpReError::CanonicalizationIdMissing`];
/// - unrecognized scheme → [`McpReError::CanonicalizationIdUnknown`];
/// - recognized but not allowlisted → [`McpReError::CanonicalizationIdNotAllowed`];
/// - unknown field → [`McpReError::UnknownEnvelopeField`];
/// - other structural failure → [`McpReError::CanonicalizationFailed`], with a
///   structurally-absent `authorization_hash`/`on_behalf_of` upgraded to its
///   dedicated token (same priority as draft-01).
pub fn extract_draft02_request_envelope(msg: &Value) -> Result<Draft02RequestEnvelope, McpReError> {
    let raw = locate_envelope(msg, "params", REQUEST_META_KEY)?;
...
/// absent `authorization_binding` surfaces [`McpReError::AuthorizationBindingMissing`]
...
fn classify_draft02_request_envelope_error(raw: &Value, original: McpReError) -> McpReError {
    if original != McpReError::CanonicalizationFailed {
        return original;
    }
    match raw.as_object() {
...
/// - absent → [`McpReError::AuthorizationBindingMissing`];
/// - `binding_type` not in the base set → [`McpReError::AuthorizationBindingTypeUnsupported`];
/// - opaque-bytes carrying authz-system-reference-only fields (both forms / a
///   oneof violation) → [`McpReError::AuthorizationBindingAmbiguousBytes`];
...
/// authorizes — that is the `mcp-re-policy` profile's job.
fn check_authorization_binding(raw: &Value) -> Result<(), McpReError> {
    let binding = raw
        .get("authorization_binding")
        .ok_or(McpReError::AuthorizationBindingMissing)?;
    let obj = binding
        .as_object()
        .ok_or(McpReError::AuthorizationBindingMalformed)?;
    ...
    let required: &[&str] = if binding_type == BINDING_TYPE_OPAQUE_BYTES {
        ...
        if REFERENCE_ONLY.iter().any(|k| obj.contains_key(*k)) {
            return Err(McpReError::AuthorizationBindingAmbiguousBytes);
        }
    ...
    if obj.len() != required.len() {
        return Err(McpReError::AuthorizationBindingMalformed);
    }
    for key in required {
        match obj.get(*key).and_then(Value::as_str) {
            Some(s) if !s.is_empty() => {}
            _ => return Err(McpReError::AuthorizationBindingMalformed),
        }
    }
...
/// - not an object, or `type` not `"mcp-mrt"` → [`McpReError::ContinuationTypeUnsupported`];
/// - wrong field set, an empty field, or a hash that is not a well-formed
///   `sha256:<base64url>` identifier → [`McpReError::ContinuationMalformed`].
///
...
fn check_continuation(raw: &Value) -> Result<(), McpReError> {
    ...
    let obj = continuation
        .as_object()
        .ok_or(McpReError::ContinuationTypeUnsupported)?;
    ...
    if obj.get("type").and_then(Value::as_str) != Some(CONTINUATION_TYPE_MCP_MRT) {
        return Err(McpReError::ContinuationTypeUnsupported);
    }
    ...
    if obj.len() != REQUIRED.len() {
        return Err(McpReError::ContinuationMalformed);
    }
    for key in REQUIRED {
        match obj.get(key).and_then(Value::as_str) {
            Some(s) if !s.is_empty() => {}
            _ => return Err(McpReError::ContinuationMalformed),
        }
    }
    ...
    for key in ["previous_request_hash", "input_required_response_hash"] {
        let value = obj
            .get(key)
            .and_then(Value::as_str)
            .ok_or(McpReError::ContinuationMalformed)?;
        parse_hash_id(value).map_err(|_| McpReError::ContinuationMalformed)?;
    }
    Ok(())
}
...
pub fn extract_draft02_response_envelope(
    msg: &Value,
) -> Result<Draft02ResponseEnvelope, McpReError> {
    let raw = locate_envelope(msg, "result", RESPONSE_META_KEY)?;
    require_draft02_version(raw)?;
    check_canonicalization_id(raw)?;
    deserialize_envelope(raw)
...
/// or any other value → [`McpReError::UnsupportedVersion`] (the profile cannot be
/// selected). Read from the raw JSON as an untrusted selector before the
/// signature verifies.
fn require_draft02_version(raw: &Value) -> Result<(), McpReError> {
    match raw.get("version").and_then(Value::as_str) {
        Some(VERSION_DRAFT_02) => Ok(()),
        _ => Err(McpReError::UnsupportedVersion),
    }
}
...
/// [`McpReError::CanonicalizationIdMissing`]; present value classified by
/// [`classify_canonicalization_id`].
fn check_canonicalization_id(raw: &Value) -> Result<(), McpReError> {
    match raw.get("canonicalization_id").and_then(Value::as_str) {
        ...
        None => Err(McpReError::CanonicalizationIdMissing),
    }
}
...
/// ([`McpReError::CanonicalizationIdNotAllowed`]); anything else names no scheme
/// at all ([`McpReError::CanonicalizationIdUnknown`]). The verifier NEVER selects
...
fn classify_canonicalization_id(
    ...
    if allowlist.contains(&id) {
        ...
        Err(McpReError::CanonicalizationIdNotAllowed)
    } else {
...
mod tests {
    ...
    fn request_message_with_envelope(envelope: Value) -> Value {
        json!({
            ...
            "method": "tools/call",
            "params": {
                "_meta": {"se.syncom/mcp-re.request": envelope},
    ...
    fn response_message_with_envelope(envelope: Value) -> Value {
        json!({
            ...
            "jsonrpc": "2.0",
            "result": {
                "_meta": {"se.syncom/mcp-re.response": envelope},
                "content": []
            }
        })
    }
    ...
    fn draft02_request_extracts_with_both_identifiers() {
        ...
        assert_eq!(env.canonicalization_id, "mcp-re-jcs-int53-json-v1");
    }
    ...
    fn draft02_request_missing_binding_is_binding_missing() {
        ...
        assert_eq!(
            extract_draft02_request_envelope(&msg),
            Err(McpReError::AuthorizationBindingMissing)
        );
    }
    ...
    fn draft02_request_unknown_binding_type_is_type_unsupported() {
        ...
        assert_eq!(
            extract_draft02_request_envelope(&msg),
            Err(McpReError::AuthorizationBindingTypeUnsupported)
        );
    }
    ...
    fn draft02_request_opaque_missing_digest_is_malformed() {
        ...
        assert_eq!(
            extract_draft02_request_envelope(&msg),
            Err(McpReError::AuthorizationBindingMalformed)
        );
    }
    ...
    fn draft02_request_both_binding_forms_is_ambiguous_bytes() {
        ...
        assert_eq!(
            extract_draft02_request_envelope(&msg),
            Err(McpReError::AuthorizationBindingAmbiguousBytes)
        );
    }
    ...
    fn classify_recognized_but_disallowed_is_not_allowed() {
        let allowlist = ["mcp-re-jcs-int53-json-v1"];
        let known = ["mcp-re-jcs-int53-json-v1", "mcp-re-jcs-future-floats-v2"];
        assert_eq!(
            classify_canonicalization_id("mcp-re-jcs-future-floats-v2", &allowlist, &known),
            Err(McpReError::CanonicalizationIdNotAllowed)
        );
        assert_eq!(
            classify_canonicalization_id("mcp-re-jcs-int53-json-v1", &allowlist, &known),
            Ok(())
        );
        assert_eq!(
    ...
    fn continuation_wrong_type_is_type_unsupported() {
        ...
        assert_eq!(
            extract_draft02_request_envelope(&msg),
            Err(McpReError::ContinuationTypeUnsupported)
        );
    }
    ...
    fn continuation_missing_hash_field_is_malformed() {
        ...
        assert_eq!(
            extract_draft02_request_envelope(&msg),
            Err(McpReError::ContinuationMalformed)
        );
    }
    ...
    fn missing_envelope_fixture_yields_missing_envelope() {
        assert_eq!(
            ...
            Err(McpReError::MissingEnvelope)
        );
    }
    ...
    fn wrong_type_field_is_not_unknown_envelope_field() {
        ...
        assert_eq!(result, Err(McpReError::CanonicalizationFailed));
        assert_ne!(result, Err(McpReError::UnknownEnvelopeField));
    }
    ...
    fn missing_on_behalf_of_yields_on_behalf_of_missing() {
        ...
        assert_eq!(
            extract_request_envelope(&msg),
            Err(McpReError::OnBehalfOfMissing)
        );
    }
    ...
    fn missing_authorization_hash_yields_authorization_hash_missing() {
        ...
        assert_eq!(
            extract_request_envelope(&msg),
            Err(McpReError::AuthorizationHashMissing)
        );
    }
