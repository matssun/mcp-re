# MCP-S Transport Hardening Guide

**Audience:** an operator or security reviewer who wants to understand and
configure the Phase-6 transport hardening of the `mcps-proxy` sidecar — mTLS,
...
production claim ceiling). The proofs are the `//mcps-proxy:*`
test targets in the [conformance manifest](../mcps-conformance/conformance_manifest.json).
...
| **Object signature** | Ed25519-over-JCS (`mcps-core`) | the **JSON-RPC signer** — who produced this exact object |
...
Source: [`tls.rs`](../mcps-proxy/src/tls.rs).
...
Source: [`transport.rs`](../mcps-proxy/src/transport.rs).
...
`mcps.transport_binding_failed`, emitted at the proxy (the only component holding
the connection).

## KeySource

Source: [`key_source.rs`](../mcps-proxy/src/key_source.rs).
...
Source: [`durable_replay.rs`](../mcps-proxy/src/durable_replay.rs).
...
- **`file`** (requires `--replay-path`) — `DurableReplayCache`: survives process
  ...
  `mcps.replay_cache_unavailable` (fail closed) and the in-memory insert is rolled
...
- It is **single-node**, not distributed. Two processes sharing one file is
  ...
  `mcps_core::ReplayCache` trait is a documented **future** backend.
...
Source: `ServerOptions::max_client_cert_lifetime` in [`tls.rs`](../mcps-proxy/src/tls.rs).

MCP-S Core defines **no online revocation** — no CRL, no OCSP, no transparency
...
whose validity cannot be parsed, with `mcps.transport_binding_failed`.
...
MCP-S's production claim is bounded to a **single node**. Explicitly deferred
