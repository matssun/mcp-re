...
single evidence spine ([ADR-MCPS-036](https://github.com/matssun/mcp-re/discussions/384)): every row references a
...
§A row so that a change to §A propagates here without a second edit ([ADR-MCPS-033](https://github.com/matssun/mcp-re/discussions/381) §5).
...
| Confused-deputy / delegated-authority abuse (`on_behalf_of`, `authorization_hash`) | **Partial** | §A *Delegation / authorization binding* (coverage **Partial** — Core **binds**, the AuthorizationProfile interprets; [ADR-MCPS-013](https://github.com/matssun/mcp-re/discussions/362)) |
| Tool poisoning / malicious or drifted tool descriptors (rug pull) | **Out of scope** | Non-goal: MCP-RE Core is method-transparent and does not interpret method semantics ([ADR-MCPS-030](https://github.com/matssun/mcp-re/discussions/378)). Guard: method-transparency pair (verdict independent of JSON-RPC method) + the static drift guard banning concrete MCP method-name literals in `mcp-re-core/src` |
| Unsafe / malicious tool *output* handling | **Out of scope** | Non-goal: §A *Tool safety* = **none, by design** ([ADR-MCPS-030](https://github.com/matssun/mcp-re/discussions/378)). Guard: same method-transparency pair — Core makes no claim about an invoked tool's output |
| Tool sandboxing / execution-environment isolation | **Out of scope** | Non-goal: tool-invocation policy and method-semantics enforcement live **outside** Core (MTCI / policy / host layer, each with its own ADR — [ADR-MCPS-030](https://github.com/matssun/mcp-re/discussions/378)). Guard: the method-transparency drift guard keeps method-aware logic out of `mcp-re-core/src` |

## Scope note

The three **Out of scope** rows are the adjacent tool-catalog / tool-execution domain that
is most often confused with MCP-RE scope. Per [ADR-MCPS-030](https://github.com/matssun/mcp-re/discussions/378): MCP-RE
