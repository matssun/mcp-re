...
# MCP-RE × EMA composition (proposed)
...
> *proposal*. This document records how MCP-RE would compose with it so the two
...
## Diagram A — basic MCP-RE sidecar path (this is what v0.5.1 demos)

No EMA. The host client signs each call; the proxy verifies and enforces before
the inner MCP server is ever reached.

```text
┌────────────────┐  signed MCP-RE request  ┌───────────────────────────┐   stdio   ┌──────────────────┐
│ HostSession    │ ─────────────────────► │ mcp-re-proxy (sidecar)      │ ────────► │ inner MCP server │
...
```text
┌────────────┐  signed req + bound      ┌──────────────────────┐         ┌──────────────────────────┐
│ HostSession│  EMA-artifact hash       │ mcp-re-proxy           │  stdio  │ EMA-native MCP server     │
...
The backend is **not** EMA-aware — it is a private server fully behind the MCP-RE
sidecar. Here the MCP-RE delegated-authorization layer enforces the
...
│ HostSession│  derived grant           │ mcp-re-proxy                 │ ────────► │ PRIVATE backend    │
...
- MCP-RE does **not** implement EMA and makes **no** EMA claim in v0.5.1.
