# MCP-RE Code Audit — Findings Report (v0.1)
...
**Subject:** MCP-RE Rust workspace (Zero-Trust security profile for the Model Context Protocol)  
...
|---|---|---|
| `mcp-re-core` | Pure verification (canonicalization, signing, hashing, replay, envelope). No I/O. | **Production-critical** |
| `mcp-re-host` | Transport-free request signing / response verification. Compile-time guard forbids networking. | **Production-critical** |
| `mcp-re-transport` | Client-side mTLS; verifies server certificate. | **Production-critical** |
| `mcp-re-proxy` | Server-side PEP/sidecar; subprocess inner; production CLI (`mcp_re_proxy_cli`). | **Production-critical** |
| `mcp-re-policy` | Phase-5 reference authorization profile. | **Production-critical** |
| `mcp-re-conformance` | Drift guards + conformance harness + traceability manifest guard. | Non-production (demo/conformance) |
| `mcp-re-demo` | Demonstration harness + e2e binaries. | Non-production (demo/conformance) |
| `mcp-re-demo-server` | Long-lived demonstration MCP server. | Non-production (demo/conformance) |
| `mcp-re-demo-fileserver` | One-shot demonstration MCP server. | Non-production (demo/conformance) |
...
|---|---|---|---|---|---|---|
| `mcp-re-core` | 0 | 2 | 2 | 3 | 6 | 13 |
| `mcp-re-host` | 0 | 0 | 0 | 3 | 4 | 7 |
| `mcp-re-transport` | 0 | 0 | 4 | 3 | 8 | 15 |
| `mcp-re-proxy` | 0 | 1 | 3 | 3 | 6 | 13 |
| `mcp-re-policy` | 0 | 0 | 1 | 7 | 4 | 12 |
| `mcp-re-conformance` | 0 | 0 | 3 | 7 | 4 | 14 |
| `mcp-re-demo` | 0 | 0 | 1 | 4 | 6 | 11 |
| `mcp-re-demo-server` | 0 | 0 | 0 | 4 | 7 | 11 |
| `mcp-re-demo-fileserver` | 0 | 0 | 0 | 2 | 8 | 10 |
...
- **Crate:** `mcp-re-core`  
- **Location:** `mcp-re-core/src/canonical.rs:298-389`  
- **Category:** boundary  
- **Lens:** general  
...
- **Crate:** `mcp-re-proxy`  
- **Location:** `mcp-re-proxy/src/persistent_inner.rs:286-324`  
...
### M-1 — Missing on_behalf_of maps to mcp-re.canonicalization_failed, not mcp-re.on_behalf_of_missing (P005)

- **Crate:** `mcp-re-core` · **Location:** `mcp-re-core/src/constraints.rs:140-154 + src/pipeline.rs:301-306` · **Category:** error-handling · **Lens:** conformance

P005 requires that an ABSENT on_behalf_of yields mcp-re.on_behalf_of_missing. But RequestEnvelope.on_behalf_of is a non-optional String, so a missing field fails serde deserialization in deserialize_envelope and (not matching the 'unknown field' prefix) maps to McpReError::CanonicalizationFailed. check_on_behalf_of in the pipeline only ever sees a present value, so it can only emit OnBehalfOfInvalidFormat (empty), never OnBehalfOfMissing. The McpReError::OnBehalfOfMissing variant is therefore DEAD in the pipeline: grep shows it is defined and its wire token unit-tested, but no code path produces it and no test exercises its production. The pipeline module doc (pipeline.rs:48-52) explicitly claims it is 'asserted reachable in the unit tests by feeding an envelope-shaped value that omits it through the field check directly' — that test does not exist. This is both a spec-conformance deviation (wrong error token for the absent case) and a false comment.
...
- **Crate:** `mcp-re-transport` · **Location:** `mcp-re-transport/src/lib.rs:260` · **Category:** DoS · **Lens:** security
...
- **Crate:** `mcp-re-transport` · **Location:** `mcp-re-transport/src/lib.rs:235` · **Category:** DoS · **Lens:** security
...
- **Crate:** `mcp-re-proxy` · **Location:** `mcp-re-proxy/src/cli.rs:609-615` · **Category:** resource-handling · **Lens:** general
...
- **Crate:** `mcp-re-proxy` · **Location:** `mcp-re-proxy/src/durable_replay.rs:124-142` · **Category:** replay · **Lens:** general
...
- **Crate:** `mcp-re-proxy` · **Location:** `mcp-re-proxy/src/persistent_inner.rs:303` · **Category:** fail-closed · **Lens:** security
...
- **Crate:** `mcp-re-conformance` · **Location:** `mcp-re-conformance/conformance_manifest.json:3` · **Category:** drift-guard / completeness · **Lens:** conformance

The conformance_manifest.json description states it is the 'Drift-guarded enumeration of ... every //... Bazel rust_test target' and 'The single source of truth for conformance counts'. The drift_guard_test BUILD comment repeats this ('every vector + every //... rust_test target'). But the manifest's bazel_test_targets array enumerates only 23 targets from 5 packages (mcp-re-conformance 7, mcp-re-core 2, mcp-re-host 2, mcp-re-policy 2, mcp-re-proxy 10). The mcp-re monorepo has 4 further packages with rust_test targets that are NEITHER listed NOR scanned: mcp-re-demo (12), mcp-re-demo-server (3), mcp-re-transport (2), mcp-re-demo-fileserver (1) = 18 unguarded targets. on_disk_test_targets() in drift_guard_test.rs only reads 5 BUILD env vars (CONFORMANCE/CORE/HOST/POLICY/PROXY), so the count assertion (23==23) passes vacuously for the omitted packages: a target could be added/removed/renamed in mcp-re-demo / mcp-re-transport / mcp-re-demo-server / mcp-re-demo-fileserver with the guard staying green. This directly violates P128 (manifest must be single source of truth for ALL target enumeration) and P194 (guard must be load-bearing) for those packages. Note the SECURITY traceability guard DOES scan all 8 of those packages (it has MCP_RE_BUILD_DEMO/DEMO_SERVER/TRANSPORT), so the omission is specific to the conformance drift guard and is an inconsistency, not a wiring impossibility.

> *Evidence:* conformance_manifest.json line 3: '...every //... Bazel rust_test target. The single source of truth for conformance counts.' / drift_guard_test.rs on_disk_test_targets() iterates only [("MCP_RE_BUILD_CONFORMANCE",..),("MCP_RE_BUILD_CORE",..),("MCP_RE_BUILD_HOST",..),("MCP_RE_BUILD_POLICY",..),("MCP_RE_BUILD_PROXY",..)] — no DEMO/DEMO_SERVER/TRANSPORT/FILESERVER. Shell confirms mcp-re-demo=12, mcp-re-demo-server=3, mcp-re-transport=2, mcp-re-demo-fileserver=1 nt_rust_test targets, none recorded.

### M-12 — Cross-transport parity test omits two of the six P182-mandated vectors (tampered_id and missing_envelope)

- **Crate:** `mcp-re-conformance` · **Location:** `mcp-re-conformance/tests/http_harness_test.rs:182` · **Category:** boundary / completeness · **Lens:** conformance
...
|---|---|---|---|---|
| 1 | `mcp-re-core` | Unbounded recursion in serde-Value canonicalization path (from_serde_value) | `mcp-re-core/src/canonical.rs:109-146` | boundary |
| 2 | `mcp-re-core` | Unknown-field discrimination relies on matching the serde_json error message string prefix | `mcp-re-core/src/constraints.rs:140-154` | error-handling |
| 3 | `mcp-re-core` | json_rpc_error_object silent fallback emits wrong code and drops the mcp-re taxonomy (P044/P109) | `mcp-re-core/src/wire.rs:36-38` | fail-closed |
| 4 | `mcp-re-host` | Unchecked i64 addition for expires_at; request_lifetime_secs unvalidated (negative/zero accepted) | `mcp-re-host/src/session.rs:109` | boundary |
| 5 | `mcp-re-host` | Raw HostSigner is publicly exported and directly usable in a client flow, partially weakening P137 | `mcp-re-host/src/lib.rs:19` | idiom |
| 6 | `mcp-re-host` | Host signs caller-supplied _meta keys other than .request, allowing the sidecar verified-context key to be smuggled into the signed preimage | `mcp-re-host/src/signer.rs:94` | canonicalization / verified-context |
| 7 | `mcp-re-transport` | round_trip ignores the HTTP status line and Content-Length: any response framing is accepted as a 'body' | `src/lib.rs:312-320` | correctness |
| 8 | `mcp-re-transport` | server_name_host silently maps unknown ServerName variants to the literal "localhost" Host header | `src/lib.rs:279-288` | idiom |
| 9 | `mcp-re-transport` | extract_body silently returns the entire buffer when no CRLFCRLF header terminator is found | `mcp-re-transport/src/lib.rs:312` | boundary |
| 10 | `mcp-re-proxy` | Inner responses whose result is a non-object (array/scalar) are forwarded UNSIGNED | `mcp-re-proxy/src/proxy.rs:328-332` | crypto |
| 11 | `mcp-re-proxy` | Public SubprocessInner::new / with_log_sink panic on an empty inner_command slice | `mcp-re-proxy/src/cli.rs:535-545` | boundary |
| 12 | `mcp-re-proxy` | Persistent inner with a null/absent caller id returns the first non-empty line, enabling response cross-talk on degenerate ids | `mcp-re-proxy/src/persistent_inner.rs:286-325` | correctness |
| 13 | `mcp-re-policy` | Issuer-signature preimage is built via serde_json (no JCS-safe domain / duplicate-key detection), diverging from the hash-binding canonicalization | `mcp-re-policy/src/reference.rs:131` | crypto |
| 14 | `mcp-re-policy` | Artifact's internal `profile` field is never validated against REFERENCE_PROFILE_ID | `mcp-re-policy/src/reference.rs:66` | boundary |
| 15 | `mcp-re-policy` | Argument-constraint scope matching does not reject extra (unconstrained) request arguments | `mcp-re-policy/src/reference.rs:252` | boundary |
| 16 | `mcp-re-policy` | RevocationSource fail-closed contract is documented but unenforced and unverified by the only concrete impl | `mcp-re-policy/src/revocation.rs:13-19,41-45` | fail-closed |
| 17 | `mcp-re-policy` | Phase-5 authorization vectors are not re-run over stdio / Streamable HTTP harnesses | `mcp-re-policy/tests/vectors_test.rs:59-98` | boundary |
| 18 | `mcp-re-policy` | Direct ReferenceProfile::authorize bypasses the raw-bytes JCS/duplicate-key gate; only the evaluator's hash-binding step enforces it | `mcp-re-policy/src/reference.rs:131-138` | canonicalization |
| 19 | `mcp-re-policy` | Artifact's internal `profile` field is not cross-checked against the selecting authorization-block profile | `mcp-re-policy/src/reference.rs:66-77` | authorization |
| 20 | `mcp-re-conformance` | HTTP harness leaks the server thread and bound socket when a POST fails mid-loop | `mcp-re-conformance/src/http_target.rs:65` | resource-handling |
| 21 | `mcp-re-conformance` | read_http_body silently truncates the body when the peer closes before Content-Length is satisfied | `mcp-re-conformance/src/http.rs:33` | error-handling |
| 22 | `mcp-re-conformance` | Drift-guard BUILD-file parsers are fragile hand-rolled scanners (sound for the current corpus) | `mcp-re-conformance/tests/drift_guard_test.rs:116` | maintainability |
| 23 | `mcp-re-conformance` | Guard non-empty sanity checks use weak lower bounds that do not catch partial parser/wiring regressions | `mcp-re-conformance/tests/drift_guard_test.rs:301` | anti-gaming |
| 24 | `mcp-re-conformance` | Hand-rolled nt_rust_test BUILD scanner can misparse if 'name' is not the rule's first attribute | `mcp-re-conformance/tests/drift_guard_test.rs:116` | drift-guard / idiom |
| 25 | `mcp-re-conformance` | outcome_token trusts response error.message verbatim with no signature verification (self-reported negative outcome) | `mcp-re-conformance/src/stdio_target.rs:128` | test-assurance |
| 26 | `mcp-re-conformance` | BUILD-file target-name parser keys on substring 'name', will mis-extract if crate_name precedes name | `mcp-re-conformance/tests/drift_guard_test.rs:116` | test-assurance |
| 27 | `mcp-re-demo` | Cert/material generation panics via expect() in library src, contradicting the crate's documented 'never panic' contract | `mcp-re-demo/src/demo_fixtures.rs:107-147` | error-handling |
| 28 | `mcp-re-demo` | demo_e2e (one-shot) bin captures no proxy stderr / inner oracle, unlike its persistent sibling — weaker evidence for the same boundary claims | `mcp-re-demo/src/bin/demo_e2e.rs:225-227` | boundary |
| 29 | `mcp-re-demo` | Persistent-demo evidence: server_cert_verified is aliased to response_hash_verified, not an independent cert/handshake fact | `src/e2e_persistent_flow.rs:627` | anti-gaming |
| 30 | `mcp-re-demo` | mtls_verified / proxy_process_started derived from authorized_calls>0, not from a transport-layer signal | `src/e2e_persistent_flow.rs:636-637` | anti-gaming |
| 31 | `mcp-re-demo-server` | `initialize` flips the lifecycle gate before any request validation, so a malformed initialize still unlocks the server | `src/server.rs:178-181` | correctness |
| 32 | `mcp-re-demo-server` | CRLF-terminated lines leave a trailing \r in the JSON-RPC payload | `src/stdio.rs:38-43` | boundary |
| 33 | `mcp-re-demo-server` | Received-log write/flush failures are silently swallowed | `src/server.rs:112-121` | error-handling |
| 34 | `mcp-re-demo-server` | No JSON-RPC notification handling — notifications (no id) get a response with id:null, and notifications/initialized is treated as method-not-found | `mcp-re-demo-server/src/server.rs:136-192` | idiom |
| 35 | `mcp-re-demo-fileserver` | JSON-RPC notifications receive a response, violating JSON-RPC 2.0 (notifications/initialized handshake) | `mcp-re-demo-fileserver/src/server.rs:49` | boundary |
| 36 | `mcp-re-demo-fileserver` | Comment claims file_type() is used to avoid following symlinks, but code calls entry.metadata() which follows symlinks | `mcp-re-demo-fileserver/src/server.rs:175-181` | idiom |

### 5.2 Informational (53)

| # | Crate | Title | Location | Category |
|---|---|---|---|---|
| 1 | `mcp-re-core` | wire::json_rpc_error_object has no tests and emits a hardcoded null-id fallback that drops the request id | `mcp-re-core/src/wire.rs:20-39` | error-handling |
| 2 | `mcp-re-core` | Parser materializes entire input as Vec<char> (4x memory amplification) | `mcp-re-core/src/canonical.rs:256-261` | idiom |
| 3 | `mcp-re-core` | canonicalize_json_value cannot detect duplicate object keys (P088/P026 boundary) | `mcp-re-core/src/canonical.rs:91-104 + src/signing.rs:73-80` | crypto |
| 4 | `mcp-re-core` | reject_notification documentation overstates safety for non-object inputs | `mcp-re-core/src/constraints.rs:68-73` | idiom |
| 5 | `mcp-re-core` | Missing signature.alg/key_id maps to canonicalization_failed at step 4, not invalid_signature at step 7 | `src/constraints.rs:90-97,140-154; src/envelope.rs:23-33; src/pipeline.rs:168-173` | error-handling |
| 6 | `mcp-re-core` | OnBehalfOfMissing error variant is unreachable through the pipeline (absent on_behalf_of yields canonicalization_failed) | `src/pipeline.rs:301-306; src/error.rs:62-63; src/envelope.rs:46-47` | idiom |
| 7 | `mcp-re-host` | request_hash recomputed via serialize->parse->canonicalize round-trip instead of reusing the just-built request Value | `mcp-re-host/src/session.rs:127` | idiom |
| 8 | `mcp-re-host` | id_key collapses all unserializable JSON-RPC ids to the empty-string key | `mcp-re-host/src/session.rs:227` | error-handling |
| 9 | `mcp-re-host` | HostSigner performs no well-formedness validation of on_behalf_of / authorization_hash before signing | `mcp-re-host/src/signer.rs:82-92` | boundary |
| 10 | `mcp-re-host` | SystemClock reports negative seconds for a pre-epoch clock rather than failing | `mcp-re-host/src/clock.rs:32-42` | fail-closed |
| 11 | `mcp-re-transport` | from_pem maps PrivateKeyDer parse error through BadClientMaterial but client-cert PEM parse error closure double-wraps redundantly | `src/lib.rs:112-120` | error-handling |
| 12 | `mcp-re-transport` | io_or_handshake error classification relies on undocumented rustls io::Error wrapping and may misclassify post-handshake errors | `src/lib.rs:299-308` | error-handling |
| 13 | `mcp-re-transport` | Host header falls back to literal "localhost" for non-DNS/non-IP server names (silent, cosmetic) | `mcp-re-transport/src/lib.rs:286` | silent_fallback |
| 14 | `mcp-re-transport` | Client permits TLS 1.2 (safe defaults), not pinned to TLS 1.3 | `mcp-re-transport/src/lib.rs:178-179` | crypto |
| 15 | `mcp-re-transport` | UnexpectedEof tolerated when reading response (matches proxy framing) — verified not exploitable | `mcp-re-transport/src/lib.rs:260-265` | boundary |
| 16 | `mcp-re-transport` | Server-cert verification is genuinely real in the default build (no accept-any leakage) | `mcp-re-transport/src/lib.rs:162-182` | fail-closed |
| 17 | `mcp-re-transport` | Server-certificate verification is genuinely enforced with a real WebPkiServerVerifier (positive finding) | `mcp-re-transport/src/lib.rs:148` | crypto |
| 18 | `mcp-re-transport` | Fault-injection accept-any verifier is correctly gated and absent from the default build (positive finding) | `mcp-re-transport/src/lib.rs:328` | fail-closed |
| 19 | `mcp-re-proxy` | Threaded serve() is exported but unused and untested; incompatible with the RefCell-based Proxy | `mcp-re-proxy/src/tls.rs:288-313` | idiom |
| 20 | `mcp-re-proxy` | now_unix() silently coerces a pre-epoch clock to 0 | `mcp-re-proxy/src/main.rs:33-38` | error-handling |
| 21 | `mcp-re-proxy` | Over-long client certificate rejection is surfaced as mcp-re.transport_binding_failed (not a dedicated revocation/expiry token) | `mcp-re-proxy/src/tls.rs:223-242` | error-handling |
| 22 | `mcp-re-proxy` | build_signed_response passes inner error/non-object results through unsigned | `mcp-re-proxy/src/proxy.rs:328-332` | fail-closed |
| 23 | `mcp-re-proxy` | Synthesized-_meta branch in build_forwarded_request relies on serde_json IndexMut semantics that would panic on a non-object params | `mcp-re-proxy/src/proxy.rs:305-309` | boundary |
| 24 | `mcp-re-proxy` | EnvKeySource is gated behind an explicit dev/CI opt-in with a startup warning | `mcp-re-proxy/src/cli.rs:350-356; src/main.rs:69-78` | secret-handling |
| 25 | `mcp-re-policy` | Scope matching keys tool/resource solely on params.name; non-name-keyed methods collapse to empty-string tool | `mcp-re-policy/src/reference.rs:218` | correctness |
| 26 | `mcp-re-policy` | Unreachable error fallback in json_rpc_authorization_error emits the wrong JSON-RPC code (-32603 vs MCP-RE -32003) | `mcp-re-policy/src/wire.rs:33` | error-handling |
| 27 | `mcp-re-policy` | Signature verification uses the serde canonicalization path (no duplicate-key detection); safety depends entirely on the prior hash-binding gate | `mcp-re-policy/src/reference.rs:131-187` | crypto |
| 28 | `mcp-re-policy` | check_scope reads method/tool via unwrap_or(""), so a malformed request degrades silently to scope-denied rather than malformed | `mcp-re-policy/src/reference.rs:218-235` | error-handling |
| 29 | `mcp-re-conformance` | outcome_token trusts the unsigned JSON-RPC error.message field as the verdict token | `mcp-re-conformance/src/stdio_target.rs:128` | crypto |
| 30 | `mcp-re-conformance` | plain_echo_inner maps malformed/non-JSON input to Value::Null rather than surfacing an error | `mcp-re-conformance/src/fixtures.rs:81` | error-handling |
| 31 | `mcp-re-conformance` | Hardcoded duplicate of preimage-stable _meta identifier in now_unix_for_case | `mcp-re-conformance/src/target.rs:203` | idiom |
| 32 | `mcp-re-conformance` | Unbounded Content-Length read in conformance HTTP harness (test-scope DoS) | `mcp-re-conformance/src/http.rs:33` | dos |
| 33 | `mcp-re-demo` | Dead computation: request_hash result discarded in negative case 9 | `mcp-re-demo/src/bin/demo_negative.rs:445` | idiom |
| 34 | `mcp-re-demo` | mtls_client bin failure path prints a structured line then returns Err — duplicate/garbled reporting on stdout+stderr | `mcp-re-demo/src/bin/demo_mtls_client.rs:166-172` | error-handling |
| 35 | `mcp-re-demo` | now_unix() silently floors to 0 on a pre-epoch clock (silent fallback in grant/freshness window sizing) | `src/bin/demo_e2e.rs:65-70` | fail-closed |
| 36 | `mcp-re-demo` | demo_e2e.rs (one-shot positive bin) does not capture proxy stderr, so it asserts no inner-reach / machine evidence | `src/bin/demo_e2e.rs:225-227` | idiom |
| 37 | `mcp-re-demo` | Self-issued grant in the multi-process flow vs separate ISSUER in the in-process demos — intentional but undocumented divergence | `src/e2e_flow.rs:136-154` | idiom |
| 38 | `mcp-re-demo` | demo_e2e proxy is spawned with stdout=null, discarding the inner-launch/lifecycle diagnostic channel (positive bin only) | `src/bin/demo_e2e.rs:225-227` | fail-closed |
| 39 | `mcp-re-demo-server` | Field doc says "append-only" but the log is truncated on attach — minor doc/behavior wording mismatch | `src/server.rs:70-77` | idiom |
| 40 | `mcp-re-demo-server` | Request is JSON-parsed twice per line (handle_should_stop then handle) — minor redundancy | `src/stdio.rs:44-45` | idiom |
| 41 | `mcp-re-demo-server` | `echo` with bad arguments is recorded as a served call even though it returns isError:true | `src/server.rs:260-289` | correctness |
| 42 | `mcp-re-demo-server` | Received-log write failures are silently dropped, weakening the anti-gaming oracle under I/O pressure | `mcp-re-demo-server/src/server.rs:112-121` | fail-closed |
| 43 | `mcp-re-demo-server` | Each inbound line is JSON-parsed twice (handle_should_stop then handle) | `mcp-re-demo-server/src/stdio.rs:44-45` | idiom |
| 44 | `mcp-re-demo-server` | Received-log write/flush errors are silently swallowed (acceptable for instrumentation, but the anti-gaming signal could under-record on disk-full) | `src/server.rs:112-121` | fail-closed |
| 45 | `mcp-re-demo-server` | shutdown is processed before initialize and from any lifecycle state (intentional, low impact on a demo inner) | `src/server.rs:182-191` | boundary |
| 46 | `mcp-re-demo-fileserver` | Symlink-confinement check is skipped when the joined path does not canonicalize (defense-in-depth gap) | `mcp-re-demo-fileserver/src/server.rs:228` | crypto |
| 47 | `mcp-re-demo-fileserver` | canonicalize failure of the demo root is reported as ReadDir(".") regardless of the requested path | `mcp-re-demo-fileserver/src/server.rs:227` | error-handling |
| 48 | `mcp-re-demo-fileserver` | initialize silently ignores the client's requested protocolVersion | `mcp-re-demo-fileserver/src/server.rs:100` | idiom |
| 49 | `mcp-re-demo-fileserver` | main: unknown-argument handling rejects but argv parsing stops scanning value-position tokens correctly only for the single known flag | `mcp-re-demo-fileserver/src/main.rs:21` | idiom |
| 50 | `mcp-re-demo-fileserver` | Symlink-escape refusal is not exhaustive: a symlink whose target does not (yet) exist bypasses the canonical containment check | `mcp-re-demo-fileserver/src/server.rs:224-235` | boundary |
| 51 | `mcp-re-demo-fileserver` | TOCTOU between canonicalize() containment check and read_dir() | `mcp-re-demo-fileserver/src/server.rs:228-167` | boundary |
| 52 | `mcp-re-demo-fileserver` | No DoS bound on directory entry count / response size (unbounded read) | `mcp-re-demo-fileserver/src/server.rs:170-192` | dos |
| 53 | `mcp-re-demo-fileserver` | stdio line reader has no per-line length bound (unbounded input) | `mcp-re-demo-fileserver/src/stdio.rs:25-30` | dos |
...
|---|---|---|---|---|
| P005 | conformance | high | on_behalf_of is REQUIRED-present in Core | mcp-re-core:partial |
| P007 | conformance | high | authorization_hash present and sha256-prefixed format | mcp-re-core:partial |
| P025 | conformance | high | JSON-RPC id typing — string preferred, integer only if safe | mcp-re-core:implemented; mcp-re-host:partial; mcp-re-proxy:not_applicable |
| P031 | security | high | Freshness window with symmetric clock skew | mcp-re-host:partial |
| P043 | conformance | high | Frozen error taxonomy tokens | mcp-re-core:partial; mcp-re-policy:not_applicable; mcp-re-demo:implemented |
| P044 | conformance | medium | JSON-RPC error object shape on the wire | mcp-re-core:partial; mcp-re-policy:implemented |
| P052 | conformance | medium | Vectors re-run transport-agnostically over stdio and Streamable HTTP | mcp-re-core:not_applicable; mcp-re-policy:partial; mcp-re-conformance:partial |
| P053 | conformance | high | Vector family coverage minimum | mcp-re-core:implemented; mcp-re-policy:partial; mcp-re-conformance:not_applicable |
| P074 | security | high | v1 revocation posture: enforce maximum client-cert lifetime; no online CRL/OCSP claim | mcp-re-proxy:implemented; mcp-re-demo:partial |
| P086 | conformance | critical | Transport-agnostic security envelope (stdio and Streamable HTTP) | mcp-re-host:implemented; mcp-re-transport:partial; mcp-re-proxy:implemented; mcp-re-demo:partial |
| P109 | conformance | medium | Standardized JSON-RPC error objects with mcp-re error taxonomy | mcp-re-core:partial; mcp-re-policy:implemented; mcp-re-demo-fileserver:not_applicable |
| P120 | security | high | KeySource favors sign-over-export; no public key-export API | mcp-re-host:partial |
| P124 | security | critical | Transport binding required: reject signer/peer mismatch and plaintext downgrade | mcp-re-proxy:partial |
| P126 | security | critical | Durable replay cache atomic check-and-insert; shared cache is true atomic primitive | mcp-re-proxy:partial |
| P128 | conformance | medium | Drift-guarded conformance manifest is single source of truth for counts/targets | mcp-re-policy:implemented; mcp-re-conformance:partial |
| P137 | conformance | high | HostSession signs every request (no raw HostSigner in client flow) | mcp-re-host:partial; mcp-re-proxy:not_applicable; mcp-re-conformance:not_applicable; mcp-re-demo:implemented |
| P155 | security | high | Authorization grant binds full tuple | mcp-re-policy:partial |
| P179 | security | high | Persistent inner fails closed on crash / malformed line / handshake failure | mcp-re-demo-server:partial; mcp-re-demo-fileserver:partial |
| P182 | conformance | high | Cross-transport / object parity of core conformance vectors | mcp-re-policy:partial; mcp-re-conformance:partial; mcp-re-demo:implemented |
| P187 | security | high | Anti-gaming: assert externally observable facts, never a printed OK | mcp-re-conformance:partial; mcp-re-demo:implemented; mcp-re-demo-server:implemented |
| P194 | conformance | high | Anti-gaming: guards are load-bearing (go red when control breaks) | mcp-re-transport:implemented; mcp-re-conformance:partial |
...
| P111 | general | high | mcp-re-core has zero networking/async/filesystem/MCP-server dependencies |
...
| P145 | security | critical | Proxy verifies the MCP-RE Core envelope before any downstream action |
...
| P151 | security | high | Missing MCP-RE request envelope rejected |
...
| P183 | general | high | ADR-MCPS-001/012 firewall: no dependency outside components/mcp-re |
| P184 | general | medium | mcp-re-host is transport-free (no networking/async/socket) |
| P185 | general | medium | mcp-re-transport carries bytes only (does no signing) |
...
|---|---|---|---|
| `mcp-re-core` | low | Signed preimage is canonicalized via serde_json::Value path, not the audited raw-bytes JCS canonicalizer; relies on cross-path equivalence | `src/signing.rs:73-80; src/canonical.rs:101-146` |
| `mcp-re-core` | info | verify_request re-canonicalizes the object three times per request (step3 raw + step11 preimage + final request_hash) | `src/pipeline.rs:165,194,216` |
| `mcp-re-host` | low | SystemClock pre-epoch fallback can emit a malformed RFC3339 issued_at/expires_at (negative year) rather than failing at the host | `mcp-re-host/src/clock.rs:39` |
| `mcp-re-transport` | info | server_name_host falls back to "localhost" for non-DNS/non-IP ServerName variants | `mcp-re-transport/src/lib.rs:286` |
| `mcp-re-transport` | info | Transport binding (mTLS peer == object signer) is NOT performed in this crate — confirm it is enforced at the proxy | `mcp-re-transport/src/lib.rs:230` |
| `mcp-re-proxy` | low | Inner-produced error responses are forwarded UNSIGNED (P093 deviation) | `mcp-re-proxy/src/proxy.rs:328` |
| `mcp-re-proxy` | low | Hostile inner can inject a forged .verified / response envelope inside result._meta that is relayed to the host | `mcp-re-proxy/src/proxy.rs:334` |
| `mcp-re-proxy` | info | Verified-context injected to inner omits an explicit policy-decision field | `mcp-re-proxy/src/proxy.rs:285` |
| `mcp-re-policy` | low | Argument scope is a subset-equality constraint; unconstrained request arguments are always permitted | `mcp-re-policy/src/reference.rs:240-255` |
| `mcp-re-demo` | low | DemoFixtures public API exposes raw 32-byte private signing-key seeds (signer_seed/server_seed) | `src/demo_fixtures.rs:264,270` |
| `mcp-re-demo` | low | demo_mtls_client deterministic mode uses a FIXED clock + hardcoded nonce seed, reachable via a runtime --deterministic flag | `src/bin/demo_mtls_client.rs:120-126,177-179,218` |
| `mcp-re-demo` | info | Inner-launch hardening relies entirely on mcp-re-proxy InnerLaunchConfig; demo only sets coarse rlimits and is honest about non-containment | `src/demo_proxy.rs:65-84` |
| `mcp-re-demo` | info | BUILD.bazel deps reference only in-workspace mcp-re-* crates and audited @crates_mcp_re — firewall intact | `BUILD.bazel:25-34,124-131,358-361` |
| `mcp-re-demo-server` | info | tools/call records receipt BEFORE executing, but a malformed-argument echo still records — record reflects dispatch, not successful execution | `src/server.rs:260-277` |
| `mcp-re-demo-fileserver` | low | Non-existent canonicalization target bypasses canonical containment (lexical-only fallback) | `mcp-re-demo-fileserver/src/server.rs:228-235` |
| `mcp-re-demo-fileserver` | info | read_dir follows symlinks for entry classification despite comment claiming otherwise | `mcp-re-demo-fileserver/src/server.rs:176-181` |

---

## 9. Per-Crate Assessment

### `mcp-re-core` — production-critical
...
### `mcp-re-transport` — production-critical
...
### `mcp-re-proxy` — production-critical
...
### `mcp-re-policy` — production-critical
...
### `mcp-re-demo-fileserver` — non-production
...
- **L-9.** The non-production demo crates (mcp-re-demo, mcp-re-demo-server, mcp-re-demo-fileserver) were reviewed only for evidence honesty and spec fidelity, not as hardened production surfaces, consistent with their stated non-production status
...
unwrap()/expect()/panic! markers in src              : 11
  by crate: mcp-re-core 7, mcp-re-proxy 2, mcp-re-demo-server 1, mcp-re-demo-fileserver 1
Risk-keyword feature gates                           : 1
  mcp-re-transport `fault_accept_any_server` (OFF by default — intentional fault-injection control)
```
