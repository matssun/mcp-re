...
| 10 | **Response signed by the proxy/server side** | On the happy path (#7), inspect the response bytes. | The response carries the `name.sundvall/mcps-security.response` block; `server_signer` == `--server-signer`; signature verifies against the server key in the resolver. |
