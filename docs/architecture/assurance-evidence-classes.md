...
| | *1-E landed — the seal census.* §2 named ONE theorem whose statement asserts a structural fact the registry records as `test://`, and called it this record's thesis. Measured across the estate: **seven theorems and six units**, five of them `critical`. The FIRST run of the census under-counted and the reason is the finding's own shape: the registries hard-wrap prose at 90 columns, so `apply is the only\nmutator` does not match `the only mutator` — a regular expression over wrapped prose measures the wrapping. Normalised, it found THM-0012 (*apply is the only mutator*, the whole argument for its conclusion) and a SECOND seal in `proxy.delegated_resolver_materialization`, which already carries a structural sibling for a different one. THM-0108 says *POSSESSION IS THE PROOF, three times*, in capitals, about three types — so three probes, not one. THM-0029/0031/0033 are one family making the CO-PROVENANCE argument 0D-9 met at `proxy.trust_plan`, and THM-0031 states what a behavioural control cannot reach: *that both predecessors ultimately arose from A connection establishes nothing; the defect this excludes is two honest products of two DIFFERENT connections.* The census's own control: the eleven 0D splits are correctly excluded, because each now carries a structural sibling. Explicitly NOT a worklist — bulk promotion is the converse of the error Ruling 3 forbids. Packet: `verification/reviews/packets/seals-stated-as-tested-2026-09-18.md` | |
...
| | *1-G landed — falsifier coverage, estate-wide.* The `verifier_results` audit found two theorems no probe attacks while N1 reads them satisfied; this asks how general that is. **130 live theorems, 51 named by a probe, 79 by none** — and **22 are root-reachable with EVERY supporting unit covered and NONE owing**, so N1 is satisfied, `review` prints no obligation, the debt registry holds no row, and nothing names the claim. **11 are critical, and TWO are declared ROOTS: THM-0074 and THM-0091.** The first draft said 25 and 14 on a looser rule — *some* unit probed rather than *every* unit covered and none owing — and the strict rule matters: a theorem one of whose units honestly OWES has its gap recorded where gaps belong, and counting it would inflate the population by what the ratchet is already holding. Under the strict rule the 'covered but owing' bucket is EMPTY, so the 22 is the whole hidden population rather than a sample. THM-0091 stays in the list even after its own decomposition lands, which is the sharpest argument for requiring the link: its new falsifiers are `structural://` probes and **that registry has no `theorem` key at all**. The honest caveat is in the packet: `[[probe]].theorem` is OPTIONAL (133 of 180 name one; the structural registry has no such key), so the number counts labelling and coverage together — and the point is that the graph cannot tell them apart. N1's subject is a proposition and its object is a `[[unit]]`; when one unit supports ten claims it has one answer for ten questions. Two remedies that compose: require the field and derive per-theorem coverage (cheap, makes it visible), then split the units (expensive, fixes it). Packet: `verification/reviews/packets/theorem-falsifier-coverage-2026-09-18.md` | |
| | *1-I landed — the five roots whose decomposition already exists.* THM-0077, THM-0078, THM-0072, THM-0042 and THM-0071 each have a real decomposition, so the question was whether the LEAVES are honest. **No incorrect root statement, no missing `depends_on`, no candidate root** — and one shape, five times: `tls_listener_state` names 3 propositions in its own description, `exchange_lifecycle` 3, `scitt_receipt_offline` 6, `audit_record_coordinates` 3, and with `verifier_results`'s 10 that is five instances of ADR-MCPRE-061 question 2 — *an answer to question 1 that needs an "and"*. None is wrong about what it establishes; each is wrong about how many things that is. Also: THM-0078's severity is INVERTED against its claim (the `medium` leaf its second clause rests on inherits `high`, which is N3 working), THM-0072's whole novelty lives in a 3-control owing unit, and THM-0042's corpus unit is `measured`-shaped inside a `tested` one. Recorded because it is right: THM-0042's scope demotes the `s01` interop vector IN PLACE rather than regenerating it, because *that no MCP-RE code produced it is the whole value of that vector*. Packet: `verification/reviews/packets/remaining-roots-audit-2026-09-18.md` | |
| | *Phase 1 REPORT: `verification/reviews/packets/phase1-report-2026-09-18.md`.* Every declared root examined consequence-first. **Four decomposed** (THM-0094, THM-0095, THM-0091, THM-0012), **eight audited**. The completion criterion is NOT met and the obstacle is measured and identical in every remaining case: **five units each answer for several claims at once** (`verifier_results` 10, `scitt_receipt_offline` 6, `tls_listener_state` 3, `exchange_lifecycle` 3, `audit_record_coordinates` 3), and no root above them can have one owner per leaf until they are split. Six estate-wide findings, 24 numbered Phase-2 obligations, 189 controls newly registered under ADR-MCPRE-069 across three projects, one production correction, one candidate root stated and not added, and **18 claim corrections batched for owner specification review** — the only thing this campaign could not do for itself, because `claim_surface_gate.py` refuses a published claim whose statement has moved since the owner read it. **Two of the six censuses were corrected in flight, both by re-deriving the number rather than re-reading the prose** | |
| | *1-B landed — **THM-0095**, the TypeScript SDK root.* Eleven leaves in the closure and three outside it — one leaf MORE than the Python member, because `post_close_emission` is reachable here and not there: `Promise.race` does not stop the losing arm, so without a per-leg guard a continuation chain goes on signing and POSTing fresh answer legs after `onclose` fired. Python's task group really cancels. That is what the root family is for — two implementations of one promise whose propositions are not the same set. The census is far worse than Python's: **167 controls, 33 declared**, and the gap concentrated where the claim is strongest — the whole `transport_replay.test.ts` (the RECORDED session refused six ways) claimed by nothing, all 31 `authorization.test.ts` controls including both seal controls, all 22 of `custody.test.ts`. The anchor claim said *no member is defaulted* and named **four of seven**; all seven are now its battery. **139 controls newly registered**, 213 passing across four pinned Node majors. The parity oracle (29 here, 11 in Python) is an identified, deliberately UNRATIFIED `new-proposition`. Packet: `verification/reviews/packets/thm-0095-consequence-decomposition-2026-09-18.md` | |
| | *1-C landed — **THM-0091**, the sidecar ingress root, and the one §2 called the sharpest illustration of this record's thesis.* Seven leaves, **two of them STRUCTURAL** — the theorem's own statement asserted *possession of the scope IS that permission* and *the authority set is derived from that scope and never from the flag*, and nothing established either; S22 and S23 are the compile refusals those sentences always needed, both `refused by E0451`. **A mutation probe then found the seal was not load-bearing at the seam that opens the socket:** M86 deletes `BindScope::decide`'s refusal, and `serve::tests::bind_refuses_a_non_loopback_address_unless_it_is_declared` — whose doc comment says *"the guard must be what refuses, at the seam that opens the socket"* — stayed GREEN, because `serve::bind` re-implemented the condition, copied the message verbatim and never obtained a scope. R-COMPOSE, measured. Corrected in nine lines; M86 now turns both controls red. Also: `serve/guards.rs` held two predicates the root's statement names and belonged to a unit whose subject is an ADMITTED request, and `config/local.rs` was in the paths with no control declared. Seven falsifiers over nine probes, 63 obligations unchanged — this root's leaves discharge N1 rather than inheriting debt. Packet: `verification/reviews/packets/thm-0091-consequence-decomposition-2026-09-18.md` | |
| | *1-H landed — **THM-0012**, the fourth undecomposed root, and the one §2's measurement could not see.* Its table found the three roots whose whole closure reaches NO falsifier; THM-0012's sole unit has one, so it counted as covered while being exactly as undecomposed as THM-0091 — undecomposed and unfalsified are different properties and §2 recorded one of them. Its statement's second clause, *apply is the only mutator*, is the bridge from the relation to the conclusion (a relation with a unique predecessor says nothing about a RECORDED state if something else could write it) and nothing established it. Two leaves, **two probes for two producer routes**: S24 constructs a lifecycle at `Stopped` (`E0451`), S25 assigns the field on one already held (`E0616`) — the second is what the statement names, and a holder that could assign would make `apply`'s refusal a formality. **NO claim correction needed**, the only root in this group where the statement was already right. Its scope already answers questions 9 and 11 correctly — `admits_requests` classified as a descriptive witness *no production path consumes*, and `FailedToStart` recorded as unreachable with its consequence. Packet: `verification/reviews/packets/thm-0012-consequence-decomposition-2026-09-18.md` | |
| | ***PHASE 1 CLOSED*** at `06994c644a594ac3e48d48208a9e7424a40023d6`. All twelve roots decomposed consequence-first; 192 propositions, each with one carrier and one honest evidence class; 43 live premises all typed; **0** implicit propositions between leaf and root, **0** stale unit references, **0** units storing a derived severity, and **0** campaign branches holding an artefact main has never held. Six correction records carry 16 corrections, 3 of them dependency corrections. N1 moved **63 -> 87** against 138 -> 192 propositions, which is the criterion working: decomposition made hidden conjunctions countable, and the two decreases (M172, M176) are the only real discharges. The sharpest result is one a fingerprint cannot express — **THM-0074's claim digest did not change but its assurance meaning did**: `theorem_claim` is `sha256:9f4cc66a` on both sides while the dependency closure went 48 -> 66, because the premise its antecedent had always quantified over was absent from the graph. Report: `verification/reviews/packets/phase1-closure-2026-09-18.md` | |
...
| | ***PHASE 2 CLOSED***. **N1: 87 -> 0.** 192 units, 304 probes, **170** units claiming `mutation://` evidence and **170** attacked — `0` claiming-but-not-attacked and `0` attacked-but-not-claiming, so no probe suite runs outside the attestation closure. All **169** `tested` units sit at effective Medium or above, so the rule's population and the class's are the same set. The migration ratchet holds **0** rows and stays, because with none left its rule says exactly one thing: every `tested` proposition at Medium or above names a falsifier, or the gate fails. Both SDK lanes were built from nothing (19 Python + 19 TypeScript probes) before the cargo lane's 60 obligations were touched — the order the Phase-1 closure derived, because 27 obligations and two whole system roots were blocked on building each one once. **One compound probe**, `M282`: three sites each enforce `post_close_emission` completely, so a one-anchor weakening left every control green and the lane reported the conjunct as unprotected, which was FALSE. Every other multi-site case here is composition, not defence in depth. The phase's second product is **twelve controls written or repaired** because a falsifier proved the battery could not see the check it was about — a control asserting only the accepting direction, one whose prose named the adversarial case and whose assertions did not, one testing a delimiter production does not use, a conjunct declared and never tested, and `toEqual` unable to see an emitted-but-undefined member. Report: `verification/reviews/packets/phase2-closure-2026-09-19.md` | |
