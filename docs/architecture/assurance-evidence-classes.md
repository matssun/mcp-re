...
| | *1-B landed — **THM-0095**, the TypeScript SDK root.* Eleven leaves in the closure and three outside it — one leaf MORE than the Python member, because `post_close_emission` is reachable here and not there: `Promise.race` does not stop the losing arm, so without a per-leg guard a continuation chain goes on signing and POSTing fresh answer legs after `onclose` fired. Python's task group really cancels. That is what the root family is for — two implementations of one promise whose propositions are not the same set. The census is far worse than Python's: **167 controls, 33 declared**, and the gap is concentrated where the claim is strongest — the whole `transport_replay.test.ts` (the RECORDED session refused six ways, the substance of the Python member's evidence) claimed by nothing, all 31 `authorization.test.ts` controls including both seal controls, all 22 of `custody.test.ts`. The anchor claim said *no member is defaulted* and named **four of seven**; all seven are now its battery. **139 controls newly registered**, 213 passing across four pinned Node majors. The parity oracle (29 controls here, 11 in Python) is an identified, deliberately UNRATIFIED `new-proposition`: it must not become a root, because cross-SDK emission parity is an internal consistency contract and not a boundary promise. Packet: `verification/reviews/packets/thm-0095-consequence-decomposition-2026-09-18.md` | |
