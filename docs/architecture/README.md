...
- [**ADR-MCPRE-068**](assurance-evidence-classes.md) — evidence classes: what kind of thing supports a security claim. 📄 Proposed 2026-09-17, revision 2. Assurance TCB, not a product claim: the registry can say *theorem → unit* but not *proved / structural / tested / measured*, nor why a chain terminates (*assumed / external-boundary / review-obligation*), so no rule can require the evidence kind to be ADEQUATE to what the claim promises. Measured: 21 of 21 probes over the sealed-owner challenge set attack runtime predicates and none falsifies a seal, while `http_profile.verifier_result_separation` already produces compile-refusal evidence recorded as `test://`. Adds one sort to ADR-MCPRE-059; supersedes nothing.
- [**ADR-MCPRE-069**](evidence-that-runs-but-nothing-claims.md) — evidence that runs but nothing claims. 📄 Proposed 2026-09-17. Subordinate to ADR-MCPRE-068 and the mirror of its defect: 626 of 2168 test functions inside declared unit paths are in no unit's battery, so they run, pass, and are evidence for nothing. Four per-symbol dispositions; the fourth writes theorems, which is why it is a separate record.
