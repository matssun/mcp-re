...
**1. ~~Extract E — the outbound-fetch network policy (336 lines).~~ DONE (MCPRE-161).** `mcp-re-proxy/src/outbound_fetch/` — four modules, and `ocsp.rs` fell 1271 → 980. **392 lines of POLICY compiled unconditionally** — the 336 this census measured, plus the destination type — with only the ~100-line `ureq` binding (`resolver.rs` and `VettedDestination::agent`) behind a feature, because ADR-MCPS-018 keeps the default closure lean and the Bazel base flavor links no HTTP client at all. That is a real constraint on the BINDING, not the accidental coupling of the guard this census objected to. The provenance the guard turns on is now carried by the TYPE: `VettedDestination::certificate_derived` and `::operator_configured` are two constructors, and `VettedDestination::agent` hands out the configured client rather than a boolean, so a caller can neither assert a certificate's URL is operator-configured nor turn the connect-time vetting off for one it built as certificate-derived. It also removed a test-only kill switch from `OcspChecker` rather than relocating it. Original rationale: it is not OCSP. It is the control any server-side outbound fetch in this proxy needs, and it is currently unreachable in the default build because it lives behind a feature gate that has nothing to do with it. Moving it to its own module (compiled unconditionally) makes it available to the async OCSP this implementation is retained for, to SCITT registration, and to anything else — and it removes 26% of this file without touching the protocol.

**2. ~~Give the §3.2 chain a success product (Q11).~~ DONE (MCPRE-161).** `verify_and_map_response` returns a `TrustedRevocationAnswer` with a private field, no constructor taking a status, and itself as the sole producer — reached only after all five checks. `OcspChecker::allows` takes `RevocationEvidence`, which is `Answered(_)` or `NotEstablished(NoResponderConfigured | DestinationRefused)`: a responder that says *"I do not know"* was reached and verified, and a fetch that never happened was not, so the two stopped being the same value. Original rationale: `verify_and_map_response` should return a value whose existence means the five checks passed — not a bare `Copy` enum anyone can name. This is the same finding as EX-004's `EvidenceCommitment` and EX-005's `AttestedIngressVerified`, in its sharpest form: there is no representation to seal because none was created. It is also the prerequisite for the second theorem row in §5.

**3. Then §14 for the remainder — STILL NOT GRANTED.** E is extracted and the residue is 980 production lines, so the precondition is met and the decision is now available to be made. It is deliberately left to whoever takes the remainder up: this census called the exception *the expected end state, not an assumed one*, and an implementation slice granting itself the exception its own census anticipated would be exactly the self-grant ADR-061 §14 forbids. With E extracted, the residue is ~935 lines of one protocol authority, over the threshold, and the honest disposition is a **reviewed exception**: RFC 6960 §3.2 is a single security argument whose five conjuncts reference each other, and separating them would damage the reasoning rather than clarify it. That exception should be recorded when the extraction lands — **not now**, because an exception granted over a file that still contains an unrelated 336-line control would be granting it for the wrong unit.
...
| deviation | status |
|---|---|
| ~~A 336-line general outbound-fetch/SSRF control lives inside a feature-gated OCSP module~~ | **CLOSED (MCPRE-161)** — `outbound_fetch/`, compiled unconditionally, provenance in the type |
| ~~The §3.2 trust chain's success product is a freely-constructible three-valued enum~~ | **CLOSED (MCPRE-161)** — `TrustedRevocationAnswer`, private field, one producer, and admission consumes `RevocationEvidence` |
| The `inet_aton` canonicalizer's eight `indexing_slicing` sites had never been measured, because the module they lived in is behind a feature gate the ratchet's default lane does not enable | **found by the extraction**, and closed with it: a slice pattern over `vals.as_slice()` states the arity and the field widths as one fact |
