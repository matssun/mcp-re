...
| …and that relation cannot be routed around, because `ClientCredentialWindow::new` is the only producer of an inhabitant | construction boundary of `config_state::client_credential_window` | THM-0102 · `unit://proxy.client_credential_window_sole_producer` · structural probe S04 (`E0451`) | in registry |
...
| …and the value/source pairing happens only inside the communication-assurance authority, so `source` is a provenance report rather than a label any module may attach | authority boundary of `communication_assurance` | THM-0024 · `unit://proxy.certificate_identity_authority_boundary` · structural probes S12 (`E0624`), S13 (`E0451`) | in registry |
...
| …and "every" is the word it means: no caller can produce an inhabitant `interpret` did not accept | representation boundary of `communication_assurance::peer_identity_value` | THM-0023 · `unit://proxy.peer_identity_value_sole_producer` · structural probe S08 (`E0451`) | in registry |
...
| …and those bytes were placed by that owner, not by a caller — which is what holds every provider and consumer to one definition of the encoding | representation boundary of `communication_assurance::ed25519_public_key` | THM-0025 · `unit://proxy.ed25519_public_key_sole_producer` · structural probe S07 (`E0451`) | in registry |
...
| …and the two keys are INDEPENDENTLY OBTAINED: neither operand nor the resulting fact can be assembled by a caller | construction boundaries of the two evidence types and the fact value | THM-0026 · `unit://proxy.credential_key_correspondence_sole_producer` · structural probes S09, S10, S11 (`E0451`) | in registry |
