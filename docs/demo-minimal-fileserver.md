# MCP-RE Minimal Fileserver Demo

**Audience:** an engineer who wants to RUN the self-contained MCP-RE single-node
demo from a fresh clone, with Bazel only, and see — end to end — what the MCP-RE
...
[MCP-RE Core Specification](spec/mcp-re-core-spec.md)) or the
claim boundary (that lives in the
[MCP-RE Security Boundary](spec/security-boundary.md)).

## Purpose

The demo wires the existing MCP-RE building blocks into one cohesive, runnable
round trip against a real, MCP-RE-**unaware** inner MCP server:

```text
HostSession (client) signs an authorized list_files request
  -> mcp-re-proxy verifies the Core envelope (signature, freshness, replay)
  -> mcp-re-proxy evaluates Phase 5 delegated authorization (allow/deny)
  -> mcp-re-proxy strips the external request envelope
  -> mcp-re-proxy injects the sidecar-owned verified context (sole writer)
  -> mcp-re-demo-fileserver executes list_files, confined to a demo root
  -> mcp-re-proxy signs the response
...
- **`demo_negative`** — the fail-closed security path: ten rejected cases, each
  printing the exact frozen `mcp-re.*` reason code and whether the inner server
  was reached.

The inner server, `mcp-re-demo-fileserver`, is a minimal plain-MCP stdio server
that knows nothing about MCP-RE. It exposes one tool, `list_files`, confined to a
...
The demo demonstrates the **single-node** MCP-RE claim:
...
- verifies the request object signature against an injected trust resolver, and
  rejects a tampered body or a tampered JSON-RPC id (`mcp-re.invalid_signature`);
- enforces freshness and **local** replay protection
  (`mcp-re.expired_request`, `mcp-re.replay_detected`);
- enforces audience binding (`mcp-re.invalid_audience`) and requires the MCP-RE
  request envelope to be present (`mcp-re.missing_envelope`);
...
- enforces Phase 5 delegated authorization **before dispatch**, so a
  signed-but-unauthorized path is denied (`mcp-re.authorization_scope_denied`)
  and the inner server is never reached;
- signs the response so the client can bind it to the exact request it sent,
  rejecting a wrong-hash binding (`mcp-re.response_hash_mismatch`) or a corrupted
  response signature (`mcp-re.response_sig_invalid`).

See the [MCP-RE Security Boundary](spec/security-boundary.md)
...
the sibling in-workspace MCP-RE crates plus the pinned serde subset. It needs
...
workspace-relative fixture — see [`mcp-re-demo/src/demo_paths.rs`](../mcp-re-demo/src/demo_paths.rs)),
so no env vars are required for the Cargo path:

```sh
# Cargo (after `cargo build --workspace --bins`):
cargo run -p mcp-re-demo --bin demo_positive   # the positive happy path
cargo run -p mcp-re-demo --bin demo_negative   # the ten fail-closed cases
...
bazel test //mcp-re-demo:all //mcp-re-demo-fileserver:all
```

The runnable binaries are `//mcp-re-demo:demo_positive` and
`//mcp-re-demo:demo_negative`. The test targets follow the `nt_rust_test`
convention, one target per test file, e.g. `//mcp-re-demo:demo_positive_test`,
`//mcp-re-demo:demo_negative_test`, `//mcp-re-demo:demo_proxy_test`,
`//mcp-re-demo:demo_authorization_test`, `//mcp-re-demo:demo_client_test`, and
`//mcp-re-demo-fileserver:server_test`.
...
mutual-TLS socket to `mcp_re_proxy_cli` running as a **separate OS process**. (The
client models the host boundary, not the model: the LLM never holds key material —
`HostSession` signs on behalf of the host layer, and `mcp-re-transport` only carries
bytes.) This is the topology
governed by the
the MCP-RE-P6.6 e2e test plan:

```text
this process: DemoHostClient (HostSession) signs + mcp-re-transport mTLS POST
   │  real mTLS socket (127.0.0.1:<ephemeral>)
   ▼
mcp_re_proxy_cli  (SEPARATE OS process: mTLS terminate + verify, Core verify,
...
bazel test //mcp-re-demo:demo_e2e_test \
  //mcp-re-demo:demo_negative_e2e_test \
  //mcp-re-demo:demo_transport_e2e_test \
  //mcp-re-demo:demo_posture_e2e_test
```
...
### Why a separate `mcp-re-transport` crate (crate-boundary note)

The multi-process client needs a real mTLS transport, but that transport does
**not** live on `mcp-re-host`. The boundary is deliberate:

- **`mcp-re-host` is intentionally transport-free.** It is the agent's local
  key/actor context: it signs the MCP-RE request envelope
  (`HostSigner` / `HostSession`) and verifies signed server responses
  (re-exporting `mcp_re_core::verify_response`). It "produces and consumes raw
...
- **`mcp-re-transport` provides the client-side transport adapter.** It is a
  ...
  request/response bytes, does **no signing**, and depends on neither `mcp-re-host`
  nor `mcp-re-proxy`.
- **Responsibility stays put.** `mcp-re-host` remains responsible for signing
  requests and verifying responses; `mcp-re-transport` only carries bytes; the
  proxy remains the server-side authority. The demo client composes the two:
  `HostSession` signs, `MtlsClient` carries.

### Expected successful output

`bazel run //mcp-re-demo:demo_e2e` brings up the proxy, drives the authorized P1
flow over the real mTLS socket, and prints the verified response and a final
`OK`. The proxy also emits informational `mcp-re-proxy:` lines (key-file mode
...
```text
proxy-up addr=127.0.0.1:<port> inner=mcp_re_demo_fileserver_bin authz=reference replay=file binding=exact
...
OK: authorized list_files round-tripped client -> mcp_re_proxy_cli (separate process, real mTLS) -> mcp_re_demo_fileserver_bin -> client; transport-binding exact satisfied (mTLS identity == request signer)
```
...
|----------|----------------|-----------------|----------------------------------|
| P1 | Authorized `list_files` round-trips client → proxy → inner → client over real mTLS; signed response verifies vs the stored request hash; binding `exact` satisfied; fixture entries (`q1.txt`, `q2.txt`) returned | full pipeline | `//mcp-re-demo:demo_e2e_test` (`positive_path_multi_process_mtls_round_trip`) |
| A1 | Body tampered after signing → `mcp-re.invalid_signature`; inner NOT reached | Core verify | `//mcp-re-demo:demo_negative_e2e_test` (`a1_tampered_request_body_rejected_over_wire`) |
| A2 | JSON-RPC id tampered after signing → `mcp-re.invalid_signature`; inner NOT reached | Core verify | `//mcp-re-demo:demo_negative_e2e_test` (`a2_tampered_jsonrpc_id_rejected_over_wire`) |
| A3 | Replay on a FRESH mTLS connection → `mcp-re.replay_detected` (first send succeeds, second denied) | Core / replay | `//mcp-re-demo:demo_negative_e2e_test` (`a3_replayed_request_rejected_on_fresh_connection`) |
| A4 | Expired request → `mcp-re.expired_request`; inner NOT reached | Core / freshness | `//mcp-re-demo:demo_negative_e2e_test` (`a4_expired_request_rejected_over_wire`) |
| A5 | Wrong audience → `mcp-re.invalid_audience`; inner NOT reached | Core verify | `//mcp-re-demo:demo_negative_e2e_test` (`a5_wrong_audience_rejected_over_wire`) |
| A6 | Missing MCP-RE envelope → `mcp-re.missing_envelope`; inner NOT reached | Core verify | `//mcp-re-demo:demo_negative_e2e_test` (`a6_missing_envelope_rejected_over_wire`) |
| A7 | Caller-smuggled `.verified` block stripped & replaced by the sidecar; call still succeeds and verifies under the server | proxy strip/inject | `//mcp-re-demo:demo_negative_e2e_test` (`a7_smuggled_verified_metadata_stripped_and_replaced_over_wire`) |
| A8 | Validly-signed but non-granted path → `mcp-re.authorization_scope_denied`; deny-before-dispatch, inner NOT reached | Phase-5 authz | `//mcp-re-demo:demo_negative_e2e_test` (`a8_unauthorized_path_rejected_over_wire`) |
| A9 | Server signs a different body under the same id → client rejects with `McpReError::ResponseHashMismatch` (inner reached; client refuses the binding) | client response verify | `//mcp-re-demo:demo_negative_e2e_test` (`a9_wrong_response_hash_rejected_by_client_over_wire`) |
| A10 | Corrupted response signature → client rejects with `McpReError::ResponseSigInvalid` (inner reached; client refuses the signature) | client response verify | `//mcp-re-demo:demo_negative_e2e_test` (`a10_bad_response_signature_rejected_by_client_over_wire`) |
| T1 | No client cert presented → mTLS handshake rejected; inner NOT reached | mTLS handshake | `//mcp-re-demo:demo_transport_e2e_test` (`t1_no_client_cert_rejected_at_handshake`) |
| T2 | Client cert from an untrusted CA → handshake rejected; inner NOT reached | mTLS handshake | `//mcp-re-demo:demo_transport_e2e_test` (`t2_untrusted_client_ca_rejected_at_handshake`) |
| T3 | Trusted cert whose identity ≠ request signer under binding `exact` → `mcp-re.transport_binding_failed`; deny-before-dispatch | transport binding | `//mcp-re-demo:demo_transport_e2e_test` (`t3_identity_not_signer_transport_binding_failed`) |
| T4 | Client cert over `--max-client-cert-lifetime` → `mcp-re.transport_binding_failed`; inner NOT reached | transport posture | `//mcp-re-demo:demo_transport_e2e_test` (`t4_over_lifetime_client_cert_rejected`) |
| T5 | Untrusted server cert → the verifying client aborts the handshake (`TransportError::Handshake`) before sending the body; proves server authentication | mTLS handshake (client) | `//mcp-re-demo:demo_transport_e2e_test` (`t5_untrusted_server_cert_refused_by_client`) |
| C1 | `--key-source env` fails closed: WITHOUT `--allow-env-keysource` the parse-time gate refuses it; and (MCPS-076) because `EnvKeySource` is gated behind the non-default `dev_env_key_source` feature, the production binary refuses env key material EVEN WITH `--allow-env-keysource` (diagnostic names the feature rebuild). Either way: non-zero exit, no listening socket. Positive env-load control is the feature-built `//mcp-re-proxy:dev_env_key_source_test`. | startup posture | `//mcp-re-demo:demo_posture_e2e_test` (`c1_env_keysource_without_opt_in_fails_closed_no_listener`, `c1_env_keysource_fails_closed_even_with_opt_in_in_production_build`) |
| C2 | Request accepted by one proxy process is rejected `mcp-re.replay_detected` by a fresh proxy process sharing the same durable `--replay-path` | durable replay | `//mcp-re-demo:demo_posture_e2e_test` (`c2_replay_detected_after_proxy_restart_with_shared_durable_cache`) |
...
- `//mcp-re-transport:mtls_client_test` — the verifying mTLS client:
...
- `//mcp-re-demo:demo_mtls_client_test` — the runnable client path
  (`HostSession` sign → `MtlsClient` carry → response verify) against a real
  `mcp_re_proxy::serve_once` server, in-process.
- `//mcp-re-demo:demo_fixtures_test` — the shared `DemoFixtures` helper is
...
Everything above fronts `mcp_re_demo_fileserver_bin`, an inner shaped like a
**one-shot** server: `mcp_re_proxy_cli` spawns a fresh inner subprocess **per
...
`mcp_re_demo_server_bin` — that is **spawned and initialized once** and then serves
**many requests over one persistent process**. The demo server is deliberately
**MCP-RE-unaware**: it speaks plain MCP JSON-RPC over newline-delimited stdio
...
this process: DemoHostClient (HostSession) signs + mcp-re-transport mTLS POST
   │  real mTLS socket (127.0.0.1:<ephemeral>)
   ▼
mcp_re_proxy_cli --inner-mode persistent
   ...
   ▼
mcp_re_demo_server_bin  (the ordinary LONG-LIVED inner MCP server;
...
bazel test //mcp-re-demo:demo_e2e_persistent_test \
  //mcp-re-proxy:persistent_inner_test \
  //mcp-re-proxy:persistent_session_test \
  //mcp-re-proxy:persistent_scope_test \
  //mcp-re-demo-server:server_test \
  //mcp-re-demo-server:persistence_e2e_test
```

The proxy binary and the demo-server binary are delivered via Bazel runfiles and
resolved from the `$(rlocationpath ...)` env vars (`MCP_RE_PROXY_CLI`,
...
For the one-shot fileserver, `mcp_re_proxy_cli` spawns the inner **per request,
...
block** (`mcp-re.authorization_block_missing`) — fail-closed is the default. So a
...
- **`mcp-re-demo-server`** — the ordinary, **long-lived**, **MCP-RE-unaware** inner
...
- **`mcp-re-proxy`** — the **Policy Enforcement Point (PEP)**. It is the server-side
  ...
  long-lived session.
- **`mcp-re-host`** — intentionally **transport-free**. `HostSession` signs the
  request envelope and verifies signed responses; it holds no socket and no async.
- **`mcp-re-transport`** — **carries bytes only**. The verifying blocking rustls
  mTLS client: presents the client cert and verifies the proxy's server cert; it
  does **no** signing.

### Expected successful output

`bazel run //mcp-re-demo:demo_e2e_persistent` brings up the proxy in persistent
...
`mcp-re-proxy:` stderr lines, including `inner process model = persistent
(spawn-once + initialize handshake; …)` and a single `inner_spawned` event. The
salient lines are (hashes/ports vary per run):

```text
mcp-re-proxy: inner process model = persistent (spawn-once + initialize handshake; long-lived inner serves many requests over one process)
mcp-re-proxy: inner-event inner_spawned inner=<…>/mcp_re_demo_server_bin Spawned { pid: <pid> }
proxy-up addr=127.0.0.1:<port> inner=mcp_re_demo_server_bin inner_mode=persistent authz=reference replay=file binding=exact
...
denied-call tool=reset_items reason=mcp-re.authorization_scope_denied (rejected before dispatch; persistent inner never forwarded)
OK: 3 authorized tools/call round-tripped over ONE persistent inner (client -> mcp_re_proxy_cli --inner-mode persistent (separate process, real mTLS) -> mcp_re_demo_server_bin -> client); admin reset_items scope-denied before dispatch; transport-binding exact satisfied (mTLS identity == request signer)
```
...
the MCP-RE-unaware server); the rest are the sidecar fronting it.

| Scenario | What it proves | Enforcing layer | Bazel test target (`#[test]` fn) |
|----------|----------------|-----------------|----------------------------------|
| Long-lived server, in-process | `initialize` gates the lifecycle; `tools/list` advertises the three scoped tools; each tool's `tools/call` behaves; malformed/unknown inputs are JSON-RPC errors, not panics; `shutdown` is acknowledged | demo server (MCP-RE-unaware) | `//mcp-re-demo-server:server_test` (11 fns incl. `initialize_returns_well_formed_result`, `tools_list_includes_three_scoped_tools_after_initialize`, `shutdown_is_acknowledged_and_flagged_to_stop`) |
| Long-lived server, real process | The REAL binary serves `initialize` + several `tools/call` over **one** process (newline-delimited stdin), one response per request, correlated by id in order, clean exit on EOF | demo server persistence | `//mcp-re-demo-server:persistence_e2e_test` (`many_requests_over_one_long_lived_process`) |
| Proxy fronts a persistent inner | The proxy spawns the real demo server **once**, does the `initialize` handshake once, forwards N verified requests over the **same** process: one `inner_spawned`, N `inner_request_forwarded`; fail-closed on inner crash / malformed line / handshake failure; a pre-dispatch denial yields **zero** added `inner_request_forwarded` while the inner stays alive | proxy persistent inner | `//mcp-re-proxy:persistent_inner_test` (6 fns: `persistent_inner_initializes_and_forwards_one_request`, `n_requests_over_one_persistent_inner_process`, `pre_dispatch_denial_yields_zero_forwards_while_inner_stays_alive`, `inner_crash_mid_session_fails_closed_not_hang_or_panic`, `malformed_response_line_fails_closed`, `handshake_failure_aborts_construction`) |
| Per-request strip/inject over one session | Over N (≥3) requests on the SINGLE session, a forged caller `.verified` is stripped and a sidecar-owned context (verifier = this proxy, verified_signer = the verified inbound signer, with on_behalf_of/request_hash) is injected **fresh every request**; one `inner_spawned`, N `inner_request_forwarded`, N `inner_response_signed`, one clean `inner_exited` on teardown | proxy strip/inject | `//mcp-re-proxy:persistent_session_test` (`per_request_strip_inject_over_one_persistent_session`) |
| Phase-5 scopes public / protected / admin | Over one persistent session: public `echo` succeeds with the baseline grant; protected `list_items` succeeds with its grant and is `mcp-re.authorization_scope_denied` with only the public grant; admin `reset_items` is denied (no grant); each denial adds **zero** `inner_request_forwarded` and the session keeps serving (a later authorized call still succeeds); exactly 3 forwarded, 3 signed, 1 `inner_spawned`, 1 `inner_exited` on teardown | Phase-5 authz (deny-before-dispatch) | `//mcp-re-proxy:persistent_scope_test` (`phase5_scopes_over_one_persistent_session`) |
| Multi-process mTLS over the long-lived server | The real `mcp_re_proxy_cli --inner-mode persistent` (separate OS process, mTLS, authz=reference, durable replay, binding=exact) fronts the real demo server: three authorized `tools/call` (echo, list_items, echo) round-trip over ONE inner process and each signed response verifies vs the stored request hash under the proxy's server signer; the admin `reset_items` is `mcp-re.authorization_scope_denied` and the session keeps serving; the proxy ran as a separate process | full pipeline, persistent | `//mcp-re-demo:demo_e2e_persistent_test` (`persistent_multi_call_multi_process_mtls`) |
...
`//mcp-re-demo:demo_e2e_persistent` is the runnable form of the last row.
...
```sh
mcp-re-demo-fileserver --demo-root <DEMO_ROOT>
```
...
`bazel run //mcp-re-demo:demo_positive` prints a structured allow-decision line,
...
`PASS` line per case carrying the frozen `mcp-re.*` reason code. It exits `0` only
...
```text
MCP-RE local fail-closed paths — each case must be rejected with its frozen mcp-re.* reason:
...
- **`//...` label errors / target not found.** Make sure you are at the
  repository root and use `//mcp-re-demo:...` labels.
- **`cannot locate the inner 'mcp-re-demo-fileserver' binary` (Cargo).** The demo
...
- [MCP-RE Security Boundary](spec/security-boundary.md) — the
  authoritative statement of the single-node claim and the forbidden claims.
- [MCP-RE Conformance Guide](conformance-guide.md) — how to run the executable
  conformance suite and the drift-guarded conformance manifest
  (`mcp-re-conformance/conformance_manifest.json`) that enumerates
  every vector and test target.
- MCP-RE-P6.6 e2e test plan — the plan that defines the multi-process topology
  and the P1 / A1–A10 / T1–T5 / C1–C2 scenario matrix demonstrated above.
- MCP-RE-P6.6 demo findings — the honest gate statement and the security-boundary
  linkage. The dedicated demo test-traceability mapping now lives in the matrix
  above.
- MCP-RE-P6.6B demo findings — the long-lived-server gate statement and why the
