# MCP-RE Code & Security Re-Audit — 0.2
...
| Predecessor | [0.1 audit (2026-06-01)](./MCP-RE-code-audit-2026-06-01.md) |
...
|---|---:|---:|---:|---:|---:|
| `mcp-re-core` | 0 | 0 | 2 | 4 | 30 |
| `mcp-re-policy` | 0 | 1 | 4 | 9 | 41 |
| `mcp-re-proxy` | 4 | 12 | 21 | 41 | 170 |
| `mcp-re-transport` | 0 | 2 | 3 | 5 | 13 |
| **Total** | **4** | **15** | **30** | **59** | **254** |

All four criticals and the dominant high cluster localize to `mcp-re-proxy` (OCSP + Redis replay + CLI wiring); the single policy high is the manifest-atomicity break.
...
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/ocsp.rs:257-265`
...
- **Verification verdict:** CONFIRMED — 3/3 skeptics. Caveat: latent today — no production code outside `mcp-re-policy` calls `verify()`, and only the in-memory reference store exists. High on a contract/fail-closed basis: the documented atomicity guarantee is violated and a hostile server writes a chosen pin during a failing verify; it becomes live the moment a persistent store is wired.

### H-2 — No OCSP nonce sent or validated — responder replies are replayable
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/ocsp.rs:310-322`
...
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/ocsp.rs:258-264`
...
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/ocsp.rs:316-318, 257-265`
...
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/ocsp.rs:259-264`
...
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/redis_store.rs:79-82` (caller `shared_replay.rs:168-169`)
...
- **Crate / location:** `mcp-re-proxy` · `mcp-re-proxy/src/redis_store.rs:53-57` (connect) and `93-102` (per-op query)
...
> **High-finding accounting.** The 15 confirmed HIGH findings comprise 1 in `mcp-re-policy` (H-1) and 14 across the proxy/transport surface (H-2 … H-11 documented above, plus 4 further proxy/transport highs grouped under the same OCSP/replay clusters). The crate table (§4) is authoritative for per-crate counts: `mcp-re-transport` 2, `mcp-re-proxy` 12, `mcp-re-policy` 1.
...
|---|---:|---:|---:|
| `mcp-re-core` | 2 | 4 | 30 |
| `mcp-re-policy` | 4 | 9 | 41 |
| `mcp-re-proxy` | 21 | 41 | 170 |
| `mcp-re-transport` | 3 | 5 | 13 |
| **Total** | **30** | **59** | **254** |

Medium (30) is concentrated in `mcp-re-proxy` (21) — residual hardening and conformance edge-cases around the new backends. Low (59) is style/defensive-depth/minor robustness. Info (254) is observations, documentation notes, and confirmed positive controls; no action required.
...
| **M-11/M-13** — drift guard omits packages | **CLOSED** | Guard now scans all 9 mcp-re packages (was 5), with a fail-closed regression test; `bazel query` ground-truth confirms zero drift. |
...
|---|---|---|
| Request signature binding — every admitted request Ed25519-verified over the canonical full JSON-RPC object; no alternate preimage | **HOLDS** (high) | Single admit gate `Proxy::handle_with_transport` (`proxy.rs:202`) → `mcp_re_core::verify_request` is the sole admission decision (`proxy.rs:214-223`); manifest/delegated-signer introduce no alternate preimage. |
...
6. **L-1 — supply-chain scan.** Run `cargo-deny` / SBOM advisory + license + duplicate-version scan across the `mcp-re-*` crate graph (carried from the 0.1 critic-gap list, still unverified) and gate it in CI.
