...
//!
//! MCP-S **binds, never interprets** authorization evidence (bind-not-interpret).
...
/// [`McpsError::AuthorizationBindingProfileRequired`]). The returned binding is
...
pub trait AuthorizationBindingProvider {
    ...
    fn provide(&self, ctx: &BindingRequestContext) -> Result<AuthorizationBinding, McpsError>;
}
...
/// type the route does not permit is [`McpsError::AuthorizationBindingTypeUnsupported`].
/// A provider that cannot produce the (mandatory) binding propagates its typed
/// error (e.g. [`McpsError::AuthorizationBindingMissing`]) — fail closed. The
...
) -> Result<AuthorizationBinding, McpsError> {
...
impl AuthorizationBindingProvider for OpaqueBytesProvider {
    fn provide(&self, _ctx: &BindingRequestContext) -> Result<AuthorizationBinding, McpsError> {
...
pub trait AuthorizationReferenceResolver {
    ...
    fn resolve(&self, ctx: &BindingRequestContext) -> Result<AuthzReference, McpsError>;
}

/// Provider for the `authz-system-reference` form. WITHOUT a configured resolver it
/// fails closed ([`McpsError::AuthorizationBindingMissing`]) — the reference form
...
impl AuthorizationBindingProvider for StructuredObjectHashingProvider {
    fn provide(&self, _ctx: &BindingRequestContext) -> Result<AuthorizationBinding, McpsError> {
        Err(McpsError::AuthorizationBindingProfileRequired)
    }
}
