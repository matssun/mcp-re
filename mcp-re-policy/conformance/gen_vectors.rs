...
//!
//! Run: `bazel run //components/mcp-re/mcp-re-policy:gen_phase5_vectors > \
...
//! which replays each vector through `mcp_re_core::verify_request` (must succeed —
...
fn now_unix() -> i64 {
    mcp_re_core::parse_rfc3339_utc(ISSUED_AT).expect("parse issued_at") + NOW_OFFSET
}
...
fn authorization_block(profile_id: &str, artifact_bytes: &[u8]) -> Value {
    json!({
        "profile": profile_id,
        "artifact": mcp_re_core::b64url_encode(artifact_bytes),
    })
}
...
fn vector(
    ...
    let now = match now_offset_override {
        Some(ts) => mcp_re_core::parse_rfc3339_utc(ts).expect("parse override now"),
        None => now_unix(),
    };
    json!({
        "name": name,
...
fn main() {
    ...
    {
        let artifact = mint_reference_grant(&base_spec(), &issuer_key(), ISSUER_KEY_ID).unwrap();
        let block = authorization_block("se.syncom/mcp-re-authz-biscuit-v1", &artifact);
