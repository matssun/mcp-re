//! The `se.syncom/mcp-re.authorization` sibling `_meta` block
//! (ADR-MCPS-013).
//!
...
/// in `mcp-re-core`) so Core stays byte-for-byte unchanged; it is never part of any
...
pub const AUTHORIZATION_META_KEY: &str = "se.syncom/mcp-re.authorization";
...
pub struct AuthorizationBlock {
    ...
    /// `se.syncom/mcp-re-authz-reference-v1`).
    pub profile: String,
...
mod tests {
    ...
    fn extracts_a_well_formed_block_and_decodes_the_artifact() {
        ...
        let req = request_with_block(json!({
            "profile": "se.syncom/mcp-re-authz-reference-v1",
            "artifact": b64url_encode(bytes),
        }));
        let block = extract_authorization_block(&req).expect("extract");
        assert_eq!(block.profile, "se.syncom/mcp-re-authz-reference-v1");
