...
//!
//! Profile id `se.syncom/mcp-re-authz-reference-v1`. This is a native,
...
pub const REFERENCE_PROFILE_ID: &str = "se.syncom/mcp-re-authz-reference-v1";
...
fn verify_issuer_signature(
    ...
    // `verify_ed25519_with` returns its own McpReError sentinel on any failure;
...
mod tests {
    ...
    fn now() -> i64 {
        mcp_re_core::parse_rfc3339_utc(NOT_BEFORE).expect("parse not_before") + 1_800
    }
    ...
    fn verified_for(artifact: &[u8]) -> VerifiedRequest {
        VerifiedRequest {
            ...
            audience: SERVER.to_string(),
            authorization: mcp_re_core::VerifiedAuthorization::Draft01Hash {
    ...
    fn mint_with_raw_grants(grants: Value) -> Vec<u8> {
        let mut artifact = json!({
            ...
            "revocation_id": "rev-1",
            "signature": { "alg": mcp_re_core::SIG_ALG_ED25519, "key_id": ISSUER_KEY_ID },
        });
        let preimage = mcp_re_core::canonicalize_json_value(&artifact).unwrap();
        artifact["signature"]["value"] = Value::String(issuer_key().sign(&preimage));
        mcp_re_core::canonicalize_json_value(&artifact).unwrap()
    }
