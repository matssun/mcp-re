        ...
        match &verified.authorization {
            ...
            VerifiedAuthorization::Draft02Binding {
                ...
                AuthorizationBinding::AuthzSystemReference {
                    ...
                    // if none is registered (MCP-RE binds, never interprets).
...
mod tests {
    ...
    fn now() -> i64 {
        mcp_re_core::parse_rfc3339_utc(NOT_BEFORE).expect("parse") + 1_800
    }
    ...
    fn verified_with(authorization: VerifiedAuthorization) -> VerifiedRequest {
        let canonicalization_id = match &authorization {
            VerifiedAuthorization::Draft02Binding { .. } => Some("mcp-re-jcs-int53-json-v1".to_string()),
    ...
    fn unknown_profile_is_unsupported() {
        let artifact = mint_reference_grant(&spec(), &issuer_key(), ISSUER_KEY_ID).unwrap();
        let (request, verified) = request_and_verified(&artifact, "se.syncom/mcp-re-authz-biscuit-v1");
        let decision = evaluator().evaluate(
            &verified,
            &request,
            &resolver(),
    ...
    /// proving the hand-off path without MCP-RE interpreting reference semantics.
    ...
    impl crate::profile::AuthorizationReferenceResolver for StubReferenceResolver {
        ...
        fn authorize_reference(
            ...
            _request: &Value,
            _resolver: &dyn mcp_re_core::TrustResolver,
