...
//! can launch `mcps-proxy` and connect to it with a matching client cert.
//!
//! Only the TLS certs/keys vary per run (rcgen mints fresh keys); the MCP-S
...
//!
//!   cargo run -p mcps-demo --example emit_mtls_fixtures -- <out-dir>

use std::fs;
use std::path::Path;

use mcps_demo::demo_fixtures::DemoFixtures;
