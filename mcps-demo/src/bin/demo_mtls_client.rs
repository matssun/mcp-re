...
//!
//! Signs ONE MCP-S `tools/call` with the stateful `HostSession` (nonce from an
...
//! the proxy over mTLS via the reusable `mcps-transport` client (which also
...
//!
//! This bin only ORCHESTRATES: signing/correlation live in `mcps-host`
//! ([`MtlsClientRunner`](mcps_demo::MtlsClientRunner) drives `HostSession`) and the
//! mTLS connection + wire framing live in `mcps-transport`. The bin holds NO
...
//!
//! ```sh
//! bazel run //mcps-demo:demo_mtls_client -- \
...
fn run() -> Result<(), String> {
    ...
    // identity against the configured server CA. All TLS lives in mcps-transport.
