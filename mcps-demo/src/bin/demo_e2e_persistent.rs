...
//! `mcps-demo-server` (MCPS-066, MCPS-EPIC-P6.6B).
//!
...
//!    │  real mTLS socket
//!    ▼
//! mcps_proxy_cli --inner-mode persistent  (SEPARATE OS process: mTLS, Core verify,
...
fn run() -> Result<(), String> {
    ...
    println!(
        "proxy-up addr={} inner=mcps_demo_server_bin inner_mode=persistent authz=reference \
...
/// A spawned `mcps_proxy_cli` OS process, killed (and reaped — with its
...
/// Spawn the real `mcps_proxy_cli --inner-mode persistent` wrapping the demo
