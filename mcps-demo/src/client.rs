//! The demo `HostSession` client (MCPS-046, ADR-MCPS-015 / MCPS-EPIC-P6.5).
//!
...
//!   [`HostSigner`](mcps_host::HostSigner) directly — so freshness, nonce, and
...
//!   keeps any local stdio piping in the demo crate, so `mcps-host` itself
...
use mcps_host::VerifiedResult;
use serde_json::Value;

/// A demo client that signs MCP-S requests and verifies the bound responses,
...
impl<C: Clock, N: NonceSource> DemoHostClient<C, N> {
    ...
    /// in-flight `id` fails closed with [`McpsError::ReplayDetected`].
    pub fn sign_request(
        &mut self,
        id: &Value,
        method: &str,
    ...
    /// [`McpsError::MissingEnvelope`]; a wrong binding yields
    /// [`McpsError::ResponseHashMismatch`] and leaves the pending entry in place.
    ...
    /// inner ERROR as [`mcps_host::UnwrappedResult::InnerError`]. Consumers that
