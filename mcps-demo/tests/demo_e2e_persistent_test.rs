//! Multi-process mTLS E2E over the LONG-LIVED `mcps-demo-server` (MCPS-066,
//! MCPS-EPIC-P6.6B).
//!
...
//!
//! ```text
//! this test process: DemoHostClient (HostSession) signs + mcps-transport mTLS POST
...
//!     rejected with `mcps.authorization_scope_denied`, after which a fourth
...
use mcps_demo::assemble_assertions;
use mcps_demo::independently_verify_response;
use mcps_demo::inner_received_id;
use mcps_demo::run_persistent_e2e;
use mcps_demo::server_response_public_key;
use mcps_demo::DemoFixtureFiles;
use mcps_demo::DemoFixtures;
use mcps_demo::PersistentE2eEvidence;
use mcps_demo::PERSISTENT_DENIED_ID;
use mcps_demo::PERSISTENT_ECHO_1_ID;
use mcps_demo::PERSISTENT_ECHO_2_ID;
use mcps_demo::PERSISTENT_LIST_ID;
use mcps_demo::TOOL_ECHO;
use mcps_demo::TOOL_LIST_ITEMS;
...
/// A spawned `mcps_proxy_cli` OS process, killed (and reaped — together with its
...
/// Spawn the real `mcps_proxy_cli --inner-mode persistent` as its OWN process
...
fn spawn_persistent_proxy(fixtures: &DemoFixtures) -> ProxyProcess {
    ...
    let replay_dir = std::env::temp_dir().join(format!("mcps_persist_replay_{}", std::process::id()));
    ...
    let mut child = Command::new(&cli)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn mcps_proxy_cli --inner-mode persistent");
