...
//! MCPS-EPIC-P6 Child Issue 4).
//!
//! These tests drive the EXISTING `mcps-proxy` serving path with Phase 5
//! (ADR-MCPS-013) policy enforcement turned on, while the proxy launches the REAL
//! `mcps-demo-fileserver` binary as its inner stdio subprocess. They prove the
...
//!      escaping `path`) → `mcps.authorization_scope_denied`, and the inner
//!      fileserver is never spawned;
//!   3. an `authorization_hash` that does not bind the attached artifact →
//!      `mcps.authorization_hash_mismatch`, inner never spawned;
//!   4. an EXPIRED grant (`now` past its `expires_at`) → `mcps.authorization_expired`,
//!      inner never spawned.
//!
...
fn authorization_hash_mismatch_is_denied_before_dispatch() {
    ...
    assert_eq!(error_message(&response), "mcps.authorization_hash_mismatch");
    assert!(
        !sink.inner_was_reached(),
...
fn expired_grant_is_denied_before_dispatch() {
    ...
    assert_eq!(error_message(&response), "mcps.authorization_expired");
    assert!(
        !sink.inner_was_reached(),
