//! `mcp-re-demo-fileserver` — a minimal, MCP-RE-UNAWARE stdio MCP server (MCPS-045).
//!
//! This is the in-tree demo target for MCP-RE-EPIC-P6.5: an ORDINARY MCP server
//! that the MCP-RE sidecar (`mcp-re-proxy`) wraps unchanged. It speaks plain MCP
...
//! the pure `mcp-re-core`, it MAY use `std::fs` / `std::io` (it is a file server).
