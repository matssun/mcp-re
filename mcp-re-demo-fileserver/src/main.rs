//! `mcp-re-demo-fileserver` binary entrypoint (MCPS-045).
//!
...
//! `--received-log <PATH>` or the `MCP_RE_DEMO_FILESERVER_RECEIVED_LOG` env var
//! (the flag wins). OFF by default, so ordinary runs are unaffected.
//!
//! Usage:
//!   mcp-re-demo-fileserver --demo-root <DIR> [--received-log <PATH>]
...
use mcp_re_demo_fileserver::serve_stdio;
use mcp_re_demo_fileserver::FileServer;

/// Env var fallback for the received-request log path (the `--received-log` flag
/// takes precedence when both are present).
const RECEIVED_LOG_ENV: &str = "MCP_RE_DEMO_FILESERVER_RECEIVED_LOG";
...
fn parse_args(argv: &[String]) -> Result<CliArgs, String> {
    ...
    let demo_root = demo_root.ok_or_else(|| {
        "usage: mcp-re-demo-fileserver --demo-root <DIR> [--received-log <PATH>]".to_string()
    })?;
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("mcp-re-demo-fileserver: {err}");
            ExitCode::FAILURE
        }
    }
}
