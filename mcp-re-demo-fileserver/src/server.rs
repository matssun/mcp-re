...
//! MCP-RE-UNAWARE stdio MCP server.
//!
...
//!
//! It knows nothing about MCP-RE signing, envelopes, or verified context — that
//! is the sidecar's job (the proxy wraps this server unchanged). The
//! `net.mcp-re.intendedScope` annotation on each tool is pure metadata: the server
...
//!
//! Confinement (independent of, and in addition to, any MCP-RE authorization):
...
impl FileServer {
    ...
    /// `requestState` — the ADR-MCPS-047 / D5 contract. MCP-RE never interprets it; only
...
/// JSON-array bytes. Opaque to MCP-RE and the client (echoed verbatim); meaningful
/// only to this server, which decodes it to resume — the ADR-MCPS-047 / D5 contract.
/// Hand-rolled hex keeps this MCP-RE-unaware server dependency-free.
