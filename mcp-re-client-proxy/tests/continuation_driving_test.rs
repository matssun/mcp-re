...
//! terminal result. The local client never touches an MCP-RE field.
...
/// A stateless MCP-RE remote that BRANCHES on the signed request: a request WITHOUT
...
fn params(seq: u32) -> CallParams {
    CallParams {
        ...
        nonce: format!("mcp-re-client-proxy-continuation-test-nonce-{seq:03}"),
...
fn proxy_drives_input_required_then_continuation() {
    ...
    // No MCP-RE field leaked to the client.
