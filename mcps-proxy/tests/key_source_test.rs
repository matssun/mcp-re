...
use mcps_proxy::key_source::FileKeySource;
...
fn tmp(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("mcps_ks_{}_{name}", std::process::id()))
}
...
fn file_source_missing_file_is_not_found() {
    let source = FileKeySource {
        signing_key_seed_path: "/nonexistent/mcps/seed".to_string(),
        tls_cert_path: "/nonexistent/mcps/cert".to_string(),
        tls_key_path: "/nonexistent/mcps/key".to_string(),
        client_ca_path: "/nonexistent/mcps/ca".to_string(),
    };
    assert!(matches!(
...
fn env_source_loads_all_material() {
    ...
    let seed_v = "MCPS_TEST_SEED_ENV";
    let cert_v = "MCPS_TEST_CERT_ENV";
    let key_v = "MCPS_TEST_KEY_ENV";
    let ca_v = "MCPS_TEST_CA_ENV";
...
fn env_source_missing_var_is_not_found() {
    let source = EnvKeySource {
        signing_key_seed_var: "MCPS_TEST_DEFINITELY_UNSET_VAR".to_string(),
