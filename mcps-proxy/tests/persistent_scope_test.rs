//! MCPS-065 (MCPS-EPIC-P6.6B) — Phase-5 authorization over the LONG-LIVED
//! `mcps-demo-server`'s three scoped tools, through a real [`Proxy`] fronting a
//! single [`PersistentSubprocessInner`].
//!
//! The demo server (#3956) is MCP-S-UNAWARE: it publishes each tool's intended
//! scope as inert `annotations.net.mcps.intendedScope` metadata and enforces
