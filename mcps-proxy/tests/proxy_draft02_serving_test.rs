...
//!
//! The draft-02 request is signed by `mcps-client-core` (the real client seam);
//! the draft-01 request by `mcps-host` (the legacy ambassador).
...
fn draft01_serving_path_is_unchanged() {
    ...
    let expected_hash =
        mcps_core::request_hash(&serde_json::from_slice::<Value>(&request).unwrap()).expect("hash");
