...
//!
//! Stands up a fleet of independent serving `mcps-proxy` instances (each its own
...
//!
//! Feature-gated on `redis_replay` and skipped when `MCPS_TEST_REDIS_URL` is
//! unset (hard-failed under `MCPS_REQUIRE_LIVE_INFRA`), mirroring
...
/// Redis → skip, unless `MCPS_REQUIRE_LIVE_INFRA` demands the live lane run.
fn redis_url() -> Option<String> {
    ...
    if url.is_none() && require_live_infra() {
        panic!(
            "MCPS_REQUIRE_LIVE_INFRA is set but MCPS_TEST_REDIS_URL is unavailable; \
...
fn replica(url: &str) -> Proxy {
    ...
    let store = RedisAtomicReplayStore::connect(url).expect("connect to MCPS_TEST_REDIS_URL Redis");
    Proxy::new(
        server_key(),
        SERVER,
        SERVER_KEY_ID,
...
fn signed_request(now: i64, nonce: &str) -> Vec<u8> {
    ...
    let expires_at = mcps_core::unix_to_rfc3339_utc(now + 600);
...
fn admit_on_replica_a_then_replay_rejected_on_replica_b() {
    let Some(url) = redis_url() else {
        eprintln!("SKIP admit_on_replica_a_then_replay_rejected_on_replica_b: MCPS_TEST_REDIS_URL unset");
        return;
    };
    let now = now_unix();
...
fn replay_is_rejected_on_every_sibling_regardless_of_admitting_node() {
    let Some(url) = redis_url() else {
        eprintln!("SKIP replay_is_rejected_on_every_sibling_regardless_of_admitting_node: MCPS_TEST_REDIS_URL unset");
        return;
    };
    let now = now_unix();
    ...
    assert_eq!(error_message(&nodes[0].handle(&request, now)), "mcps.replay_detected");
    assert_eq!(error_message(&nodes[2].handle(&request, now)), "mcps.replay_detected");
}
...
fn distinct_nonces_are_admitted_across_the_fleet() {
    let Some(url) = redis_url() else {
        eprintln!("SKIP distinct_nonces_are_admitted_across_the_fleet: MCPS_TEST_REDIS_URL unset");
        return;
    };
    let now = now_unix();
    let nodes = fleet(&url, 3);
