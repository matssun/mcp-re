...
//! `MCPS_TEST_REDIS_URL` env var: when it is unset the test prints a skip notice
...
fn redis_url() -> Option<String> {
    let url = std::env::var("MCPS_TEST_REDIS_URL").ok().filter(|u| !u.trim().is_empty());
...
fn node(url: &str) -> SharedReplayCache {
    let store = RedisAtomicReplayStore::connect(url)
        .expect("connect to MCPS_TEST_REDIS_URL Redis");
...
fn cross_node_insert_via_a_is_replay_via_b() {
    let Some(url) = redis_url() else {
        eprintln!(
            "SKIP cross_node_insert_via_a_is_replay_via_b: MCPS_TEST_REDIS_URL unset \
...
fn single_node_fresh_then_replay() {
    let Some(url) = redis_url() else {
        eprintln!(
            "SKIP single_node_fresh_then_replay: MCPS_TEST_REDIS_URL unset \
             (no Redis available in this environment)"
        );
        return;
    };
...
/// below the absolute-epoch range. Gated on `MCPS_TEST_REDIS_URL` exactly like the
...
fn live_pttl_is_bounded_window_not_absolute_epoch() {
    let Some(url) = redis_url() else {
        eprintln!(
            "SKIP live_pttl_is_bounded_window_not_absolute_epoch: MCPS_TEST_REDIS_URL \
