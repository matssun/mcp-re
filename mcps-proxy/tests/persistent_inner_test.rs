//! MCPS-063 (MCPS-EPIC-P6.6B) — the proxy fronts a LONG-LIVED inner MCP server.
//!
//! These tests drive the REAL `mcps-demo-server` binary (#3956) as the
...
fn pre_dispatch_denial_yields_zero_forwards_while_inner_stays_alive() {
    ...
    assert_eq!(value["error"]["message"].as_str(), Some("mcps.missing_envelope"));
