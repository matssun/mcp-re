...
//! `mcps-proxy` replicas each run the ADR-021 Push tier (a bounded trust cache +
...
//! expiry.
//!
//! redis_replay-gated; skipped when `MCPS_TEST_REDIS_URL` is unset (hard-failed
//! under `MCPS_REQUIRE_LIVE_INFRA`).
...
use mcps_proxy::redis_trust_epoch_source;
use mcps_proxy::trust_cache::UnixClock;
use mcps_proxy::Proxy;
use mcps_proxy::PushInvalidationTrustCache;
use serde_json::json;
...
fn revocation_takes_effect_on_a_sibling_replica_when_the_epoch_advances() {
    let Some(url) = redis_url() else {
        eprintln!("SKIP revocation_takes_effect_on_a_sibling_replica_when_the_epoch_advances: MCPS_TEST_REDIS_URL unset");
        return;
    };
