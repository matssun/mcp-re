//! MCPS-063 (MCPS-EPIC-P6.6B) — a LONG-LIVED inner MCP server.
//!
...
//! real MCP server (the `mcps-demo-server`, #3956) is long-lived and cannot be
//! fronted by the one-shot path.
//!
//! ## Framing (matches mcps-demo-server, #3956 / #3957)
...
/// Matches the version `mcps-demo-server` advertises (#3956).
    ...
    fn initialize(&mut self) -> Result<(), String> {
        ...
        let request = json!({
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": { "name": "mcps-proxy", "version": env!("CARGO_PKG_VERSION") }
            }
        });
