...
//!
//! The unit tests in [`super::auth_epoch`] prove the store's contract on synthetic values.
...
//!
//! # Why it lives inside the owner's module tree (MCPRE-137)
//!
//! It was an integration test, reaching `EpochBoundSessionStore` and `SharedTlsAuthEpoch`
//! through the crate's public surface. Keeping it there would have meant keeping those
//! subordinates `pub` — a production interface widened by a test, which is the shape
//! ADR-061 §8 question 8 names. So the TEST moved inside the privacy boundary instead.
//!
...
// `scripts/module_size_gate.py` reads: without it the whole file counts as production
// lines, which is a measurement error, not architecture.
#[cfg(test)]
mod handshakes {
    use super::super::auth_epoch::EpochBoundSessionStore;
    use super::super::auth_epoch::SharedTlsAuthEpoch;
    use super::super::auth_epoch::TlsAuthEpoch;
    use std::sync::Arc;
