...
//! shared atomic cache (e.g. Redis) behind the same `mcp_re_core::ReplayCache`
...
//! (→ `mcp-re.replay_cache_unavailable`, fail closed) and the in-memory insert is
...
/// [`ReplayCacheError::Unavailable`] (→ `mcp-re.replay_cache_unavailable`, fail
...
mod tests {
    ...
    fn tmp(name: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("mcp_re_replay_{}_{name}", std::process::id()))
    }
