...
/// and its fail-closed checks — be proven against the real `mcp-re-core` verifier
...
mod tests {
    ...
    /// protocol mapping is provable against the real `mcp-re-core` verifier with no
    ...
    /// public key the signer reports, using the UNMODIFIED `mcp-re-core` Ed25519
    ...
    #[test]
    fn kms_signature_verifies_under_mcp_re_core_verifier() {
    ...
    fn mismatched_key_signature_fails_closed_at_the_seam() {
        ...
        assert!(
            matches!(
                signer.sign_response(b"mcp-re canonical response preimage"),
                Err(KeyError::Malformed(_))
            ),
