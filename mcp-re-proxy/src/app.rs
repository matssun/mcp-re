...
fn run_validated(
    ...
    // (ADR-MCPRE-052), so signing owns the invariant that makes it valid and trust only
    // consumes it: the issuer key is never enrolled as a request signer, and it answers
    // the Response slot.
