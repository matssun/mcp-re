...
impl InnerLaunchConfig {
    ...
    pub fn default_working_dir() -> String {
        std::env::temp_dir()
            .join(format!("mcp-re-inner-{}", std::process::id()))
            .to_string_lossy()
            .into_owned()
    }
    ...
    fn ensure_private_default_working_dir() -> Result<String, String> {
        ...
        let path = std::env::temp_dir().join(format!("mcp-re-inner-{}", std::process::id()));
...
mod tests {
    ...
    fn allowlist_error_names_only_the_var_not_its_value() {
        ...
        let err = config
            .resolve_allowlist(&env_view(&[("MCP_RE_SIGNING_SEED", "TOP-SECRET-SEED-VALUE")]))
