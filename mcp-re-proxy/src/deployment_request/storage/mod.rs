...
//!
//! Four semantic roles keep shared state: replay admits a nonce once, continuation retains
//! a base across replicas, admission holds the authoritative record a revocation is
//! written to, and the trust epoch carries the operator's invalidation signal. **They are
//! four propositions, not one.** That three of them are usually served by the same Redis
...
//!                                 AdmissionStoreRequest / TrustEpochStoreRequest
//!         ↓
...
pub use admission_storage::AdmissionStoreRequest;
