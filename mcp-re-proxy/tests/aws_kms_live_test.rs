...
//!
//! This is the lane that lets MCP-RE CLAIM AWS KMS support: a signature produced by
//! a REAL KMS `Sign` (against a real AWS endpoint or a LocalStack/internal-platform
//! emulator) MUST verify under the UNMODIFIED `mcp-re-core` Ed25519 verifier, using
...
#[test]
#[ignore = "requires a live or emulated AWS KMS (run with --ignored and MCP_RE_AWS_KMS_* + AWS creds set)"]
fn aws_kms_signature_verifies_under_mcp_re_core() {
    ...
    let preimage = b"mcp-re canonical response preimage (live KMS lane)";
    let sig = signer.sign_response(preimage).expect("KMS Sign");
    let pubkey = signer.response_public_key().expect("KMS public key");

    // THE load-bearing assertion: real KMS signature verifies under mcp-re-core.
    verify_ed25519(preimage, &sig, &pubkey)
        .expect("a live KMS Ed25519 signature MUST verify under the mcp-re-core verifier");
