//! Live AWS KMS DELEGATED-TLS lane (ADR-MCPS-028 §G, issue #61).
//!
//! The enterprise claim this lane proves: the proxy's TLS *server* private key can
//! live entirely inside AWS KMS and NEVER leave it, yet a real, fully
...
//! Ed25519 public key, the handshake completes ONLY if a live AWS KMS
//! `Sign` produced a wire-correct signature over the TLS transcript.
//! Nothing is bypassed — this is genuine proof, not a mock.
//!
//! What is in the cloud vs local (KMS is necessary, not sufficient):
//!   * IN AWS KMS: the TLS server private key (a SECOND, DISTINCT Ed25519 key from
//!     the response-signing key — ADR §G). Exercised only via `Sign` /
...
//! run with `--features aws_kms_keysource -- --ignored`.
//!
//! Required environment:
//!   * `MCP_RE_AWS_KMS_TLS_KEY_ID` — an `ECC_NIST_EDWARDS25519` key id/ARN/alias for
//!     the TLS server key (a SECOND, DISTINCT key from the object-signing one).
...
//!   * `MCP_RE_AWS_KMS_ENDPOINT`   — OPTIONAL endpoint override (emulator).
...
/// real/emulated AWS KMS; it never passes without actually verifying.
fn require_env(name: &str) -> String {
    match std::env::var(name) {
        Ok(v) if !v.is_empty() => v,
        _ => panic!(
            "aws-kms delegated-TLS lane: required env var {name} is not set — this lane \
             must run against a real/emulated AWS KMS; it does not pass without verifying"
        ),
    }
}
...
// ---------------------------------------------------------------------------
// The crux: mint a server leaf over the AWS KMS key's PUBLIC key, no private key.
// ---------------------------------------------------------------------------

/// An rcgen subject key backed by a AWS KMS key: it exposes the cloud key's PUBLIC
/// point so a CA-signed leaf can BIND it, and delegates any signing to the cloud.
/// The private key never leaves AWS — `sign()` (unused for CA-signed leaf issuance)
/// would call `Sign`, never a local key.
struct AwsRemoteKey {
    raw_public: Vec<u8>,
    signer: Arc<AwsKmsEd25519Backend>,
}

impl PublicKeyData for AwsRemoteKey {
...
impl SigningKey for AwsRemoteKey {
    fn sign(&self, msg: &[u8]) -> Result<Vec<u8>, rcgen::Error> {
        // Honest delegation: if rcgen ever needs this key to sign, it goes to
        // AWS KMS — never a local key. (Not invoked for a CA-signed leaf, where
...
fn raw_point_from_spki(spki: &[u8]) -> [u8; 32] {
    ...
    assert!(
        ...
        "expected an RFC 8410 Ed25519 SPKI (12-byte prefix + 32-byte point) from AWS KMS, got {} bytes",
        spki.len()
    );
    spki[12..44].try_into().expect("32-byte ed25519 point")
}

/// Construct the live AWS KMS TLS backend from env, plus its (cached) public point.
/// Shared by the positive and negative lanes so they all custody the TLS key in AWS.
fn aws_tls_backend() -> (Arc<AwsKmsEd25519Backend>, [u8; 32]) {
    let tls_config = AwsKmsConfig {
        region: require_env("MCP_RE_AWS_KMS_REGION"),
        key_id: require_env("MCP_RE_AWS_KMS_TLS_KEY_ID"),
        endpoint: std::env::var("MCP_RE_AWS_KMS_ENDPOINT")
            .ok()
            .filter(|s| !s.is_empty()),
    };
    // Either custody path reaches the same KMS key; on EKS the runner takes IRSA,
    // so the handshake below is signed by a pod holding no IAM key material.
    let backend = Arc::new(
        if std::env::var("MCP_RE_AWS_USE_WEB_IDENTITY").is_ok_and(|v| v == "1") {
            ...
            .expect("construct AWS KMS TLS backend through IRSA (GetPublicKey must be Ed25519)")
        } else {
            require_env("AWS_ACCESS_KEY_ID");
            AwsKmsEd25519Backend::from_env(&tls_config)
                .expect("construct AWS KMS TLS backend (GetPublicKey must succeed and be Ed25519)")
        },
    );
    let spki = backend
        .tls_public_key_spki_der()
        .expect("AWS KMS GetPublicKey (TLS)");
    let raw_public = raw_point_from_spki(&spki);
    (backend, raw_public)
}

/// Mint a CA-signed Ed25519 server leaf whose SUBJECT public key is the AWS KMS
/// key's public key. The KMS private key is not used (the CA signs the leaf).
fn make_server_leaf_for_aws_key(
    ca: &Ca,
    backend: Arc<AwsKmsEd25519Backend>,
    raw_public: [u8; 32],
) -> CertificateDer<'static> {
    let remote = AwsRemoteKey {
    ...
    let cert = params
        ...
        .expect("server leaf signed by CA over the AWS public key");
    cert.der().clone()
}
...
fn aws_kms_delegated_tls_handshake_round_trip() {
    // The TLS server key lives in AWS KMS. We only ever fetch its PUBLIC key.
    let (tls_backend, raw_public) = aws_tls_backend();
    ...
    // Fully validating client: completing this handshake means a live AWS KMS
    // Sign produced a valid Ed25519 CertificateVerify over the transcript.
    let response = client_round_trip(
        addr,
        client_config_validating(server_ca.cert.der().clone(), (client_chain, client_key)),
        b"{\"jsonrpc\":\"2.0\"}",
    )
    .expect("client round trip over a AWS-KMS-delegated TLS handshake");
...
#[test]
#[ignore = "requires a live or emulated AWS KMS (run with --ignored and MCP_RE_AWS_KMS_* set)"]
fn aws_kms_delegated_tls_wrong_key_binding_fails_closed() {
    let (tls_backend, _raw_public) = aws_tls_backend();
    let server_ca = make_ca();
    let client_ca = make_ca();
...
/// server is the real AWS-KMS-delegated proxy config; the client presents a cert
/// from a CA the proxy does not trust, so client-auth fails closed.
#[test]
#[ignore = "requires a live or emulated AWS KMS (run with --ignored and MCP_RE_AWS_KMS_* set)"]
fn aws_kms_delegated_tls_untrusted_client_rejected() {
    let (tls_backend, raw_public) = aws_tls_backend();
    let server_ca = make_ca();
