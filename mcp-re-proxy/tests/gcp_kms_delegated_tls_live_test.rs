...
//!   * `MCP_RE_GCP_KEY_VERSION_TLS` — full resource path of the Ed25519 TLS key
//!     version (`.../cryptoKeyVersions/V`, algorithm `EC_SIGN_ED25519`).
//!   * one of: `MCP_RE_GCP_ACCESS_TOKEN` (operator bearer token), or
//!     `MCP_RE_GCP_USE_METADATA=1` for the workload-identity metadata server.
...
#[test]
#[ignore = "requires a live or emulated GCP Cloud KMS Ed25519 TLS key (run with --ignored and MCP_RE_GCP_* set)"]
