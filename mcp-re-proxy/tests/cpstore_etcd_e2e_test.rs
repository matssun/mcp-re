...
/// `MCP_RE_REQUIRE_LIVE_INFRA` so CI cannot score an unavailable backend as a green
/// skip.
fn etcd_url() -> Option<String> {
    let url = std::env::var("MCP_RE_TEST_ETCD_URL")
        .ok()
...
fn cross_node_insert_via_a_is_replay_via_b() {
    let Some(url) = etcd_url() else {
        eprintln!(
            "SKIP cross_node_insert_via_a_is_replay_via_b: MCP_RE_TEST_ETCD_URL unset \
...
fn single_node_fresh_then_replay() {
    let Some(url) = etcd_url() else {
        eprintln!(
            "SKIP single_node_fresh_then_replay: MCP_RE_TEST_ETCD_URL unset \
...
fn live_lease_ttl_is_bounded_window_not_absolute_epoch() {
    let Some(url) = etcd_url() else {
        eprintln!(
            "SKIP live_lease_ttl_is_bounded_window_not_absolute_epoch: MCP_RE_TEST_ETCD_URL \
