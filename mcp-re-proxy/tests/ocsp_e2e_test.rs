...
//!
//! # Environment gating
//! The test runs ONLY when `MCP_RE_TEST_OCSP_RESPONDER_URL` is set; otherwise it
...
//! reads:
//!   * `MCP_RE_TEST_OCSP_RESPONDER_URL` — the responder URL, e.g.
//!     `http://127.0.0.1:8888`.
//!   * `MCP_RE_TEST_OCSP_ISSUER_DER`    — path to the issuing CA certificate (DER).
//!   * `MCP_RE_TEST_OCSP_GOOD_DER`      — path to a known-GOOD leaf cert (DER),
//!     issued by that CA and NOT revoked in the responder's index.
//!   * `MCP_RE_TEST_OCSP_REVOKED_DER`   — path to a known-REVOKED leaf cert (DER),
...
//!     -subj "/CN=mcp-re-ocsp-test-ca" -days 2 -addext "keyUsage=keyCertSign,cRLSign"
...
//! export MCP_RE_TEST_OCSP_RESPONDER_URL=http://127.0.0.1:8888
//! export MCP_RE_TEST_OCSP_ISSUER_DER=$PWD/ca.der
//! export MCP_RE_TEST_OCSP_GOOD_DER=$PWD/good.der
...
fn ocsp_env() -> Option<(String, String, String, String)> {
    let Ok(url) = std::env::var("MCP_RE_TEST_OCSP_RESPONDER_URL") else {
        if std::env::var("MCP_RE_REQUIRE_LIVE_INFRA").is_ok_and(|v| !v.is_empty()) {
            panic!(
                "MCP_RE_REQUIRE_LIVE_INFRA is set but MCP_RE_TEST_OCSP_RESPONDER_URL is unavailable \
    ...
    let Some((url, issuer_path, good_path, revoked_path)) = ocsp_env() else {
        eprintln!(
            "SKIP live_responder_reports_good_and_revoked_without_restart: \
             MCP_RE_TEST_OCSP_RESPONDER_URL is unset (no OCSP responder provisioned). \
