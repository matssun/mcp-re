# MCP-S conformance vectors (MCPS-002)

These JSON files are the **executable specification** for the MCP-S security
profile. They are regenerated against the **frozen** wire vocabulary (MCPS_SPEC
...
- `expected`          — `verify_ok` or an exact `mcps.*` error token (MCPS_SPEC §8).
...
in `mcps-conformance/conformance_manifest.json` — the single
source of truth, drift-guarded by
`//mcps-conformance:drift_guard_test` (MCPS-031). Do not quote a
...
```
cargo test -p mcps-core --test vectors_test write_fixtures -- --ignored --exact
```
