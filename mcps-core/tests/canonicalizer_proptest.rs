...
//! canonicalizer — the most security-critical unit in MCP-S, since the entire
...
use mcps_core::canonicalize_value;
use mcps_core::JcsValue;
...
fn mutate_semantics_changing(v: &JcsValue) -> JcsValue {
    match v {
        ...
        JcsValue::Object(members) => {
            ...
            let mut fresh = String::from("__mcps_fresh__");
...
mod seed_corpus {
    ...
    fn canon(input: &str) -> Result<Vec<u8>, McpsError> {
    ...
    fn duplicate_keys_rejected_on_raw_path() {
        assert_eq!(canon(r#"{"a":1,"a":2}"#).unwrap_err(), McpsError::CanonicalizationFailed);
        assert_eq!(
    ...
    fn surrogate_pairs() {
        ...
        assert_eq!(canon(r#""\uD800""#).unwrap_err(), McpsError::CanonicalizationFailed);
        assert_eq!(canon(r#""\uDC00""#).unwrap_err(), McpsError::CanonicalizationFailed);
    }
    ...
    fn safe_integer_boundary() {
        ...
        assert_eq!(canon_str("-9007199254740991"), "-9007199254740991");
        assert_eq!(canon("9007199254740992").unwrap_err(), McpsError::CanonicalizationFailed);
        assert_eq!(canon("-9007199254740992").unwrap_err(), McpsError::CanonicalizationFailed);
    }
