//! Base64URL (no padding) encoding helpers (MCPS_SPEC §3).
//!
//! All MCP-S signature and hash values are Base64URL WITHOUT padding. These two
...
//! [`McpsError::CanonicalizationFailed`] as a neutral, structural decode error.
...
//! [`McpsError::InvalidSignature`] in [`crate::crypto`]).
...
///
/// A malformed input maps to [`McpsError::CanonicalizationFailed`] (a neutral
/// structural failure). Security-critical callers should instead decode with
/// their own mapping — see the module docs.
pub fn b64url_decode(s: &str) -> Result<Vec<u8>, McpsError> {
    URL_SAFE_NO_PAD
        .decode(s)
...
mod tests {
    ...
    fn decode_rejects_padding() {
        ...
        assert_eq!(b64url_decode("aGk=").unwrap_err(), McpsError::CanonicalizationFailed);
    }

    #[test]
    fn decode_rejects_non_alphabet() {
        // '*' is not in the URL-safe alphabet.
        assert_eq!(b64url_decode("ab*c").unwrap_err(), McpsError::CanonicalizationFailed);
    }
