//! RFC 3339 UTC timestamp parsing and freshness checking (MCPS_SPEC §5 / ADR-006,
...
//!
//! A malformed timestamp maps to [`McpsError::ExpiredRequest`]. Rationale: if a
...
/// value maps to [`McpsError::ExpiredRequest`] (fail closed — see module docs).
///
/// # Examples
///
/// `1970-01-01T00:00:00Z` parses to `0`.
pub fn parse_rfc3339_utc(s: &str) -> Result<i64, McpsError> {
    ...
    if bytes.len() < 20 {
        return Err(McpsError::ExpiredRequest);
    }
    ...
    let (year, month, day, hour, minute, second) = match (year, month, day, hour, minute, second) {
        ...
        _ => return Err(McpsError::ExpiredRequest),
    };
...
/// returns:
///
/// - [`McpsError::ExpiredRequest`] if either timestamp is malformed (delegated
///   to [`parse_rfc3339_utc`]'s fail-closed mapping),
/// - [`McpsError::ExpiredRequest`] if `expires_at < issued_at` (a nonsensical
///   window),
/// - [`McpsError::ExpiredRequest`] if `now_unix < issued_at − skew`
...
/// Format Unix seconds (UTC) as the strict RFC 3339 form MCP-S uses
...
mod tests {
    ...
    fn garbage_and_wrong_separators_rejected() {
        assert_eq!(parse_rfc3339_utc(""), Err(McpsError::ExpiredRequest));
        assert_eq!(parse_rfc3339_utc("not-a-date"), Err(McpsError::ExpiredRequest));
        assert_eq!(
