...
/// The incubation extension identifier (ADR-MCPS-010 / MCPS_SPEC §1). It appears
/// inside the signed `_meta` keys and is therefore part of the canonical
/// preimage: it may change between draft versions during incubation and freezes
/// at 1.0. Controlled, explicitly NON-official.
pub const EXTENSION_ID: &str = "name.sundvall/mcps-security";

/// `_meta` key under which the request envelope is carried.
pub const REQUEST_META_KEY: &str = "name.sundvall/mcps-security.request";

/// `_meta` key under which the response envelope is carried.
pub const RESPONSE_META_KEY: &str = "name.sundvall/mcps-security.response";

/// `_meta` key under which the (unsigned, local) verified-context sidecar block
/// is carried. Never part of any signed preimage.
pub const VERIFIED_META_KEY: &str = "name.sundvall/mcps-security.verified";
...
mod tests {
    ...
    fn extension_id_is_the_incubation_identifier() {
        assert_eq!(EXTENSION_ID, "name.sundvall/mcps-security");
    }

    #[test]
    fn meta_keys_are_namespaced_under_the_extension_id() {
        assert_eq!(REQUEST_META_KEY, "name.sundvall/mcps-security.request");
        assert_eq!(RESPONSE_META_KEY, "name.sundvall/mcps-security.response");
        assert_eq!(VERIFIED_META_KEY, "name.sundvall/mcps-security.verified");
