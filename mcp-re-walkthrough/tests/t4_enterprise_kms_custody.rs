...
//!
//! ```sh
//! cargo build -p mcp-re-client-proxy-cli --features gcp_kms
//! cargo build -p mcp-re-proxy            --features gcp_kms_keysource
//! MCP_RE_GCP_ACCESS_TOKEN=… \
//!   MCP_RE_GCP_KEY_VERSION=<server key version> \
//!   MCP_RE_GCP_KEY_VERSION_CLIENT=<client key version> \
...
fn both_identities_in_cloud_kms_round_trip_the_four_hop() {
    ...
    // Transparency: the ordinary client never sees an MCP-RE envelope — the
