...
//! identity (URI SAN) to EQUAL the request signer. Message-level MCP-RE still does
...
//! pinned by the in-process `mcp-re-proxy` suite; here we prove the OUTCOME — denied
...
use mcp_re_walkthrough::ClientCert;
use mcp_re_walkthrough::FourHop;
use mcp_re_walkthrough::FourHopOptions;
use mcp_re_walkthrough::TransportBinding;
...
impl LogPath {
    fn new() -> Self {
        ...
        let dir = base.join(format!("mcp-re-walkthrough-t3-log-{}-{n}", std::process::id()));
...
fn the_same_mismatched_cert_passes_with_binding_off() {
    ...
    assert_eq!(
        ...
        Some(mcp_re_walkthrough::SEED_TEXT),
...
fn a_wrong_expected_server_name_fails_closed_with_no_inner_data() {
    ...
    assert!(
        !resp.to_string().contains(mcp_re_walkthrough::SEED_TEXT.trim()),
