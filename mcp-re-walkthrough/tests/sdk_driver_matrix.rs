...
//!
//! Every MCP-RE SDK is an INTERCHANGEABLE client: it signs requests and verifies
...
//!
//! The Rust reference driver (`mcp-re-client-proxy-cli`) is always present; each
...
use mcp_re_walkthrough::tool_call_continuation;
...
fn every_configured_sdk_driver_round_trips_a_signed_call() {
    ...
    for driver in &drivers {
        ...
        // sits on the client leg — and the plain client sees no MCP-RE envelope.
        ...
        assert!(
            ...
            "driver '{}' leaked an MCP-RE envelope to the plain client: {response}",
            driver.label
        );
        assert!(
...
/// one new MCP-RE evidence class end to end through EVERY interchangeable client leg:
///
/// ```text
/// signed request
...
fn every_configured_sdk_driver_completes_an_mrt_continuation() {
    ...
    for driver in &drivers {
        ...
        // elicitation as plain MCP with NO MCP-RE envelope leaked.
        ...
        assert!(
            ...
            "driver '{}' leaked an MCP-RE envelope on the elicitation: {elicit}",
            driver.label
        );
        ...
        assert!(
            ...
            "driver '{}' leaked an MCP-RE envelope on the terminal: {terminal}",
            driver.label
        );
    }
}
