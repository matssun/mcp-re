//! The MCP-RE persona-ladder walkthrough harness (ADR-MCPS-045).
//!
...
//!     ▼
//!   mcp-re-demo-fileserver    ── ordinary, MCP-RE-unaware stdio MCP server
//! ```
//!
//! The local client speaks ONLY plain MCP; all signing/verification lives in the
//! two proxies. The transport is mTLS-on-loopback throughout — MCP-RE's guarantee
...
//! drop. The proxy + fileserver binaries are resolved via `mcp-re-test-paths`
...
pub enum TransportBinding {
    ...
    /// mTLS client identity (URI SAN), else `mcp-re.transport_binding_failed`.
    Exact,
}
...
/// against ANY MCP-RE SDK (Rust today; Python/TypeScript upcoming), not just the
/// Rust reference proxy.
///
/// Each driver is a subprocess honoring ONE stdio contract: plain MCP JSON-RPC in
/// on stdin, one plain MCP JSON-RPC response out per line on stdout, with MCP-RE
...
/// `mcp-re-client-proxy-cli` is the reference implementation of that contract; an SDK
...
pub struct ClientDriver {
    ...
    /// `["python3", "-m", "mcp_re_sdk.driver"]`). The shared contract args are
    /// appended by the harness.
    pub command: Vec<String>,
}

impl ClientDriver {
    /// The Rust reference driver: `mcp-re-client-proxy-cli`, resolved via
    /// `mcp-re-test-paths` (Bazel runfiles OR cargo target).
    pub fn rust() -> Self {
        ClientDriver {
            label: "rust".to_string(),
            command: vec![resolve_bin("MCP_RE_CLIENT_PROXY_CLI")],
        }
    }
    ...
    ///
    /// Recognized keys: `MCP_RE_DRIVER_PYTHON`, `MCP_RE_DRIVER_TS`.
    pub fn available() -> Vec<ClientDriver> {
        let mut drivers = vec![ClientDriver::rust()];
        for (label, key) in [
            ("python", "MCP_RE_DRIVER_PYTHON"),
            ("typescript", "MCP_RE_DRIVER_TS"),
        ] {
...
/// `MCP_RE_GCP_ACCESS_TOKEN` or, with `MCP_RE_GCP_USE_METADATA=1`, the metadata server.
#[cfg(feature = "gcp_kms")]
fn fetch_kms_pubkey_b64url(key_version: &str) -> String {
    use mcp_re_proxy::kms_keysource::KmsEd25519Backend;
    ...
    mcp_re_core::VerificationKey::from_bytes(&raw)
...
impl TempDir {
    ...
    fn new(tag: &str) -> std::io::Result<Self> {
        ...
        let path = base.join(format!("mcp-re-walkthrough-{tag}-{}-{n}", std::process::id()));
...
impl FourHop {
    ...
    pub fn launch_with(opts: FourHopOptions) -> Self {
        ...
        if opts.tamper_server_pubkey {
            ...
            profile.client_server_pubkey = mcp_re_core::SigningKey::from_seed_bytes(&fixtures.signer_seed())
                .public_key()
                .to_b64url();
        }
    ...
    /// response it yields (the local client never sees an MCP-RE field).
...
/// Resolve a child binary via `mcp-re-test-paths` (Bazel runfiles OR cargo target).
...
/// the plain client is unaware of the MCP-RE binding.
...
/// Extract the `error.message` (the surfaced `mcp-re.*` wire reason) from a
