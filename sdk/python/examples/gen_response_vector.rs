...
//! runs the real `mcp-re-client-core` return-leg chain (verify → classify → decide →
...
fn scenario(
    ...
    let mode = match mode_str {
        "require_mcp_re" => EnforcementMode::RequireMcpRe,
...
fn main() {
    ...
    // unsigned: a plain MCP response with no MCP-RE envelope.
    let unsigned = json!({
    ...
    let scenarios = json!([
        ...
        scenario(
            ...
            &resolver,
            &rh,
            Some("mcp-re-jcs-int53-json-v9-mismatch"),
            None,
            "require_mcp_re",
            false
        ),
    ...
    let fixture = json!({
        "_comment": "Golden vectors for the Python verify_response binding. Generated by \
                     `cargo run --example gen_response_vector` from mcp-re-client-core (independent \
