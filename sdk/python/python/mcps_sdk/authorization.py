...
a later verifier can tie the request to the authorization artifact, without MCP-S
...
These providers mirror ``mcps-client-core::authz`` and delegate digest computation
to the audited core (``mcps_sdk.AuthorizationBinding``), so the binding is produced
...
``McpsConfig.authorization_policy`` (fails closed on a disallowed binding type).
"""
...
class OpaqueBytesProvider:
    ...
    (``mcps_sdk.AuthorizationBinding.opaque_bytes``), never by this layer.
    """
    ...
    def provide(self, ctx: BindingRequestContext) -> Any:
        ...
        return mcps_sdk.AuthorizationBinding.opaque_bytes(bytes(data))
...
class AuthzSystemReferenceProvider:
    ...
    def provide(self, ctx: BindingRequestContext) -> Any:
        if self._resolver is None:
            ...
            raise ValueError("mcps.authorization_binding_missing")
        ref = self._resolver(ctx)
        return mcps_sdk.AuthorizationBinding.authz_system_reference(
...
class StaticAuthorizationProvider:
    """Wrap one prebuilt ``mcps_sdk.AuthorizationBinding`` (reused across requests)."""
