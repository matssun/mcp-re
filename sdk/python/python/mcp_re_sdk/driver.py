"""MCP-RE conformance driver — the Python SDK as an interchangeable client leg.

This is the Python side of the multi-SDK test architecture (see
``mcp-re-walkthrough`` `ClientDriver`). It is a thin stdio bridge that makes the
Python SDK a drop-in for the Rust reference ``mcp-re-client-proxy-cli``: it reads one
plain MCP JSON-RPC request per line on stdin, signs it with the SDK, POSTs it over
mTLS to the ``mcp-re-proxy`` PEP, verifies the server-signed response, strips the
MCP-RE envelope, and writes one plain MCP JSON-RPC response per line on stdout.

The signing/verification is the AUDITED ``mcp-re-client-core`` logic via the SDK's
...
# JSON-RPC server-error code carrying a fail-closed MCP-RE rejection back to the
# harness (matches the SDK's McpReHttpTransport reject code).
_MCP_RE_REJECTED_CODE = -32099
...
def _canonical_audience(six_field: str) -> str:
    """Reproduce ``mcp_re_client_core::AudienceTuple::to_audience_string`` from the
    6-field ``--audience`` form (``scheme,host,port,tenant,route,realm``). Mirrors
    ``mcp-re-client-core/src/audience.rs``; a drift makes the round trip fail closed
...
def _gcp_access_token(use_metadata: bool) -> str:
    ...
    metadata``) or ``MCP_RE_GCP_ACCESS_TOKEN`` — mirroring the Rust backend's sources."""
    if use_metadata:
    ...
    token = os.environ.get("MCP_RE_GCP_ACCESS_TOKEN", "")
    if not token:
        raise SystemExit(
            "MCP_RE_GCP_ACCESS_TOKEN must be set for --key-source gcp-kms "
...
def _build_signer(args: argparse.Namespace):
    ...
    policy = mcp_re_sdk.SignerPolicy(args.signer_id, environment="dev-test", require_mcp_re=True)
    return signer, policy
...
def _strip_envelope(obj: dict) -> dict:
    """Remove the MCP-RE response envelope from ``result._meta`` so the harness sees
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="mcp_re_sdk.driver", add_help=False)
...
def main(argv: list[str] | None = None) -> int:
    ...
    while True:
        ...
        if method is None:
            ...
            emit(_reject(rid, "mcp-re.missing_envelope"))
            continue
        ...
        if request_state is not None:
            ...
            if entry is None:
                emit(_reject(rid, "mcp-re.continuation_malformed"))
                continue
            continuation_kwargs = {
        ...
        try:
            ...
            emit(_reject(rid, f"mcp-re.driver_error: {exc}"))
            continue
...
def _reject(rid, reason) -> dict:
    return {
        "jsonrpc": "2.0",
        "id": rid,
        "error": {"code": _MCP_RE_REJECTED_CODE, "message": reason or "mcp-re.verification_failed"},
    }
