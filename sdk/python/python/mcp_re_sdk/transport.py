...
verifies every inbound response against the audited ``mcp-re-client-core`` bindings.
...
class McpReConfig:
    ...
    signer: Any  # mcp_re_sdk.Signer
    policy: Any  # mcp_re_sdk.SignerPolicy
    resolver: Any  # mcp_re_sdk.TrustResolver
    audience: str
    on_behalf_of: str
    ...
    # ``mcp_re_sdk.AuthorizationBinding`` — the digest is then computed in the audited
    # core from the real artifact bytes, not supplied as a constant.
    # ``authorization_policy`` (an ``mcp_re_sdk.AuthorizationBindingPolicy``) fails a
    ...
    enforcement_mode: str = "require_mcp_re"
    legacy_allowed: bool = False
    ttl_seconds: int = 300
    route_id: str = "default"
    # Inbound policy for SERVER-INITIATED messages (a server->client request or
    # notification — NOT a response to one of our signed requests). The MCP-RE
    # evidence model binds a server's signature to the client's `request_hash`; a
    # server-initiated message has none, so `mcp-re-client-core` cannot verify it.
    # STRICT MCP-RE is the client-initiated request/response subset (to be extended
    ...
    # NO-EVIDENCE. It is NOT strict enterprise MCP-RE and must never be presented as
...
def _binding_kwargs(config: McpReConfig, method: str, params: Any, deadline_unix: int) -> dict:
    ...
    binding) under a real :class:`~mcp_re_sdk.authorization.BindingRequestContext`, so
    ...
    if hasattr(provider, "provide"):
        ...
        binding = provider  # a prebuilt mcp_re_sdk.AuthorizationBinding
    ...
    *,
    ...
    if request_state is not None:
        ...
        if entry.route_id != config.route_id or entry.audience != config.audience:
            raise ValueError(
                "mcp-re.continuation_malformed: continuation route/audience does not match "
...
class InboundOutcome:
    ...
    reason: Optional[str] = None  # the mcp-re.* wire reason on reject
...
def verify_inbound(
    ...
    *,
    ...
    verified; on accept the MCP-RE envelope is stripped and a plain SessionMessage is
    ...
    default (``mcp-re.missing_envelope`` for an id-bearing server request,
    ``mcp-re.notification_forbidden`` for a notification — both in the frozen
    ...
    if has_method:
        ...
        reason = "mcp-re.missing_envelope" if rid is not None else "mcp-re.notification_forbidden"
    ...
    if rid is None:
        ...
        return InboundOutcome("reject", reason="mcp-re.missing_envelope")
    ...
    try:
        ...
        # mcp-re.* wire code so reject reasons are consistent with the verify path.
