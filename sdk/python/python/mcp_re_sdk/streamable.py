...
:func:`~mcp_re_sdk.transport.verify_inbound` — so the correlated-response verification
AND the server-initiated inbound policy (fail-closed by default; see
``McpReConfig.allow_unverified_server_initiated``) apply uniformly at all three sites.
...
from .transport import InboundOutcome, McpReConfig, verify_inbound
...
def sse_data_events(raw: bytes) -> List[bytes]:
    ...
    try:
        ...
        raise ValueError("invalid UTF-8 in text/event-stream body") from exc
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    events: List[bytes] = []
    data_lines: List[str] = []
...
def verify_inbound_messages(
    ...
    *,
    ...
    :func:`~mcp_re_sdk.transport.verify_inbound`, so a correlated response is
    ...
    for payload in payloads:
        ...
        try:
            ...
            outcomes.append(InboundOutcome("reject", reason="mcp-re.missing_envelope"))
    return outcomes
