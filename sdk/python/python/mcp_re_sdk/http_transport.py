"""MCP-RE request/response transport — one signed POST per ``ClientSession`` request.

This is step (ii): a dedicated transport that maps ``mcp.ClientSession``'s
persistent-stream model onto the production ``mcp-re-proxy``'s wire, which is **one
...
MCP_RE_REJECTED_CODE = -32099


class McpReHttpTransport:
    ...
    def __init__(
        ...
        *,
        ...
        # later request. Without it, an answer leg raises `mcp-re.continuation_malformed`.
        self._mrt: dict = {}
        self._tg = None
    ...
    async def _round_trip(self, session_message: Any, rid: Any) -> None:
        ...
        try:
            try:
                ...
                await self._app_read_send.send(self._reject_message(rid, "mcp-re.missing_envelope"))
                return
    ...
    def _reject_message(rid: Any, reason: Optional[str]) -> Any:
        ...
        error = JSONRPCError(
            jsonrpc="2.0",
            id=rid,
            error=ErrorData(code=MCP_RE_REJECTED_CODE, message=reason or "mcp-re.verification_failed"),
        )
