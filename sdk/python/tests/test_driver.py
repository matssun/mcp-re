...
(`mcps-walkthrough` `sdk_driver_matrix`); these tests guard the pieces that have no
server in the loop and could silently drift — chiefly the audience canonicalization
that must reproduce `mcps_client_core::AudienceTuple::to_audience_string` exactly.
"""

import base64
import json

from mcps_sdk import driver
...
def test_canonical_audience_mirrors_rust_audience_tuple():
    ...
    # AudienceTuple produces (mcps-client-core/src/audience.rs).
    out = driver._canonical_audience("https,proxy.internal,8443,acme,tools,prod")
    assert out == (
        "mcps-audience:v1:scheme=https;host=proxy.internal;port=8443;"
...
def test_strip_envelope_removes_only_the_mcps_key_and_empties_meta():
    import mcps_sdk

    key = mcps_sdk.response_meta_key()
...
def test_reject_shape_is_a_correlated_json_rpc_error():
    err = driver._reject("req-9", "mcps.downgrade_forbidden")
    assert err["id"] == "req-9"
    assert err["error"]["message"] == "mcps.downgrade_forbidden"
    # One compact line, parseable by the harness.
    assert json.loads(json.dumps(err))["error"]["code"] == driver._MCPS_REJECTED_CODE
