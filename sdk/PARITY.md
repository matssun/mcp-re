...
| A client->server response | `ClientResponseUnsupported` reported to `McpReConfig.on_undeliverable`; the session continues | `ClientResponseUnsupported` thrown from `send()` | Same refusal, same reason: MCP-RE profiles a signed request and a signed notification, and a response is neither. Python's pump loop is the parent task of the task group running every concurrent exchange, so raising there would cancel every unrelated in-flight signed call — and the trigger is peer-influenceable, since a verified reply body carrying a `method` parses as a server->client request that `ClientSession` answers. The refusal is identical; only its blast radius differs. |
| An unverified notification acknowledgement surfaces as | `McpReConfig.on_undeliverable`, falling back to the module-level `on_notification_failure` (stderr); the session continues | `send()` rejects | Python's transport is a stream pair with no per-message reply channel; TypeScript's `Transport.send` is a method call that can reject. Both fail closed; both are visible; neither treats the message as delivered. The per-transport hook is what makes this safe: two transports in one process cannot swallow each other's failures, and one unverifiable 202 — for a routine `notifications/initialized` from a proxy whose delegated key is past `exp` — cannot tear down a session the peer does not own. |
