...
> | Transport adapter (`McpReHttpTransport`) — a real `Client` signs/verifies by construction | **done** (request/response only) |
...
serializing them. The bound (`maxConcurrentExchanges`, default 8) exists because each
in-flight exchange holds a connection in your `poster` and a signing operation (a KMS
round trip under non-exporting custody).

**Notifications are unsupported — this is a request/response adapter.** MCP-RE's evidence
rides on the response; a one-way `notifications/*` message has none, so there is nothing
to bind a signature to and nothing to verify. The adapter therefore **drops every
client→server notification**, including `notifications/initialized` and
`notifications/cancelled`. Install `onDroppedNotification` to observe it — by default it
is silent. This is a real limitation, not a detail: whether MCP-RE signs one-way
notifications, rejects them outright, or formally declares a request/response-only subset
is undecided ([#418](https://github.com/matssun/mcp-re/issues/418)).
