...
one-way `notifications/*` message throws `NotificationsUnsupported`.
...
Until that lands, a standard `Client` cannot complete its lifecycle — it must send
`notifications/initialized` — so `unsafeDropNotifications: true` is the interim escape
