...
// Fixed clock: 2026-05-28T20:00:00Z (see mcp-re-core time tests).
...
fn sign_request_uses_injected_clock_and_rng() {
    ...
    let envelope = &value["params"]["_meta"]["se.syncom/mcp-re.request"];
    ...
    assert_eq!(nonce, mcp_re_core::b64url_encode(&[0xABu8; 16]));
...
fn verify_response_rejects_wrong_binding() {
    ...
    assert_eq!(result.err(), Some(McpReError::ResponseHashMismatch));
}
...
fn verify_response_for_unknown_id_is_missing_envelope() {
    ...
    let result = session.verify_response(&response, &server_resolver());
    assert_eq!(result.err(), Some(McpReError::MissingEnvelope));
}
...
fn signing_a_duplicate_in_flight_id_is_rejected() {
    ...
    assert_eq!(result.err(), Some(McpReError::ReplayDetected));
...
fn a_failed_verification_does_not_evict_the_pending_entry() {
    ...
    assert_eq!(
        ...
        Some(McpReError::ResponseHashMismatch)
    );
...
fn cancelled_id_refuses_response_correlation() {
    ...
    assert_eq!(
        ...
        Some(McpReError::MissingEnvelope)
    );
}
...
/// Networking/async crate substrings that must NEVER appear in `mcp-re-host`'s
/// declared dependencies. `mcp-re-host` is transport-free (ADR-MCPS-015): the
/// transport (stdio / Streamable HTTP / mTLS) is the caller's concern, deferred
/// to a future `mcp-re-host-transport` crate. Matching is on whole crate-name
...
const FORBIDDEN_TRANSPORT_CRATES: &[&str] = &[
    ...
    // `mcp-re-transport`, which may depend on `mcp-re-host` — never the reverse.
    // Forbidding the name here makes "mcp-re-host stays transport-free" enforceable,
    // not merely documented.
    "mcp-re-transport",
    "mcp-re-host-transport",
];
...
#[test]
fn mcp_re_host_carries_no_networking_or_async_dependencies() {
    ...
    assert!(
        offenders.is_empty(),
        "mcp-re-host must stay transport-free (ADR-MCPS-015): forbidden networking/async \
         crate(s) found in its dependency declarations: {offenders:?}. The transport belongs \
         in a future mcp-re-host-transport crate, not here."
    );
    ...
    assert!(cargo_tokens.contains("mcp-re-core"), "mcp-re-core dep present");
