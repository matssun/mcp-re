//! MCP-RE client-side ambassador (MCPS-014, ADR-MCPS-003).
//!
//! The host is the agent's local key/actor context. It injects and signs the
//! MCP-RE request envelope ([`HostSigner`]) and verifies signed server responses
...
//! networking/async and depends only on the pure `mcp-re-core` primitives.
...
// Response verification is exactly mcp-re-core's: the host re-exports it as the
...
pub use mcp_re_core::unwrap_verified_result;
pub use mcp_re_core::verify_response;
pub use mcp_re_core::McpReError;
pub use mcp_re_core::TrustResolver;
pub use mcp_re_core::UnwrappedResult;
pub use mcp_re_core::VerifiedResponse;
