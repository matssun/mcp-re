...
use mcp_re_core::UnwrappedResult;
...
/// (reachable only inside `mcp-re-host`), and `#[non_exhaustive]` blocks external
...
impl VerifiedResult {
    ...
    /// proof token cannot be forged from outside `mcp-re-host` (issue #83).
