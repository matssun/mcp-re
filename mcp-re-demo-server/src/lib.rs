//! `mcp-re-demo-server` — an ordinary, MCP-RE-UNAWARE, LONG-LIVED stdio MCP server
//! (MCPS-062).
//!
//! This is the in-tree demo target for MCP-RE-EPIC-P6.6B: a PERSISTENT MCP server
//! that the MCP-RE sidecar (`mcp-re-proxy`) will learn to front unchanged. It is
//! the long-lived counterpart of `mcp-re-demo-fileserver`: a single process
