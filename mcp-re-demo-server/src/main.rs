//! `mcp-re-demo-server` binary entrypoint (MCPS-062).
//!
...
//! Enable it with `--received-log <PATH>` or the `MCP_RE_DEMO_SERVER_RECEIVED_LOG`
...
//!
//! Usage:
//!   mcp-re-demo-server [--seed <ITEM>]... [--received-log <PATH>]
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("mcp-re-demo-server: {err}");
            ExitCode::FAILURE
        }
    }
}
