...
- **MCP-RE Core** — The pure-crypto verification layer (`mcp-re-core`) that signs/verifies MCP messages and is deliberately *method-transparent* (does not parse MCP method semantics).
...
- **Audit success vocabulary** — Only events the error enum cannot express: `mcp-re.request.accepted`, `mcp-re.response.signed`.
...
- **MTCI in the proposal** — Composability clarification only: conditional voice, no repo citation/link in the first proposal, not roadmap/subproject. One worked example as confusion-prevention (tool catalog is the adjacent domain most-confused with MCP-RE scope). Load-bearing line: "MCP-RE can protect messages that carry extension data, but does not define the semantics of those extensions."
...
- **draft-01 freeze (0.5)** — 0.5 adds zero wire-envelope fields; request/response envelopes unchanged. No in-release field-add path. A claim unsupportable by draft-01 is cut from 0.5 and ejected to a separate `draft-02` ADR (new field + threat model + tests as post-0.5 work). Scope-freeze line: "MCP-RE 0.5 is proposal-readiness over draft-01. No wire-envelope changes. Field gaps become draft-02 work."

_Avoid_: "small field addition" / "just one metadata field" / "NSA alignment field" inside 0.5.

- **MCP-RE discovery (advisory)** — A capability *hint* about whether a server speaks MCP-RE; an ALPN-like signal, never a security boundary. Resolved by grilling toward the Discovery & Enforcement ADR (next free number 043); **advisory only, must not weaken policy nor count as proof**.
- **Stateless-primary discovery** — The canonical MCP-RE discovery model: no pre-flight ask; `local client policy → signed MCP-RE request → verified signed response`. The first verified signed exchange *is* both the proof of MCP-RE support and the effective discovery result (HSTS posture, not ask-then-trust). Chosen because [[draft-01 freeze (0.5)]]'s successor ADR-MCPS-026 tracks the MCP 2026-07-28 stateless model that removes `initialize`/`initialized`.
- **Legacy/session discovery convenience** — For session transports that still run `initialize`, a server MAY advertise MCP-RE under `capabilities.experimental["se.syncom/mcp-re"]`; **non-canonical, advisory, never required for the stateless model, never proof**.
- **Client enforcement mode** — Local-only client/proxy policy: `require_mcp_re` (strict, fail-closed), `allow_legacy_explicit` (migration; legacy only where config permits), `opportunistic_mcp_re` (development only, possibly cut). Never server-directed; never travels on the wire as authority.
- **Server MCP-RE mode** — Server/proxy posture mirroring the client: `mcp_re_required` (reject unsigned/non-MCP-RE), `mixed_explicit` (accept both, legacy only by config), `legacy_only` (ordinary MCP, no MCP-RE claims). An MCP-RE endpoint rejects unsigned runtime calls; a separate legacy endpoint may exist but the *same* endpoint must not silently downgrade.
- **Trust anchor (no new infra)** — MCP-RE creates no trust anchors; it consumes them. Under `require_mcp_re` a `server_signer` must resolve to an already-trusted anchor via (1) OOB operator config / enterprise trust bundle (`expected_server_signers`) or (2) authenticated transport identity *verifiably bound* to the route/audience (config host→signer map, cert SAN/SPIFFE→signer, or cert-bound/signed assertion of the object-signing key). `server_signer_hint` is a lookup hint only — it can never introduce trust. **TOFU forbidden under `require_mcp_re`**; no anchor ⇒ strict mode unavailable ⇒ fail closed (never discover-and-trust). TOFU-with-pinning only in explicitly-labelled dev/permissive modes, never called enterprise security. Public signer registry / transparency log / first-contact bootstrap = **non-goal**. Reuses existing `TrustResolver` (`live_trust.rs`), OCSP (`ocsp.rs`), WebPKI/mTLS (ADR-028), revocation/rotation window (ADR-021).
...
- **Two-sided protocol** — Full MCP-RE requires a client-side component (signs requests, binds authz, tracks request hashes, verifies signed responses) plus the server-side verifier/signer. Three integration modes: native client (future ecosystem), **local client-side proxy (first adoption bridge)**, SDK wrapper (second). Priority fixed by [[post-v06-roadmap]]: proxy → Python SDK → demo, native later (TS SDK after Python).
- **Proxy transparency** — The local client speaks plain MCP to a local endpoint and never emits/parses/negotiates MCP-RE; all signing/verification/discovery/downgrade enforcement live in the proxy on the remote leg. Only OOB config (local URL, route name) + surfaced security errors leak. No MCP-RE field is ever an ordinary-client requirement.
...
- **Client error taxonomy = reuse `wire_code()`** — All client-side protocol/security failures map to the frozen `mcp-re-core` `wire_code()` taxonomy; no parallel client wire taxonomy. Local exception classes/UX hints allowed but each MUST map to a stable wire code (interop + proxy/SDK parity + audit drift guard).
- **Client key custody (mechanism-neutral, strict properties)** — Under `require_mcp_re` a client signing identity is mandatory (missing/revoked/unknown/policy-mismatch → fail closed). ADR mandates *properties not products*: signer identified in evidence; bound to route/audience/client-identity policy; private material non-exportable or delegated to a non-exporting signer where supported; production `require_mcp_re` MUST NOT use unprotected file keys; rotation/revocation by explicit config. Acceptable: OS keychain/Secure Enclave, hardware key, HSM/KMS, workload identity, mTLS-bound identity, delegated signing service. Hardware/KMS-only is a *hardening profile*, never the base rule. Dev file keys exist only in explicitly-labelled dev/test. **Two profiles (do not conflate): baseline property-based `require_mcp_re` MAY accept a configured software-held-private signer — explicitly NOT the high-assurance posture; the opt-in non-exporting hardening profile rejects software-held keys; unprotected dev-file keys are forbidden under production `require_mcp_re` in both. Enforced by `CustodyClass` + `SignerPolicy::require_non_exporting`, surfaced in the Python SDK as `Signer`/`SignerPolicy`.**
- **SDK wrap-or-fork rule** — An SDK wrapper is allowed only if the underlying SDK exposes hooks to sign the exact outbound bytes/canonical preimage before send and verify the exact inbound response bytes before app parsing. If not, **no transparent-wrapper claim** — fallback is a small transport adapter or minimal/forked client layer owning serialization boundaries. **First SDK = Python** (Anthropic's well-supported Python MCP SDK is easy to test/validate and the maintainer's stronger language; TypeScript follows).
