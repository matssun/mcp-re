...
fn freshness_window_overflow_fails_closed_instead_of_panicking() {
    ...
    let mut session = HostSession::new(
        ...
        SeededNonceSource::new(&[0xABu8; 32]),
        mcps_host::MAX_REQUEST_LIFETIME_SECS,
    )
...
fn new_rejects_a_lifetime_over_the_adr_ceiling() {
    let over = mcps_host::MAX_REQUEST_LIFETIME_SECS + 1;
...
fn new_accepts_the_ceiling_and_below() {
    for ok in [1i64, 60, mcps_host::MAX_REQUEST_LIFETIME_SECS] {
        assert!(
...
// --- MCPS_SPEC §4 id domain (producer-side, audit LOW) ------------------------
//
...
fn sign_request_rejects_out_of_domain_ids() {
    ...
    for id in out_of_domain {
        ...
        assert_eq!(
            result.err(),
            Some(McpsError::CanonicalizationFailed),
