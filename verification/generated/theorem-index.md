...
| THM-0028 | Channel-associated credential evidence cannot exist beside a relationship | proxy.channel_associated_credential | unit://proxy.channel_associated_credential | live |
...
### THM-0028 — Channel-associated credential evidence cannot exist beside a relationship

**Statement.** Every ChannelAssociatedCertificateCredentialEvidence value was produced by communication_assurance::rustls_established_channel::associated_credential from a connection the establishment mechanism reported as no longer handshaking, and carries the non-empty credential chain that mechanism reported for that same connection. The seal is what carries the claim. The representation is private to the product's module and the only constructor is `pub(super)`, so the mechanism adapter is the sole producer; a caller cannot assemble the value from a chain it obtained some other way, nor from no chain at all. The historical `TransportIdentity` — public fields, a total constructor — is exactly the shape this excludes. Establishment is a predecessor, not a decision made here. Each serving path calls the adapter after its own successful-establishment boundary: the async path after `TlsAcceptor::accept` has succeeded, the blocking path after the request read that drives the rustls handshake to completion. Because a `ServerConnection` exists before its handshake and therefore proves nothing, the adapter asks the mechanism rather than trusting the type, and declines to speak while the mechanism says establishment has not completed.
