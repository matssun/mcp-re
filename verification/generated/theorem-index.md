...
### THM-0096 — The runtime installs exactly the continuation capability the deployment selected

**Statement.** The MRTR continuation correlation capability the serving runtime holds is the one the validated continuation-control state selected. A deployment that selected no store installs NO correlation store — not a node-local or in-process substitute — and declares that absence explicitly. A deployment that selected one holds exactly that store and declares it ON. A selected store that cannot be established, including one this build carries no backend for, refuses startup and names what is missing; it is never announced as OFF.

**Security consequence.** An operator cannot end up with a deployment that reports a security capability's posture differently from the one it holds. A named store that fails to materialize cannot be reduced to the posture of an operator who named none, so cross-replica human-approval flows cannot silently become uncorrelated; and an absent capability cannot be widened into a node-local tier whose scope — one process, no replica switch — is not the one that was selected.

**Scope — what this does NOT establish.** CONFIGURATION PROJECTION ONLY. It establishes which capability the serving path holds and what the transcript says about it, not what a request may do with one. That a leg needing correlation fails closed, and that an absent capability reaches it as a deployment fact, is THM-0093's; that a running store is reachable only by the actor the verifier resolved is THM-0087's. Neither is restated here and neither is a premise of this. It says nothing about what an acknowledged write to the established store durably establishes. That is a per-mechanism external fact with its own premise, and this claim does not use it. It is not a liveness claim. A selected capability that cannot be established refuses startup, which is inside the claim rather than a violation of it, and nothing here says a deployment that selected the capability will keep it. That the plan is a faithful projection of the validated configuration is the planner's fact, reached through THM-0077 rather than asserted here. THE DECLARATION CONJUNCT IS CLAIMED AT THE STRENGTH ITS EVIDENCE HAS, which is less than it first appears. `PostureLog::assert_complete` proves that every executed startup states each seam exactly once, and it is NEVER REACHED hermetically — the posture phase sits after the replay tier is established, and every tier validation accepts needs a live Redis or etcd. What the registered evidence establishes is therefore two weaker facts: that the completeness check refuses an undeclared or doubly-declared seam when it is run, and that the continuation seam is declared exactly once in the composition root's SOURCE. "The path actually taken reached exactly one declaration" is not claimed by this theorem in any hermetic lane.
