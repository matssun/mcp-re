...
**Scope — what this does NOT establish.** RUNTIME CORRELATION AND REACHABILITY ONLY. It is about which actor can reach a live continuation entry and about the entry surviving a read that admits nothing — not about deployment posture, and not about whether the shared tier this deployment holds is the one its configuration selected. That materialization claim is THM-0077's and is deliberately not restated here. The store is OPPORTUNISTIC, which is why no "prevents startup" conjunct appears: an unavailable shared tier does not refuse startup — the deployment announces its absence and serves — and an answer leg that needs a correlated continuation fails closed at the binding rather than being admitted unbound. That is where this claim departs from the shape of its replay sibling. The provenance half and the separation half have different kinds of evidence, and the difference is stated rather than averaged. That an actor operand can only be the verifier's resolution is STRUCTURAL: `ResolvedActorId` has no constructor but one taking a `ResolvedActor`. Which operand each leg passes is MEASURED, by `unit://proxy.continuation_key_provenance`, and its two controls are defended by mutation probe M88 — because a positive assertion is also satisfied by an implementation that reads the request and happens to agree. It says nothing about what an acknowledged write durably establishes, and nothing about the continuation binding itself, which is `mcp-re-http-profile`'s.
