...
**Security consequence.** The pod's projected service-account token, the live workload-identity bearer token, and the `getPublicKey` response that BECOMES this replica's root verify key each have exactly one destination, and it is the one the validated configuration selected. R9's only Critical was this class: an endpoint that re-points that traffic sends a replayable credential to an attacker and lets a substituted authority supply a root signing key every local fail-closed check then passes against, self-consistently. The previous shape failed the deletion test rather than the threat model. Each key source called the endpoint rule and DISCARDED its answer, then built a bare HTTP client and interpolated the endpoint back into a URL string at each call site; deleting the call left a constructor that still built, still reached the endpoint, and still sent the credential. The check was being remembered at five sites. A later site that formatted its own URL — or a responder answering `302 Location:` — reached whatever it named.

**Scope — what this does NOT establish.** It is about the AUTHORITY NAMED, not the address that name resolves to. Resolved-address vetting is installed for a certificate-derived destination only, and deliberately not for an operator-configured one: an operator may legitimately point a KMS endpoint at an internal address, which is what keeps the emulator lane working. So a name admitted by THM-0089 can still resolve wherever DNS says, and neither claim closes that. It says nothing about what a KMS or STS does with a request that reaches it, nothing about key custody or exportability (THM-0064, THM-0082), and nothing about the metadata server's own trustworthiness — on every production path that endpoint is a constant, and the parameter that varies it exists for tests. EVIDENCE AND LANES, stated because they differ. The addressing conjunct is pure and its controls run in the default `cargo test -p mcp-re-proxy --lib` lane that this unit's `test://` evidence resolves in. The redirect conjunct and the single-producer structure are compiled only under the `online_ocsp`, `aws_kms_keysource` and `gcp_kms_keysource` features, so their controls — `outbound_fetch::binding::tests::an_agent_does_not_follow_a_redirect_and_never_reaches_the_second_authority` and `kms_endpoint_policy::endpoint::tests::an_endpoint_the_rule_refuses_yields_no_egress` — are measured by the Bazel `:mcp_re_proxy_ext` flavor and by the per-feature cargo lanes, and are NOT cited by the unit. A build with none of those features compiles no credential egress at all, so there is nothing for the claim to be about there.
