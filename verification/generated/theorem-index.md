...
| THM-0094 | The shipped Python SDK accepts only an answer to its own request |
| THM-0095 | The shipped TypeScript SDK accepts only an answer to its own request |
...
| THM-0087 | A continuation entry is reachable only by the actor the verifier resolved | proxy.continuation_correlation_store | unit://proxy.continuation_correlation_store, unit://proxy.continuation_key_provenance | live |
...
| THM-0090 | A credential leaves this proxy only to the endpoint its rule admitted | proxy.outbound_destination | unit://proxy.outbound_destination | live |
...
**Scope — what this does NOT establish.** RUNTIME CORRELATION AND REACHABILITY ONLY. It is about which actor can reach a live continuation entry and about the entry surviving a read that admits nothing — not about deployment posture, and not about whether the shared tier this deployment holds is the one its configuration selected. That materialization claim is THM-0077's and is not restated here. The store is OPPORTUNISTIC, which is why no "prevents startup" conjunct appears: an unavailable shared tier does not refuse startup — the deployment announces its absence and serves — and an answer leg that needs a correlated continuation fails closed at the binding rather than being admitted unbound. That is where this claim departs from the shape of its replay sibling. THE PROVENANCE IS COMPOSED, NOT LOCAL, and the composition is named because the alternative is to imply a stronger local fact than exists. THM-0051 establishes that the product the serving path carries is the one THIS exchange's verification returned, and THM-0014 — inside THM-0051's own closure — that the actor in it was resolved through the trust seam. What `unit://proxy.continuation_key_provenance` measures is the remaining step: that both legs supply that product's actor projection to the key and nothing else, defended by probe M88. No type makes this true by construction, and none is claimed to. `ResolvedActor` is deliberately unsealed and legitimately constructible through the resolver seam, so a projection of one establishes only what it was handed; the provenance is earned by the chain above and by the controls, never by a constructor's argument type. It says nothing about what an acknowledged write durably establishes, and nothing about the continuation binding itself, which is `mcp-re-http-profile`'s.
...
**Scope — what this does NOT establish.** It ends at the endpoint TEXT. What it establishes is agreement between what a reader sees and what a URL parser resolves — NOT that the address finally connected to is one this deployment would accept. Under a rebinding-capable threat model a name that passes here can still resolve to an address the deployment would refuse, and this claim must not be read as closing that gap. What it now also does is decide something: `KmsEndpoint` is the only producer in the crate of credential egress to a KMS/STS endpoint, so a key source cannot reach one without this decision. That is a fact about who must obey the rule, not a widening of the rule — the question answered is still the agreement between two readings of one string. Where the credential then goes is THM-0090. It says nothing about what a KMS does with a request that reaches it, nothing about key custody or exportability (THM-0064's and THM-0082's), and nothing about whether the provider selected was the right one for the deployment.

**Review requirement.** Owner security-specification review

### THM-0090 — A credential leaves this proxy only to the endpoint its rule admitted

**Statement.** Every credential-bearing outbound request this proxy issues — AWS KMS, the AWS STS web-identity exchange, GCP Cloud KMS, and the GCE metadata token fetch — is issued through a capability that was bound at construction to a destination which passed the outbound-destination guard, and is addressed by naming a PATH on that destination. No path moves the authority: the join places everything the caller supplied after a separator that terminates the base's authority, so a protocol-relative reference, an absolute URL, userinfo, a backslash or a tab names a path and not a host. The capability exposes no other way to address a request — there is no constructor taking a URL and no projection handing back a raw client — and the client it performs the request with is the one the destination's provenance built, with redirects disabled, so the responder cannot move the authority either. For a KMS or STS endpoint the destination is obtainable only from `KmsEndpoint`, so the rule of THM-0089 is not a call a constructor may omit: omitting it removes the value the constructor needs.

**Security consequence.** The pod's projected service-account token, the live workload-identity bearer token, and the `getPublicKey` response that BECOMES this replica's root verify key each have exactly one destination: the authority embodied by the vetted capability the request was built through. Neither caller-controlled addressing nor a responder's redirect can move it. R9's only Critical was this class: an endpoint that re-points that traffic sends a replayable credential to an attacker and lets a substituted authority supply a root signing key every local fail-closed check then passes against, self-consistently. WHAT THIS LEAF DOES NOT SAY is that the capability corresponds to the posture the validated deployment configuration selected. That is a composition over this claim and the configuration owner's, and it is stated at THM-0077, which consumes this one. Read here it would be a claim about configuration made by an owner that reads none. The previous shape failed the deletion test rather than the threat model. Each key source called the endpoint rule and DISCARDED its answer, then built a bare HTTP client and interpolated the endpoint back into a URL string at each call site; deleting the call left a constructor that still built, still reached the endpoint, and still sent the credential. The check was being remembered at five sites. A later site that formatted its own URL — or a responder answering `302 Location:` — reached whatever it named.

**Scope — what this does NOT establish.** It is about the AUTHORITY NAMED, not the address that name resolves to. Resolved-address vetting is installed for a certificate-derived destination only, and deliberately not for an operator-configured one: an operator may legitimately point a KMS endpoint at an internal address, which is what keeps the emulator lane working. So a name admitted by THM-0089 can still resolve wherever DNS says, and neither claim closes that. It says nothing about what a KMS or STS does with a request that reaches it, nothing about key custody or exportability (THM-0064, THM-0082), and nothing about the metadata server's own trustworthiness — on every production path that endpoint is a constant, and the parameter that varies it exists for tests. EVERY CONJUNCT IS INSIDE THE SUPPORTING UNIT'S EVIDENCE CLOSURE, and the reason it needs saying is that they are not all compiled by the same crate. The addressing conjunct is pure. The redirect conjunct and the capability's absence of a second addressing surface are behind `online_ocsp`, `aws_kms_keysource` and `gcp_kms_keysource`, so `unit://proxy.outbound_destination` declares those features and its battery is selected in the crate where the controls exist. The single-producer conjunct is THM-0089's — the fact that a KMS or STS destination is obtainable only from `KmsEndpoint` belongs to the endpoint owner, is measured by `unit://proxy.kms_endpoint_authority` under the two backend features, and reaches this claim through the dependency rather than being re-derived here. A build with none of those features compiles no credential egress at all, so there is nothing for the claim to be about there.
...
**Statement.** Under the fleet-strict posture, replay admission refuses before any store side effect when the deployment declares no replay tier, declares one below the strict-production minimum, or wires a store that self-reports the single-process reference class. The atomic admission is the LAST step, and a store that does not answer it refuses the exchange rather than admitting it — reported as an outage, never as a replay. Where the store acknowledges admission under a tier at or above the strict-production minimum, replay state is treated as established ACCORDING TO THE SELECTED MECHANISM'S REGISTERED PREMISE, and this theorem makes no durability claim stronger than that premise. Under Redis, ASM-0040 establishes the configured replica acknowledgements and carries its documented failover caveat. Under etcd, ASM-0041 establishes the successful linearized transaction and the lease-bound key. The two are not one proposition, and neither is restated here in stronger terms than the premise that carries it.

**Security consequence.** Replay admission cannot degrade to fail-open on infrastructure trouble. The excluded outcome is not a crash — it is a deployment that believes it has cross-replica replay protection and has process-local protection, serving a replayed request as a fresh one because the store it was supposed to consult was unreachable or was never the store the posture claimed. The outage/replay distinction is part of the claim rather than a diagnostic nicety, and both sides of it are narrower than they read. A REPLAY verdict says the replay key was already admitted and recorded under the mechanism, so the same signed request cannot be treated as fresh; it does not say the backend dispatched, and it does not say the backend executed — admitted, dispatched and executed are three states, not one. An OUTAGE says the proxy did not obtain the replay-establishment fact dispatch requires, so this attempt does not reach the backend; it does not say no store-side write occurred, because a write can happen and its acknowledgement be lost. Collapsing the two would let one verdict answer for the other, and neither is a client retry policy — what a caller should do next is decided elsewhere, from these facts and not in place of them.

**Scope — what this does NOT establish.** It is one-directional and is not a liveness claim: it says an unestablished replay state does not dispatch, never that an established one does. The two gates it composes are of different kinds and the claim keeps them apart. The DECLARED tier is the operator's statement, checked here; the store's self-reported durability class is the object's own, checked beneath. A deployment can get the first right and the second wrong, which is why both refuse, and why neither is called a check on the other. Outside fleet-strict the tier gate deliberately does not fire, and this claim says nothing about such a deployment beyond what the core gate beneath still enforces. What an acknowledgement durably established is a FOREIGN fact, registered per mechanism. Nothing here establishes that Redis's replication or etcd's consensus behaves as its premise states, and no umbrella premise stands in for both. The mechanisms are NOT equally strong and this claim does not level them. `REDIS_WAIT_QUORUM` is not linearizable replay durability and is not a guarantee that every later failover preserves the nonce: ASM-0040 establishes replica acknowledgements and explicitly declines the failover case, which ADR-MCPS-020 treats as materially reduced failover replay risk rather than as replay safety. Etcd under ASM-0041 is stronger, by consensus rather than by count. A deployment therefore obtains the strength of the tier it wired, and reading the positive direction at the stronger mechanism's altitude would be a claim no premise here carries.
...
**Statement.** Where a request carries a continuation, the retained open-leg bases are read WITHOUT being spent, and no way they can be absent yields a binding: the dispatcher fails closed rather than admitting an unbound continuation. The absences are ATTRIBUTED, not merely detected. A store that does not ANSWER, and a deployment that holds no correlation capability while this leg needs one, are refused before admission and named as facts about the DEPLOYMENT. Only the caller-side absences — the request names no `requestState`, or the store answered and the entry was never opened, has expired, or was already answered — leave no binding and let the leg be refused as the caller's. The spend is the store's atomic consume and has four outcomes, because the store's error is not its negative answer: nothing was at stake, this call spent the approval, the store answered and there was nothing live to spend, or the store did not answer and whether the entry is gone cannot be determined by anything downstream.

**Security consequence.** A signed continuation that cannot be bound to a live approval is never admitted, so a second verified actor cannot complete a human approval round trip that was not its own by presenting a continuation the deployment cannot check. The read is free, so a request about to be refused for an unrelated reason leaves a live approval intact — the refusals above the retirement stay free only because nothing above them spent anything. A deployment's own missing capability is never reported as the caller's forgery. Without that conjunct the fail-closed behaviour is still safe and its diagnosis is false: every legitimate answer leg reaching a deployment that selected no correlation store reads on the wire as a splice attempt, and a genuine splice attempt is indistinguishable from an ordinary deployment posture. An operator cannot act on either, and a defender cannot tell them apart. The four-valued retirement preserves the observed fate of the approval without inventing certainty: no approval was involved, this exchange spent it, the store answered and there was nothing live to spend, or its fate is indeterminate. These are four different facts about a person's decision, and they are carried forward so the exchange-lifecycle authority can derive the consequence. THM-0093 does not decide retry policy and does not assert that any particular retry will succeed; what it establishes is that the four facts reach that authority undistorted rather than collapsed into one another.

**Scope — what this does NOT establish.** It is about what a leg may PROCEED on and how a request-time absence is ATTRIBUTED — never about what the deployment was built or configured with. Which capability the runtime installs for the plan it was given, and what the startup transcript says about it, is THM-0096's and is deliberately not restated here. This claim observes its own `ContinuationPlane` state and consumes no proposition from THM-0096, which is why no edge to it exists: both discuss absence, and they establish different things. It says nothing about the continuation binding check itself, which is `mcp-re-http-profile`'s and is where a bound-but-wrong continuation is caught, and nothing about what an acknowledged store write durably establishes. The `Indeterminate` retirement is reported, not resolved. Nothing downstream can find out whether the entry was consumed, and this claim does not pretend otherwise — what it establishes is that the outcome is carried as its own case rather than collapsed into one of the other three.
...
**Statement.** For the shipped Python SDK, an application is not handed, as this call's answer, a response from another exchange or signer, or one that verified only unbound — and is not led to repeat a side effect by reading silence as *it did not run*. Every reply is taken from the correlation entry the request created, and a reply binding to nothing outstanding, arriving late, or repeating one already answered is refused rather than delivered. A response is accepted only under the configured trust anchor, epoch, audience and revocation posture, measured against a RECORDED delegated session rather than a constructed one. A notification is delivered only against a signed acknowledgement. An elicitation pauses a call rather than ending it, so a reply that is not terminal — including one whose result type this SDK does not recognise — is refused rather than surfacing as the call's answer. A verified reply that is not an answer to this request — one carrying a method, or one with neither result nor error — is refused rather than dispatched. What the application is told about execution is what the receipt said: whether the receipt was bound to THIS transmission, the execution and retry contract a post-dispatch rejection stated, no member the receipt did not carry, and a local failure under the SDK's own prefix rather than a wire code the peer never sent.

**Security consequence.** An application driving MCP through this SDK cannot be handed another exchange's answer, an answer from a signer this deployment does not trust, or a refusal that verified only unbound — and cannot be told that an operation did not run when the SDK does not know that. The last is the one a byte-level fixture cannot reach. Parity fixtures pin what this SDK EMITS; whether it invents `not_executed` for a receipt that stated nothing is behaviour, and the two SDKs have diverged there before.

**Scope — what this does NOT establish.** It is one MEMBER of a root family, not the family. THM-0076 is the Rust member and THM-0095 the TypeScript one; none of the three establishes anything about the others, and a green parity fixture is not a substitute — it compares bytes, and every divergence this family exists for is behavioural. CONCURRENCY AND THROUGHPUT ARE OUTSIDE IT. The exchange bound is a resource ceiling, and a difference from the TypeScript member there changes nothing about authenticity, correlation or execution. Deadline behaviour enters only where a timeout changes the client's conclusion about whether the remote side may have executed. One such deadline is REGISTERED RATHER THAN CLAIMED. The aggregate read bound is evaluated between completed `read(n)` calls, and `http.client` fills to `n` — so a trickling peer keeps one call outstanding and the bound is not consulted while it does (ASM-0042, R9-C010). The consequence stated here is exactly what follows and no more: the SDK reports NOTHING for such an exchange rather than reporting that it did not execute. The looseness reaches this claim only if a caller's own outer timeout produces a retry whose execution status the SDK never determined, and that condition is stated rather than assumed away. It says nothing about the transport below it beyond what ASM-0042 names, nothing about the proxy's own guarantees, and nothing about an application that bypasses this adapter.

**Review requirement.** Owner security-specification review

### THM-0095 — The shipped TypeScript SDK accepts only an answer to its own request

**Statement.** For the shipped TypeScript SDK, an application is not handed, as this call's answer, a response from another exchange or signer, or one that verified only unbound — and is not led to repeat a side effect by reading silence as *it did not run*. Every reply is taken from the correlation entry the request created, and a reply binding to nothing outstanding, arriving late, or repeating one already answered is refused rather than delivered; concurrent exchanges each receive their own. A response is accepted only under a complete configured trust anchor — an incomplete one is refused at construction rather than defaulted — and only outside the revocation denylist, whose shape is checked so a bare string cannot spread into a per-character list. A notification is delivered only against a signed acknowledgement. A verified reply that is not a JSON-RPC response to this request is refused rather than dispatched. What the application is told about execution is what the receipt said: a post-dispatch rejection carries the contract it stated, a receipt that stated none is given none, and a local failure travels under the SDK's own prefix.

**Security consequence.** The same consequence as the Python member, over an independently implemented boundary — and the reason both exist is that "independently implemented" is a measured fact here rather than a caution. The parity fixtures are green while the two diverge behaviourally, so a root over one of them would report coverage of the other that no evidence supports. `invents no disposition for a receipt that stated none` is the control that names the difference. Inventing `not_executed` collapses "unknown whether it ran" into "it did not run" at the one place that decides whether a caller repeats a side effect.

**Scope — what this does NOT establish.** It is one MEMBER of a root family. THM-0076 is the Rust member and THM-0094 the Python one, and none establishes anything about the others. CONCURRENCY AND THROUGHPUT ARE OUTSIDE IT, and the exclusion is a decision rather than an omission: `send_notification_verified()` running outside the concurrency bound its Python twin enforces (R9-C061, R9-C109, R9-C110) is a resource ceiling and changes nothing about authenticity, correlation or execution. ONE DEADLINE IS INSIDE IT, and it is the reason deadlines are not excluded wholesale. `timeoutMs` is both the per-socket inactivity bound and an aggregate wall clock on reading a response (R9-C095), so this SDK will abort a response whose TOTAL download exceeds it — a request the server may have executed. What this claim establishes is that the abort surfaces as a LOCAL transport outcome carrying no execution assertion, not as a clean failure a caller could read as "it did not run". The aggregate bound itself is deliberate: the inactivity bound alone bounds nothing, because every byte re-arms it. ASM-0043 carries what the abort race rests on: that an already-aborted signal is observable before dispatch, and that aborting an in-flight exchange settles the caller's promise without recalling what the server already received.

**Review requirement.** Owner security-specification review

### THM-0096 — The runtime installs exactly the continuation capability its plan names

**Statement.** For the `ContinuationControlPlan` presented to continuation materialization, a plan naming NO store produces no correlation store — not a node-local or in-process substitute — and an explicit OFF posture. A plan naming a shared store materializes and installs exactly that store and declares the capability ON. A named store that cannot be provided by this build, or that cannot be established, refuses startup and names what is missing; it is never announced as OFF.

**Security consequence.** Continuation materialization cannot silently weaken or widen the plan it receives. A named shared store cannot become OFF, and cannot become another store or a node-local one whose scope — one process, no replica switch — is not the scope the plan names; so cross-replica human-approval flows cannot silently become uncorrelated. A plan naming no store cannot acquire a fallback capability. The posture the runtime reports is the posture it holds.

**Scope — what this does NOT establish.** PLAN → RUNTIME ONLY. It establishes which capability the serving path holds for the plan it was handed, and what the transcript says about it, not what a request may do with one. That a leg needing correlation fails closed, and that an absent capability reaches it as a deployment fact, is THM-0093's; that a running store is reachable only by the actor the verifier resolved is THM-0087's. Neither is restated here and neither is a premise of this. IT DOES NOT ESTABLISH WHERE THE PLAN CAME FROM. THM-0096 says nothing about the origin of the `ContinuationControlPlan` and does not establish that the plan faithfully represents validated deployment configuration. That is a parent composition/configuration fact. This leaf therefore does not say that the OPERATOR or the VALIDATED DEPLOYMENT selected what the runtime installs; it says only that the runtime installs what its plan names. The direction is one way, and this claim never reaches its premise downward through its own consumer: validated configuration -> (parent composition facts) -> `ContinuationControlPlan` -> (THM-0096) -> runtime capability and posture -> (composed by) THM-0077's root claim. THM-0077 combines the parent composition fact with THM-0096's plan → runtime materialization proposition; THM-0096 is a premise of THM-0077 and obtains nothing from it. It says nothing about what an acknowledged write to the established store durably establishes. That is a per-mechanism external fact with its own premise, and this claim does not use it. It is not a liveness claim. A named store that cannot be established refuses startup, which is inside the claim rather than a violation of it, and nothing here says a runtime that installed the capability will keep it. THE DECLARATION CONJUNCT IS CLAIMED AT THE STRENGTH ITS EVIDENCE HAS, which is less than it first appears. `PostureLog::assert_complete` proves that every executed startup states each seam exactly once, and it is NEVER REACHED hermetically — the posture phase sits after the replay tier is established, and every tier validation accepts needs a live Redis or etcd. What the registered evidence establishes is therefore two weaker facts: that the completeness check refuses an undeclared or doubly-declared seam when it is run, and that the continuation seam is declared exactly once in the composition root's SOURCE. "The path actually taken reached exactly one declaration" is not claimed by this theorem in any hermetic lane.
