...
| R9-C079 | medium | `retention-marker` | release_before_dispatch() and pending_reservations() have no production caller — both are test-only | `SURVIVES_AND_MAPPED` | no THM | Same. | #741 |
| R9-C080 | medium | `retention-marker` | release_before_dispatch() and pending_reservations() have no production caller — R8-C093 is API-only | `SURVIVES_AND_MAPPED` | no THM | Same. | #741 |
| R9-C081 | medium | `retention-marker` | Inner-plane saturation still leaves a permanent retention marker when discovered at dispatch | `SURVIVES_AND_MAPPED` | THM-0045 | Same as C004. | #741 |
