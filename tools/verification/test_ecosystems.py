...
def test_the_dependency_manifest_and_the_lockfile_are_measured():
    ...
    declared source line. `pyproject.toml` and `uv.lock` bear exactly the weight the Rust
    unit's `Cargo.toml` and workspace `Cargo.lock` bear."""
    build = fingerprint(PY_UNIT)["components"]["build_configuration"]
    assert "sdk/python/pyproject.toml" in build
    assert "sdk/python/uv.lock" in build
...
def test_an_absent_lockfile_alternative_contributes_nothing_rather_than_failing():
    ...
    assert "sdk/python/poetry.lock" not in fingerprint(PY_UNIT)["components"]["build_configuration"]
...
def test_each_ecosystem_selects_exactly_the_declared_symbols():
    ...
    assert py[:3] == ["python3", "-m", "pytest"]
    assert py[-1] == "tests/test_correlation.py::test_probe"
    ts = test_argv(TYPESCRIPT, "sdk/typescript", "vitest", ["test/x.test.ts > a probe"])
    assert ts[:3] == ["npx", "vitest", "run"]
    assert ts[-1] == "test/x.test.ts > a probe"
